...
function existingSatisfies(need, capability) {
    if (!capability.visible || !capability.observationComplete || !capability.authorityKnown)
        return false;
...
function authorityFlags(entry) {
    ...
    for (const permission of entry.permissions) {
        if (!PERMISSION_KINDS.has(permission.kind) || !PERMISSION_ACCESS.has(permission.access))
            return null;
        if (permission.access !== 'none')
            flags.push(`${permission.kind}:${permission.access}`);
    }
...
function maximumAllows(need, flags) {
    ...
    return flags.every((flag) => {
        ...
        if (flag.startsWith('credentials:'))
            return maximum.has('credentials');
        if (flag.startsWith('model-context:'))
            return maximum.has('model-context');
        return false;
    });
}
function requiredAccessSatisfied(need, flags) {
...
function retrieve(input, need) {
    ...
    for (const entry of input.catalog) {
        ...
        if (flags === null || !maximumAllows(need, flags) || !requiredAccessSatisfied(need, flags))
            continue;
        const tags = normalizedTags(entry.tags);
        const matchedTags = need.outcomeTags.filter(tag => tags.includes(tag));
        if (matchedTags.length !== need.outcomeTags.length)
            continue;
    ...
    return values.sort((left, right) => right.projection.score - left.projection.score
...
export function resolveCapability(input) {
    ...
    const alternatives = candidates.slice(1).filter(candidate => candidate.projection.score * 1.25 >= best.projection.score && materiallyDifferent(best, candidate));
