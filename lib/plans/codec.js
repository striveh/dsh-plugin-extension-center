...
import { IMMUTABLE_PLAN_SCHEMA_VERSION, } from "./types.js";
import { decodePlanReviewEvidence } from "./review-codec.js";
import { isCurrentPnpmExecutionIdentity, isReadablePnpmExecutionIdentity, } from "./pnpm-runtime.js";
...
function decodeRecoveryExecutableBindingWithIdentity(value, path, readRetired) {
    ...
    if (officialDsh.schemaVersion !== 2 || officialDsh.packageName !== '@deepseek-ai/dsh'
        || officialDsh.packageVersion !== '0.1.2-alpha.1' || !isAbsolute(packageRoot)
    ...
    const productionDependencies = officialDsh.productionDependencies.map((dependency, index) => {
        ...
        if (!isAbsolute(dependencyRoot))
            failDomain('invalid-data', `${dependencyPath}.packageRoot must be absolute`);
        return {
    ...
    return immutableJsonClone({
        ...
        officialDsh: {
            ...
            packageRoot,
            packageTreeSha256: readSha256Digest(officialDsh.packageTreeSha256, `${path}.officialDsh.packageTreeSha256`),
            productionDependencies,
            entrypointPath,
            entrypointSha256: readSha256Digest(officialDsh.entrypointSha256, `${path}.officialDsh.entrypointSha256`),
            hostHome,
            timeoutMs,
            node: {
                schemaVersion: 1,
            ...
            supervisorPath,
            supervisorSha256: readSha256Digest(officialDsh.supervisorSha256, `${path}.officialDsh.supervisorSha256`),
            pnpm: {
                schemaVersion: 1,
                packageName: 'pnpm',
                ...pnpmIdentity,
                packageRoot: pnpmPackageRoot,
                packageTreeSha256: readSha256Digest(pnpm.packageTreeSha256, `${path}.officialDsh.pnpm.packageTreeSha256`),
                entrypointPath: pnpmEntrypoint,
                entrypointSha256: readSha256Digest(pnpm.entrypointSha256, `${path}.officialDsh.pnpm.entrypointSha256`),
                shimPath: pnpmShim,
 ...
 */
export function decodeRecoveryExecutableBinding(value, path = 'recoveryExecutable') {
 ...
 */
export function decodeStoredRecoveryExecutableBinding(value, path = 'recoveryExecutable') {
    return decodeRecoveryExecutableBindingWithIdentity(value, path, true);
}
function assertManagedObjectBinding(value, path) {
 ...
 */
export function decodePlanContent(value) {
    const record = readStrictRecord(value, [
        'schemaVersion',
        'singleUse',
        'planId',
    ...
    if (record.singleUse !== true)
        failDomain('invalid-data', 'plan.content.singleUse must be true');
    ...
    if (createdAtMs >= expiresAtMs)
        failDomain('invalid-data', 'plan validity interval is invalid');
    const decoded = {
 ...
 */
export function createImmutablePlan(value) {
    const content = decodePlanContent(value);
    return immutableJsonClone({ content, hash: canonicalSha256(content) });
}
/**
 ...
 */
export function decodeImmutablePlan(value) {
    ...
    if (canonicalSha256(content) !== hash)
        failDomain('plan-integrity', 'plan hash does not match its content');
    return immutableJsonClone({ content, hash });
}
/**
 ...
 */
export function assertImmutablePlan(plan) {
 ...
 */
export function decodePlanDecisionInput(value) {
 ...
 */
export function decodePlanUseContext(value) {
    const record = readStrictRecord(value, ['operationKind', 'targetKey', 'ownerKey', 'scopeKey', 'profileId', 'fences'], 'planContext');
    return immutableJsonClone({
...
function decodeOperationAuthorizationWithRecovery(value, readRetired) {
    const record = readStrictRecord(value, [
        'operationId',
        'planId',
        'planHash',
    ...
    const decoded = immutableJsonClone({
        ...
        recoveryExecutable: (readRetired ? decodeStoredRecoveryExecutableBinding : decodeRecoveryExecutableBinding)(record.recoveryExecutable, 'authorization.recoveryExecutable'),
 ...
 */
export function decodeOperationAuthorization(value) {
 ...
 */
export function decodeStoredOperationAuthorization(value) {
    return decodeOperationAuthorizationWithRecovery(value, true);
}
function decodeDecisionRecord(value) {
    const record = readStrictRecord(value, ['planId', 'planHash', 'operationKind', 'decision', 'decidedAtMs'], 'planState.decision');
    return immutableJsonClone({
...
function assertStateBindings(state) {
    const { plan } = state;
    ...
    if (decision.planId !== plan.content.planId
 ...
 */
function decodePlanAuthorizationStateWithRecovery(value, readRetired) {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
        failDomain('invalid-data', 'planState must be an object');
    }
    const status = readLiteral(value.status, ['pending', 'approved', 'rejected', 'expired', 'consumed'], 'planState.status');
    if (status === 'pending') {
 ...
 */
export function decodePlanAuthorizationState(value) {
 ...
 */
export function decodeStoredPlanAuthorizationState(value) {
