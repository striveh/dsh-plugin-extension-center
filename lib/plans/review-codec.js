...
export function decodePlanReviewEvidence(value, path = 'plan.content.reviewEvidence') {
    ...
    if (kind === 'plugin') {
        ...
        if (lockfile.path !== 'pnpm-lock.yaml')
            failDomain('invalid-data', `${path}.lockfile.path is unsupported`);
        ...
        if (new Set(bundles.map(item => item.id)).size !== bundles.length)
            failDomain('invalid-data', `${path}.bundles contains duplicates`);
