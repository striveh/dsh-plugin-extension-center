...
import { createOperationJournal, issueOperationReceipt, operationJournalCheckpoint, recordOperationMutation, recordOperationVerification, transitionOperation, verifyOperationJournal, } from "../operations/index.js";
import { isCurrentPnpmExecutionIdentity, } from "../plans/index.js";
import { admittedAuthorityDigest, verificationRecipeDigest } from "../policy/index.js";
import { isOfficialProfileAmbiguityError, OFFICIAL_PROFILE_AMBIGUITY_CODE, } from "../internal/plugin/types.js";
import { managedStateDigest } from "../providers/records.js";
import { HOST_RPC_PROTOCOL_VERSION } from "./rpc-contract.js";
import { centerManagementAuthorityDigest, isCenterManagementOperation } from "./management-admission.js";
...
const RETIRED_RUNTIME_QUARANTINE = 'retired-runtime-quarantined';
function currentRecoveryRuntime(authorization) {
    return isCurrentPnpmExecutionIdentity(authorization.recoveryExecutable.officialDsh.pnpm);
}
function planForReservation(reservation, states) {
...
function authorizationJournalBinding(authorization) {
...
function projectionJournalBinding(projection) {
    return Object.freeze({
...
export class OperationRunner {
    ...
    intentPlans;
    providers;
    explicitRecoveryFlights = new Set();
    constructor(state, plans, operations, locks, fetcher, intentPlans, providers) {
        this.state = state;
        this.plans = plans;
    ...
    async list() {
        ...
        for (const reservation of reservations) {
            if (operationIds.has(reservation.operationId))
                continue;
    ...
    async retiredPluginObligations(signal) {
        const obligations = [];
        ...
        for (const loaded of operations) {
            ...
            const quarantined = !terminal(loaded.projection.phase)
                ...
                || await this.retiredFailedPluginReferencesMutation(loaded, consumed);
            if (quarantined)
                this.assertExactRetiredTargetQuarantine(locks, loaded.projection.targetKey, loaded.projection.operationId);
            if (consumed.plan.content.extensionKind !== 'plugin' || !quarantined)
                continue;
        ...
        for (const reservation of await this.operations.listReservations()) {
            ...
            if (consumed.plan.content.extensionKind !== 'plugin'
                || obligationIds.has(consumed.authorization.operationId))
                continue;
            obligationIds.add(consumed.authorization.operationId);
            obligations.push(Object.freeze({
                operationId: consumed.authorization.operationId,
    ...
    async recoverOperation(operationId, signal) {
        if (this.explicitRecoveryFlights.has(operationId))
            throw new Error('operation recovery is already in progress');
    ...
    async settleManagedPluginRestart(request, loaded, applied, signal) {
        signal.throwIfAborted();
        ...
        if (request.plan.extensionKind !== 'plugin')
            throw new Error('operation is not a managed Plugin mutation');
        ...
        if (applied.restartToken === null)
            throw new Error('Plugin operation has no exact restart token');
        ...
        try {
            ...
            await this.finishTerminal(request, journal, loaded.projection.phase === 'rolling-back' ? () => provider.finalizeRollback(applied) : undefined);
        }
        catch (error) {
            if (signal.aborted)
                throw signal.reason;
            if (terminal(verifyOperationJournal(journal).phase))
                throw error;
            if (loaded.projection.phase === 'rolling-back') {
                await this.append(request, transitionOperation(journal, 'recovery-required', null, cleanupFailed ? 'cleanup-failed' : 'rollback-verification-failed', Date.now()));
                return;
            }
            journal = await this.append(request, transitionOperation(journal, 'rolling-back', null, cleanupFailed ? 'cleanup-failed' : null, Date.now()));
            try {
    ...
    assertForwardRestartBinding(request, applied) {
    ...
    assertRollbackRestartBinding(request, applied) {
        if (request.plan.extensionKind !== 'plugin')
            throw new Error('operation is not a managed Plugin mutation');
    ...
    async settleManagedPluginRollback(request, journal, rollback, signal) {
        ...
        if (rollback.rollbackRestartRequired) {
            ...
            if (current === undefined)
                throw new Error('managed Plugin rollback operation disappeared');
        ...
        signal.throwIfAborted();
        if (verification === null)
            throw new Error('Plugin live rollback verification is unavailable');
        journal = await this.append(request, recordOperationVerification(journal, verification.digest, Date.now()));
        signal.throwIfAborted();
        await provider.cleanup(rollback.prepared);
        signal.throwIfAborted();
        journal = await this.append(request, transitionOperation(journal, 'rolled-back', rollback.prepared.beforeDigest, null, Date.now()));
        signal.throwIfAborted();
    ...
    async rollbackManagedPlugin(request, journal, applied, signal) {
        const provider = this.providers.plugin;
        ...
        signal.throwIfAborted();
        if (rollback === null)
            throw new Error('Plugin rollback evidence is unavailable');
    ...
    async recover(signal) {
        ...
        for (const held of await this.locks.list()) {
            ...
            const consumed = plans.find((state) => state.status === 'consumed'
        ...
        for (const loaded of await this.operations.list()) {
            ...
            if (held !== undefined && !recoverable.has(loaded.projection.operationId))
                continue;
    ...
    async recoverLoaded(loaded, signal) {
        ...
        if (loaded.projection.phase === 'recovery-required') {
            ...
            const applied = await plugin.reconcileBreakGlassRestore(request, loaded.projection.beforeDigest, operationJournalCheckpoint(loaded.journal).headDigest);
            signal.throwIfAborted();
            if (applied === null)
                return;
        ...
        if (request.plan.extensionKind === 'plugin' && applied.restartToken !== null) {
            try {
                const current = await this.operations.load(request.authorization.operationId);
                if (current === undefined)
                    throw new Error('managed Plugin operation disappeared');
    ...
    async recoverRetiredLoaded(loaded, consumed, signal) {
        signal.throwIfAborted();
    ...
    async ensureRetiredTargetQuarantine(targetKey, operationId) {
        this.assertExactRetiredTargetQuarantine(await this.locks.list(), targetKey, operationId);
    }
    assertExactRetiredTargetQuarantine(locks, targetKey, operationId) {
    ...
    async retiredFailedPluginReferencesMutation(loaded, consumed) {
        if (loaded.projection.phase !== 'failed'
            || consumed.plan.content.extensionKind !== 'plugin'
            || loaded.projection.mutationDigests.length !== 0
            || !loaded.journal.events.some(event => (event.entry.type === 'phase-transition' && event.entry.to === 'applying'))) {
            return false;
        }
        ...
        return await provider.referencesDurableOperation(consumed.authorization.operationId, consumed.plan.content.targetKey, consumed.plan.content.profileId);
    }
    /** Retry task bookkeeping without making an already terminal target depend on its intent payload. */
    async recoverTaskReceipt(plan, authorization, receipt, signal) {
        if (plan.content.origin !== 'task')
            return;
        try {
    ...
    async execute(plan, authorization, reservation, signal) {
        ...
        try {
            ...
            if (isOfficialProfileAmbiguityError(error)) {
                journal = await this.toRecoveryRequired(request, journal, OFFICIAL_PROFILE_AMBIGUITY_CODE, cleanupFailed ? 'cleanup-failed' : null);
                return this.response(authorization.operationId, verifyOperationJournal(journal), null);
            }
            if (applied === undefined) {
                try {
    ...
    async recoverReservations(signal, recoverable) {
        signal.throwIfAborted();
        const locks = await this.locks.list();
        signal.throwIfAborted();
        const plans = await this.plans.list();
        for (const reservation of await this.operations.listReservations()) {
            signal.throwIfAborted();
            if (!recoverable.has(reservation.operationId))
                continue;
    ...
    async providerRequest(plan, authorization, artifactPath, signal) {
        ...
        if (plan.content.verificationDigest !== verificationRecipeDigest(plan.content.extensionKind, plan.content.operationKind, plan.content.desiredState))
    ...
    async finishTerminal(request, journal, afterReceipt) {
        const issued = issueOperationReceipt(journal, Date.now());
    ...
    async toRecoveryRequired(request, journal, failure, rollbackReason = null) {
        const loaded = await this.operations.load(request.authorization.operationId);
        if (loaded !== undefined)
            journal = loaded.journal;
        const phase = verifyOperationJournal(journal).phase;
        if (terminal(phase) || phase === 'recovery-required')
            return journal;
        if (phase !== 'rolling-back') {
    ...
    async claimExactRecoveryLease(targetKey, operationId) {
        ...
        if (claim === 'live')
            throw new Error('operation target lease is owned by a live Host');
        if (claim === 'unknown')
            throw new Error('operation target lease identity cannot be verified');
    }
