...
import { evaluateCandidatePolicy, mintAcquisitionIntent, candidateAdmissionFacts, currentHostPlatform, operationRestartRequired, verificationRecipeDigest, } from "../policy/index.js";
import { hasPluginConfigurationAdapter, pluginConfigurationMutationDigest, preflightSkillConfiguration, } from "../providers/index.js";
...
export class IntentPlanService {
    ...
    preflight;
    planTtlMs;
    constructor(store, plans, inventory, owners, catalog, managedPlugins, preflight, planTtlMs = 15 * 60 * 1000) {
        this.store = store;
        this.plans = plans;
    ...
    async configurationOptions(input) {
        ...
        if (entry === undefined || !entry.scopes.includes(input.scopeKey)) {
        ...
        if (input.targetKey !== null) {
            ...
            if (record === undefined
                ...
                || (input.operationKind === 'restore' && (selected.candidateRef !== entry.candidateRef
    ...
    async ownerRevision(entry, row, skillProjectRoot, managedPluginSnapshot) {
        if (entry.kind === 'plugin')
            return managedPluginSnapshot.ownerRevision;
        if (entry.kind === 'mcp')
