...
import { skillCandidate } from "../kind-candidates.js";
import { operationRestartRequired } from "../policy/index.js";
...
function pluginManifestBody(entry) {
    const candidate = capabilityResolverCandidate(entry.candidateRef, entry.artifact.version);
    if (candidate === null)
        throw new Error('Plugin has no exact package manifest review record');
    const manifest = JSON.parse(PLUGIN_MANIFEST_TEMPLATE_BODY);
...
async function pluginEvidence(input) {
    ...
    const dependencies = [
        { kind: 'extension', id: input.entry.artifact.id, beforeVersion: before?.artifactRevision ?? null, afterVersion, required: true },
    ...
    return immutableJsonClone({
        ...
        rollbackLimits: [
            ...
            ...(restartRequired ? ['restart-required-before-runtime-proof'] : []),
        ],
        notProven: [
            'catalog-admission-is-not-security-audit',
            'third-party-code-side-effects',
            ...(restartRequired ? ['post-restart-consumer'] : []),
            'user-task-outcome',
        ],
        manifest: {
...
async function skillEvidence(input) {
    ...
    if (candidate === null)
        throw new Error('Skill has no exact package review record');
