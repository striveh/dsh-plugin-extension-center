...
import { createInventorySnapshot } from "../inventory/index.js";
import { candidateUpdateSuccessor } from "../kind-candidates.js";
import { inspectSkillArtifact } from "../providers/skill-provider.js";
...
function kindState(value) {
    ...
    if (typeof state !== 'object' || state === null || Array.isArray(state))
        throw new Error('managed kind state is invalid');
    return state;
}
...
function exactCatalogObservation(catalog, target, version) {
    if (version === null)
    ...
    if (entry === undefined
        ...
        || !entry.scopes.includes(target.scopeKey)
...
function updateObservation(catalog, target, version) {
    if (version === null)
        return Object.freeze({ status: 'unknown' });
    ...
    if (current === undefined
        ...
        || !current.scopes.includes(target.scopeKey)
...
export class HostInventoryService {
    ...
    inspectManagedMcp;
    capabilities;
    constructor(store, owners, catalog, managedPlugins, inspectManagedMcp = null, capabilities = () => hostCapabilities(owners)) {
        this.store = store;
        this.owners = owners;
        this.catalog = catalog;
        this.managedPlugins = managedPlugins;
        this.inspectManagedMcp = inspectManagedMcp;
        this.capabilities = capabilities;
    }
    /** Observe one exact scope/profile without mutating desired state. */
    async list(scopeKey, profileId, projectRoot = null) {
        const managed = (await this.store.listManaged()).filter(row => row.scopeKey === scopeKey && row.profileId === profileId);
        const rows = [];
        for (const row of managed)
            rows.push(await this.centerRow(row));
    ...
    async verify(scopeKey, profileId, targetKey, projectRoot = null) {
    ...
    async externalRows(scopeKey, profileId, managed, projectRoot) {
        const output = [];
        ...
        const catalogTarget = (kind, name) => {
            const entry = catalog.find(candidate => candidate.kind === kind
                && candidate.name === name
                && candidate.scopes.includes(scopeKey));
        ...
        if (this.owners.skills !== null
            ...
            for (const item of view.skills.map(record).filter((item) => item !== undefined)
                .sort((left, right) => String(left.name).localeCompare(String(right.name)))) {
                if (typeof item.name !== 'string' || item.provider === 'extension-center')
                    continue;
                const identity = catalogTarget('skill', item.name);
                if (knownTargets.has(identity.targetKey))
                    continue;
        ...
        if (this.owners.mcpConnections !== null && scopeKey === 'profile:web') {
            ...
            for (const raw of [...snapshot.connections].map(record).filter((item) => item !== undefined)
                .sort((left, right) => String(left.id).localeCompare(String(right.id)))) {
                if (typeof raw.id !== 'string' || knownMcpConnections.has(raw.id))
                    continue;
                ...
                const names = Array.isArray(tools?.names) ? tools.names.filter((name) => typeof name === 'string').sort() : [];
                ...
                output.push({
                    ...
                    evidence: {
                        ...
                        toolGeneration: Number.isSafeInteger(tools?.generation) ? tools.generation : null,
                        qualifiedTools: names,
                    },
                });
            }
            for (const raw of [...snapshot.removed].map(record).filter((item) => item !== undefined)
                .sort((left, right) => String(left.id).localeCompare(String(right.id)))) {
                if (typeof raw.id !== 'string' || knownMcpConnections.has(raw.id))
                    continue;
                const identity = catalogTarget('mcp', raw.id);
    ...
    async skillEvidence(value, version) {
        if (version === null || this.owners.skills === null)
            return {
        ...
        if (inspection === null || !inspection.matchesExpected)
            return {
                kind: 'skill',
        ...
        const winner = snapshot.skills.map(record).find(item => item?.name === state.skillName);
        let loaded;
        try {
    ...
    async mcpEvidence(version) {
        if (version === null || this.owners.mcpConnections === null || this.inspectManagedMcp === null)
            return {
    ...
    async pluginEvidence(value, version) {
