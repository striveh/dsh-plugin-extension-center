...
const ACCESS = ['filesystem-read', 'filesystem-write', 'network', 'subprocess'];
...
function capabilityFacts(outcomeTags, dataAccess) {
...
export class CapabilityAcquisitionService {
        ...
        try {
            ...
            for (const entry of eligibleEntries) {
                if (!entry.scopes.includes(input.scopeKey)
                    || !input.outcomeTags.some(tag => entry.tags.includes(tag)))
                    continue;
    ...
    async existing(input, agent, cwd) {
        ...
        for (const schema of this.owners.tools.schemas?.(agent) ?? []) {
            ...
            if (typeof name !== 'string' || ['capability_resolve', 'capability_request_acquisition'].includes(name))
                continue;
