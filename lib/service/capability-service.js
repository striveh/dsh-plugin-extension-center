...
import { TASK_CONTINUATION_INVALID_REASONS, } from "../internal/continuation/types.js";
import { taskAcquisitionCandidates } from "../kind-candidates.js";
import { candidateAdmissionFacts, currentHostPlatform, evaluateCandidatePolicy, } from "../policy/index.js";
import { resolveCapability, } from "../resolution/index.js";
import { FileTaskAttemptStore, } from "../task-attempt/index.js";
...
const MODALITIES = ['audio', 'file', 'image', 'structured-data', 'text', 'video'];
const DATA_ACCESS = ['filesystem-read', 'filesystem-write', 'network', 'subprocess'];
const AUTHORITY = ['credentials', 'filesystem-read', 'filesystem-write', 'model-context', 'network', 'subprocess'];
...
function stringSet(value, name, allowed, allowEmpty = false) {
    if (!Array.isArray(value) || (!allowEmpty && value.length === 0) || value.length > 16
        || value.some(item => typeof item !== 'string'))
        throw new Error(`${name} must be a bounded string set`);
    const values = value;
    if (allowed === undefined) {
        if (values.some(item => !TAG.test(item)))
            throw new Error(`${name} contains an invalid tag`);
    }
    else if (values.some(item => !allowed.includes(item))) {
...
function modelNeed(value) {
    ...
    if (!exactRecord(value, keys)
        ...
        throw new Error('capability need fields are invalid');
...
function acquisitionRequest(value) {
...
function capabilityFacts(outcomeTags, dataAccess, authority = dataAccess) {
    return Object.freeze({
...
const TOOL_CAPABILITY_FACTS = Object.freeze({
...
function toolCapabilityFacts(name, description) {
...
function currentPlatform() {
    ...
    if (platform === 'unsupported')
        throw new Error('capability acquisition is unavailable on this Host platform');
    return platform;
}
function rowCandidate(row) {
    const order = ['configure', 'enable', 'update', 'restore'];
...
function parseResolutionValue(value) {
    if (!exactRecord(value, [
        ...
        throw new Error('stored task resolution is invalid');
    const candidates = value.candidates.map((item) => {
        if (!exactRecord(item, ['candidateRef', 'configuration', 'operationKind', 'targetKey'])
            ...
            || (item.targetKey !== null && typeof item.targetKey !== 'string'))
            throw new Error('stored resolution candidate is invalid');
        return item;
    });
    if (typeof value.decision !== 'string'
        ...
        throw new Error('stored resolution binding is invalid');
    restrictedAgentOptions(value.resumeAgentOptions);
    return value;
}
function restrictedAgentOptions(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    ...
    if (Object.keys(input).some(key => !['maxTokens', 'model', 'provider'].includes(key))
        ...
        || (input.maxTokens !== undefined && (!Number.isSafeInteger(input.maxTokens)
            || input.maxTokens <= 0
            || input.maxTokens > MAX_CONTINUATION_RESUME_TOKENS))) {
...
function validObservedDispatch(value, state) {
    ...
    if ((hasOwner && (typeof ownerId !== 'string' || !SAFE_ID.test(ownerId)))
        || (hasLease && (!Number.isSafeInteger(lease) || lease < 0))
        || (hasStarted && (!Number.isSafeInteger(started) || started < 0))
        || (hasReason && (typeof reason !== 'string'
            || !DELIVERY_UNKNOWN_REASONS.includes(reason))))
        return false;
    if (!hasOwner) {
        return fence === 0 && !hasLease && !hasStarted && !hasReason
            && !['dispatching', 'dispatched', 'claimed', 'delivery-unknown'].includes(state);
    }
    if (typeof fence !== 'number' || fence < 1)
        return false;
    if (state === 'dispatching')
        return hasLease && !hasReason;
    if (state === 'dispatched' || state === 'claimed')
        return !hasLease && hasStarted && !hasReason;
    if (state === 'delivery-unknown')
        return !hasLease && hasStarted && hasReason;
    return state === 'invalid' && !hasReason;
}
function observedRetryContinuationClaim(value) {
    const baseKeys = [
    ...
    return Object.freeze({
        ...
        ...(hasSupersededRevision
            ? { supersededByTaskRevision: value.supersededByTaskRevision }
            : {}),
        ...(hasInvalidReason ? { invalidReason: value.invalidReason } : {}),
    });
}
function agentExecution(agent) {
    if (typeof agent !== 'object' || agent === null)
        throw new Error('capability resolution requires an Agent-scoped execution');
    const view = agent;
    if (typeof view.id !== 'string' || !Array.isArray(view.session?.events)
        || typeof view.session.header?.cwd !== 'string')
        throw new Error('Agent session identity and cwd are unavailable');
...
export class CapabilityAcquisitionService {
    ...
    ttlMs;
    taskAttempts;
    volatileResolutions = new Map();
    taskAttemptInitialization;
    continuationReconciliationRequested = false;
    continuationReconciliation;
    constructor(state, inventory, intentPlans, plans, operations, owners, catalog, ttlMs = 15 * 60 * 1000, taskAttempts = new FileTaskAttemptStore(state.root)) {
        this.state = state;
    ...
    async resolve(value, agent, signal) {
        if (signal.aborted)
            throw signal.reason;
        ...
        if (!host.acquisition)
            throw new Error('capability acquisition is unavailable on this Host');
        const need = {
            schemaVersion: 1,
    ...
    async selectTaskCandidate(taskAttemptId, candidateRef, signal) {
        if (!CANDIDATE_REF.test(candidateRef))
            throw new Error('task candidate selection is invalid');
        await this.ensureTaskAttempts();
        const source = await this.taskAttempts.get(taskAttemptId);
        const nowMs = Date.now();
        if (source === undefined || source.expiresAtMs <= nowMs)
            throw new Error('task choice is absent or expired');
    ...
    async retryOriginalTask(taskAttemptId, signal) {
        await this.ensureTaskAttempts();
        const source = await this.taskAttempts.get(taskAttemptId);
        const nowMs = Date.now();
        if (source === undefined || source.expiresAtMs <= nowMs)
            throw new Error('task management handoff is absent or expired');
        const attempt = await this.taskAttempts.derive({
            sourceAttemptId: taskAttemptId,
            kind: 'retry-original',
            candidateRef: null,
    ...
    async cancelTaskAttempt(taskAttemptId) {
        await this.ensureTaskAttempts();
        const prior = await this.taskAttempts.get(taskAttemptId);
        if (prior === undefined)
            throw new Error('task attempt is absent');
        if (prior.outcome === 'canceled')
            return await this.projectAttempt(prior);
        ...
        const closed = await this.taskAttempts.close(prior.taskAttemptId, prior.revision, 'canceled', 'canceled-by-user', prior.result, Date.now());
    ...
    async listTaskAttempts(nowMs = Date.now()) {
        await this.ensureTaskAttempts();
    ...
    async evaluateAttempt(initial, agent, signal, selectedCandidateRef) {
        let attempt = initial;
        try {
            if (signal.aborted) {
            ...
            let existing;
            try {
                inventory = await this.inventory.list(input.scopeKey, input.profileId, input.scopeKey === 'project' ? attempt.projectRoot : null);
                existing = await this.existing(input, agent, attempt.projectRoot);
            }
            catch {
                attempt = await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, 'discovery-unavailable', 'runtime-observation-unavailable', null, Date.now());
                return this.modelProjection(attempt, 'discovery-unavailable');
            }
            const need = attempt.need;
            ...
            if (existingResolution.decision === 'use-existing') {
                attempt = await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, 'use-existing', 'existing-capability-visible', { kind: 'use-existing', capabilityId: existingResolution.capabilityId }, Date.now());
            ...
            try {
                ...
                attempt = await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, 'discovery-unavailable', 'verified-catalog-unavailable', null, Date.now());
                return this.modelProjection(attempt, 'discovery-unavailable');
            }
            ...
            for (const entry of eligibleEntries) {
                if (!entry.scopes.includes(input.scopeKey)
                    || !input.outcomeTags.every(tag => entry.tags.includes(tag)))
                    continue;
            ...
            if (resolved.decision === 'management-required') {
                ...
                attempt = await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, 'management-required', `managed-extension-requires-${resolved.action}`, result, Date.now());
            ...
            if (resolved.decision === 'choice-required') {
                ...
                attempt = await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, 'choice-required', 'material-candidate-choice-required', { kind: 'choice-required', candidateRefs: refs }, Date.now());
                return this.modelProjection(attempt, resolved.decision);
            }
            if (resolved.decision === 'no-eligible-candidate' || resolved.decision === 'discovery-unavailable') {
                attempt = await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, resolved.decision, resolved.decision, null, Date.now());
                return this.modelProjection(attempt, resolved.decision);
            }
            if (resolved.decision !== 'acquisition-candidate')
                throw new Error('task capability decision is unsupported');
            ...
            const stored = {
                ...
                value: immutableJsonClone(valueRecord),
            };
            ...
            attempt = await this.taskAttempts.transition(attempt.taskAttemptId, attempt.revision, 'awaiting-approval', {
            ...
            if (current?.outcome === null) {
                await this.taskAttempts.close(current.taskAttemptId, current.revision, signal.aborted ? 'canceled' : 'failed', signal.aborted ? 'request-canceled' : 'resolution-failed', current.result, Date.now());
            }
            throw error;
        }
    }
    /** Mint a task-origin immutable plan from three opaque bindings; never approve or execute it. */
    async request(value, agent, signal) {
        if (signal.aborted)
            throw signal.reason;
        ...
        if (attempt.outcome !== null)
            throw new Error('acquisition task attempt is terminal');
        ...
        if (candidate.configuration === null)
            throw new Error('task candidate requires trusted human configuration');
        ...
        const durableResolution = Object.freeze({
            ...resolution,
            value: immutableJsonClone(durableDetail),
        });
    ...
    async listConfigurationRequests(nowMs = Date.now(), maximum = 128) {
        ...
        for (const resolution of resolutions.values()) {
            ...
            for (const candidate of detail.candidates) {
                ...
                if (output.length > maximum)
                    throw new Error('task configuration queue exceeds its P0 bound');
            }
        }
    ...
    async configureTaskCandidate(input, nowMs = Date.now()) {
        await this.ensureTaskAttempts();
        ...
        const resolution = this.volatileResolutions.get(input.resolutionId)
            ?? await this.state.getResolution(input.resolutionId);
        if (resolution === undefined || resolution.expiresAtMs <= nowMs)
            throw new Error('task configuration resolution is absent or expired');
        const detail = parseResolutionValue(resolution.value);
        const candidate = detail.candidates.find(item => item.candidateRef === input.candidateRef);
    ...
    async activateApprovedPlan(planHash) {
        const plan = await this.plans.load(planHash);
        if (plan?.status !== 'approved' || plan.plan.content.origin !== 'task')
            return;
        ...
        if (matches.length > 1)
            throw new Error('approved task plan has duplicate continuation claims');
    ...
    async recoverApprovedPlans(signal) {
    ...
    registerVerifier() {
        const acquisition = this.owners.taskContinuations.registerVerifier({
            ...
            verify: async (claim, signal) => {
                ...
                if (typeof view.continuationId !== 'string'
                    ...
                    || typeof view.verificationPayloadDigest !== 'string')
                    return { kind: 'not-ready' };
                const receipt = await this.state.getTaskReceipt(view.continuationId);
                if (receipt === undefined
                    || receipt.verificationPayloadDigest !== view.verificationPayloadDigest)
                    return { kind: 'not-ready' };
                ...
                const plan = await this.plans.load(receipt.planHash);
                ...
                if (resolution === undefined
                    ...
                    || operation.projection.receipt?.digest !== receipt.operationReceiptDigest)
                    return { kind: 'not-ready' };
                let inventory;
                try {
                    inventory = await this.inventory.list(detail.scopeKey, detail.profileId, detail.scopeKey === 'project' ? attempt.projectRoot : null);
                }
                catch {
                    return { kind: 'not-ready' };
                }
                ...
                if (satisfied.decision !== 'use-existing')
                    return { kind: 'not-ready' };
                if (!await this.markTaskVisibilityReady(attempt.taskAttemptId))
                    return { kind: 'not-ready' };
                return Object.freeze({
                    kind: 'ready',
        ...
        const retry = this.owners.taskContinuations.registerVerifier({
            ...
            verify: (claim, signal) => this.verifyRetryContinuation(claim, signal),
        });
        const owner = this.owners.taskContinuations;
    ...
    /** Bind a loopback plan decision to its independent task attempt. */
    async recordPlanDecision(planHash, decision) {
        await this.ensureTaskAttempts();
        const state = await this.plans.load(planHash);
        if (state === undefined || state.plan.content.origin !== 'task')
            return;
        ...
        const nowMs = Date.now();
        if (attempt.outcome !== null) {
            if (decision === 'reject' && attempt.outcome === 'rejected')
                return;
            throw new Error('task plan belongs to a terminal attempt');
        }
        if (attempt.expiresAtMs <= nowMs) {
        ...
        if (decision === 'reject') {
            await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, 'rejected', 'human-rejected-plan', attempt.result, nowMs);
            return;
        }
        if (attempt.phase === 'acquiring')
            return;
        if (attempt.phase !== 'awaiting-approval')
            throw new Error('task attempt is not awaiting approval');
        attempt = await this.taskAttempts.transition(attempt.taskAttemptId, attempt.revision, 'acquiring', attempt.result, nowMs);
    }
    /** Project operation progress without using it as the task's terminal outcome. */
    async recordLifecycleResult(planHash, status) {
        await this.ensureTaskAttempts();
        const state = await this.plans.load(planHash);
        if (state === undefined || state.plan.content.origin !== 'task')
            return;
        ...
        if (resolutionId === null || resolutionId === undefined)
            return;
        const attempt = await this.taskAttempts.getByResolution(resolutionId);
        if (attempt === undefined || attempt.outcome !== null)
            return;
        const nowMs = Date.now();
        if (status === 'failed' || status === 'rolled-back' || status === 'recovery-required') {
            await this.taskAttempts.close(attempt.taskAttemptId, attempt.revision, status === 'recovery-required' ? 'recovery-required' : 'failed', `extension-operation-${status}`, attempt.result, nowMs);
            return;
        }
        ...
        if (current.phase === 'acquiring') {
            current = await this.taskAttempts.transition(current.taskAttemptId, current.revision, 'verifying-visibility', current.result, nowMs);
        }
        await this.owners.taskContinuations.reconcile();
    }
    async ensureTaskAttempts() {
        this.taskAttemptInitialization ??= this.taskAttempts.initialize();
        await this.taskAttemptInitialization;
    }
    scheduleTaskContinuationReconciliation() {
    ...
    async reconcileTaskContinuations() {
        ...
        while (true) {
            ...
            try {
                ...
                if (this.continuationReconciliation === run)
                    this.continuationReconciliation = undefined;
            }
            if (!this.continuationReconciliationRequested)
                return;
        }
    }
    async runTaskContinuationReconciliation() {
        await this.ensureTaskAttempts();
        while (this.continuationReconciliationRequested) {
            this.continuationReconciliationRequested = false;
            for (const attempt of await this.taskAttempts.list()) {
                if (attempt.outcome !== null || attempt.result?.kind !== 'acquisition-candidate')
                    continue;
                const claim = await this.observeAcquisitionContinuationClaim(attempt);
                if (claim === undefined)
                    continue;
                await this.taskAttempts.reconcileContinuation(attempt.taskAttemptId, claim.state, Date.now(), claim.invalidReason);
            }
        }
    }
    async observeAcquisitionContinuationClaim(attempt) {
        if (attempt.result?.kind !== 'acquisition-candidate' || this.owners.taskContinuations === null)
            return undefined;
        ...
        if (activationIntent === undefined)
            throw new Error('task continuation activation intent is absent');
        const value = await this.owners.taskContinuations.get(activation.continuationId);
        if (value === undefined)
            throw new Error('activated task continuation claim is absent');
    ...
    modelProjection(attempt, decision, humanConfiguration = false) {
    ...
    async markTaskVisibilityReady(taskAttemptId) {
        try {
            let current = await this.taskAttempts.get(taskAttemptId);
            if (current === undefined || current.outcome !== null)
                return false;
            if (current.phase === 'acquiring') {
                current = await this.taskAttempts.transition(current.taskAttemptId, current.revision, 'verifying-visibility', current.result, Date.now());
            }
            if (current.phase === 'verifying-visibility' || current.phase === 'restart-required') {
                current = await this.taskAttempts.transition(current.taskAttemptId, current.revision, 'ready-to-resume', current.result, Date.now());
            }
    ...
    async projectRetryContinuation(attempt) {
    ...
    async observeRetryContinuationClaim(attempt, binding) {
        const callerId = 'extension-center';
        ...
        if (!Array.isArray(values))
            throw new Error('Retry original continuation owner returned an invalid list');
        ...
        if (matches.length > 1)
            throw new Error('Retry original has duplicate continuation claims');
    ...
    async ensureRetryContinuation(attempt) {
        if (attempt.trigger !== 'retry-original'
        ...
        if (binding.canceledAtMs !== null && found === undefined)
            return;
        ...
        if (binding.continuationId !== claim.continuationId)
            throw new Error('Retry original continuation binding failed');
    ...
    async verifyRetryContinuation(claim, signal) {
        if (signal.aborted)
            throw signal.reason;
        await this.ensureTaskAttempts();
        if (typeof claim !== 'object' || claim === null || Array.isArray(claim))
        ...
        if (attempts.some(other => other.taskAttemptId !== attempt.taskAttemptId
            ...
            && other.createdAtMs >= binding.createdAtMs))
            return { kind: 'not-ready' };
        let inventory;
        let existing;
        try {
            inventory = await this.inventory.list(attempt.need.scopeKey, attempt.profileId, attempt.need.scopeKey === 'project' ? attempt.projectRoot : null);
            existing = await this.existing({
                outcomeTags: attempt.need.outcomeTags,
                inputModalities: attempt.need.inputModalities,
                outputModalities: attempt.need.outputModalities,
        ...
        if (!inventory.complete
            ...
            || resolved.capabilityId !== binding.existingCapabilityId)
            return { kind: 'not-ready' };
        return Object.freeze({
            kind: 'ready',
            continuationId: view.continuationId,
    ...
    async cancelContinuation(attempt) {
        if (attempt.result?.kind !== 'acquisition-candidate')
            return;
        const activation = await this.state.getContinuationActivation(attempt.result.continuationId);
        if (activation === undefined)
            return;
    ...
    async supersedeContinuation(attempt, replacementTaskAttemptId) {
        if (attempt.result?.kind !== 'acquisition-candidate')
            return;
        const activation = await this.state.getContinuationActivation(attempt.result.continuationId);
        if (activation === undefined)
    ...
    async existing(input, agent, cwd) {
        const output = [];
        const visibleToolAccess = new Set();
        const visibleToolAuthority = new Set();
        ...
        for (const schema of this.owners.tools.schemas?.(agent) ?? []) {
            ...
            if (typeof name !== 'string' || EXTENSION_CENTER_TOOL_NAMES.has(name))
                continue;
            const facts = toolCapabilityFacts(name, description);
            for (const item of facts.dataAccess)
                visibleToolAccess.add(item);
            for (const item of facts.authority)
                visibleToolAuthority.add(item);
            if (!facts.authorityKnown)
                visibleToolAuthorityKnown = false;
            output.push({
        ...
        for (const raw of skills.skills) {
            if (typeof raw !== 'object' || raw === null || Array.isArray(raw))
                continue;
            const item = raw;
            if (typeof item.name !== 'string' || item.invocation?.modelInvocable !== true)
                continue;
            output.push({
                ...
                authority: Object.freeze([...visibleToolAuthority, 'model-context'].sort()),
    ...
    pruneVolatileResolutions(nowMs) {
        for (const [id, resolution] of this.volatileResolutions) {
            if (resolution.expiresAtMs <= nowMs)
                this.volatileResolutions.delete(id);
        }
    }
    async resolutionCandidates(resolution, rows, entries, input, cwd) {
        ...
        for (const candidateRef of refs) {
            ...
            if (entry === undefined)
                continue;
            const configuration = entry.kind === 'mcp'
                ? null
                : entry.kind === 'skill'
...
export function capabilityToolDefinitions(service) {
    ...
    return Object.freeze([{
            ...
            execute: (args, exec) => service.resolve(args, exec.agent, exec.signal),
            ...
            output: { schema: { type: 'object', additionalProperties: true }, render: (_args, value) => text(value) },
            execute: (args, exec) => service.request(args, exec.agent, exec.signal),
