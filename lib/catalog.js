...
import { EXTENSION_CENTER_PROTOCOL_VERSION, } from "./catalog-contract.js";
import { BOOTSTRAP_CATALOG_ENVELOPE, BOOTSTRAP_CATALOG_ROOT, BOOTSTRAP_CATALOG_SIGNATURES, } from "./catalog-data.js";
import { FILESYSTEM_MCP_CANDIDATES, SKILL_CANDIDATES } from "./kind-candidates.js";
import { CAPABILITY_RESOLVER_CANDIDATES } from "./resolver-candidates.js";
...
export function catalogReviewEvidenceSupport(entry) {
    const identity = {
        kind: entry.kind,
...
export function canonicalJson(value) {
    ...
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            throw new Error('catalog canonical JSON rejects non-finite numbers');
        return JSON.stringify(value);
    }
    ...
    if (typeof value === 'object') {
        ...
        return `{${Object.keys(record).sort().map((key) => {
            ...
            if (child === undefined)
                throw new Error(`catalog canonical JSON rejects undefined field ${key}`);
...
export function verifyCatalog(root, envelope, signatures, now = Date.now()) {
    if (root.catalogId !== envelope.catalogId)
        throw new Error('catalog id does not match the packaged trust root');
    if (envelope.revision < root.minimumRevision)
        throw new Error('catalog revision is below the packaged minimum');
    ...
    if (now < issuedAt)
        throw new Error('catalog is not valid yet');
    if (now >= expiresAt || now - issuedAt > root.maximumAgeMs)
        throw new Error('catalog is expired');
    if (canonicalSha256(envelope.entries) !== envelope.entriesDigest)
        throw new Error('catalog entries digest mismatch');
    const refs = new Set();
    for (const entry of envelope.entries) {
        if (refs.has(entry.candidateRef))
            throw new Error(`duplicate candidateRef: ${entry.candidateRef}`);
        refs.add(entry.candidateRef);
        if (!entry.candidateRef.startsWith(`${entry.kind}:`))
            throw new Error(`candidate kind mismatch: ${entry.candidateRef}`);
        ...
        if (entry.scopes.length === 0)
            throw new Error(`candidate has no target scope: ${entry.candidateRef}`);
        ...
        if ((entry.license.status === 'unknown' && (entry.license.spdx !== null || entry.license.sourceUrl !== null))
    ...
    for (const signature of signatures) {
        ...
        if (key === undefined || key.algorithm !== signature.algorithm || accepted.has(signature.keyId))
            continue;
        if (verify(null, bytes, createPublicKey(key.publicKeyPem), Buffer.from(signature.value, 'base64'))) {
            accepted.add(signature.keyId);
        }
    }
    if (accepted.size < root.threshold)
        throw new Error('catalog signature threshold was not met');
    return { envelope, keyIds: [...accepted].sort() };
}
/** Verify the packaged offline bootstrap catalog. */
export function verifyBootstrapCatalog(now = Date.now()) {
    return verifyCatalog(BOOTSTRAP_CATALOG_ROOT, BOOTSTRAP_CATALOG_ENVELOPE, BOOTSTRAP_CATALOG_SIGNATURES, now);
}
/** Project a verified catalog onto the private browser RPC. */
export function catalogListResponse(catalog, hostCapabilities = {
    ...
    if ((hostCapabilities.acquisition && !ownersAvailable)
