...
import { readArtifactIntegrity, readBoundedString, readLiteral, readNonNegativeInteger, readSha256Digest, readStrictRecord, } from "../domain/codec.js";
import { failDomain } from "../domain/errors.js";
import { canonicalSha256, immutableJsonClone } from "../domain/index.js";
import { storageKey } from "./files.js";
const KINDS = ['mcp', 'plugin', 'skill'];
const OPERATIONS = ['configure', 'disable', 'enable', 'install', 'purge', 'restore', 'uninstall', 'update'];
const DESIRED_STATES = ['disabled', 'enabled', 'removed'];
const PHASES = [
    'applying',
    'authorized',
    'committed',
...
function schema(record, path) {
    if (record.schemaVersion !== 1)
        fail(path, 'has an unsupported schemaVersion');
}
...
function nullableString(value, path, maximumLength = 512) {
    return value === null ? null : readBoundedString(value, path, maximumLength);
}
function exactOptionalRecord(value, permitted, path) {
...
function rpcJson(value, path, depth = 0, budget = { nodes: 0 }) {
    budget.nodes += 1;
    if (budget.nodes > MAX_RPC_NODES)
        fail(path, 'exceeds the durable JSON node bound');
    if (depth > MAX_RPC_DEPTH)
        fail(path, 'exceeds the durable JSON depth bound');
    if (value === null || typeof value === 'boolean')
        return value;
    if (typeof value === 'string')
        return readBoundedString(value, path, 16_384);
    ...
    if (Array.isArray(value)) {
        if (value.length > 256)
            fail(path, 'exceeds the durable JSON array bound');
    ...
    if (keys.length > 128 || keys.some(key => typeof key !== 'string'))
        fail(path, 'has invalid or excessive JSON fields');
    const output = {};
...
function uniqueStrings(value, path, maximum = 64, sorted = false) {
    if (!Array.isArray(value) || value.length > maximum)
        fail(path, 'must be a bounded string array');
    ...
    if (sorted && output.some((item, index) => item !== [...output].sort()[index]))
        fail(path, 'must be sorted');
    return Object.freeze(output);
}
function boundedStrings(value, path, maximum = 64) {
    if (!Array.isArray(value) || value.length > maximum)
        fail(path, 'must be a bounded string array');
    return Object.freeze(value.map((item, index) => readBoundedString(item, `${path}[${String(index)}]`, 512)));
}
function candidateRef(value, path, kind) {
...
function absolutePath(value, path) {
    ...
    if (!isAbsolute(output) || resolve(output) !== output)
        fail(path, 'must be a canonical absolute path');
    return output;
}
function mcpHttpCoordinates(originValue, endpointValue, path) {
    ...
    if (parsedOrigin.protocol !== 'https:' || parsedEndpoint.protocol !== 'https:'
        ...
        fail(path, 'must contain canonical credential-free HTTPS coordinates');
...
function kindFromTarget(targetKey, path) {
    const kind = targetKey.slice(0, targetKey.indexOf(':'));
    return readLiteral(kind, KINDS, path);
}
function targetIdentity(kind, profileId, scopeKey, extensionId) {
    return `${kind}:${profileId}:${scopeKey}:${extensionId}`;
}
function skillConfiguration(value, path, scopeKey) {
    ...
    if (scopeKey === 'user' && projectRoot !== null)
        fail(path, 'must not bind projectRoot for user scope');
    if (scopeKey === 'project' && projectRoot === null)
        fail(path, 'must bind projectRoot for project scope');
...
function mcpConfiguration(value, path) {
    ...
    if (timeout < 100 || timeout > 300_000)
        fail(`${path}.toolCallTimeoutMs`, 'is outside 100..300000');
    ...
    if (initialDelayMs < 50 || initialDelayMs > 60_000 || maxDelayMs < 50 || maxDelayMs > 300_000
        || initialDelayMs > maxDelayMs || maxAttempts > 100)
        fail(`${path}.reconnect`, 'is outside the admitted bounds');
    const common = {
        connectionId,
...
function runtimeEvidence(value, path) {
    ...
    return Object.freeze({
        ...
        fiberPhase: readLiteral(record.fiberPhase, ['absent', 'active'], `${path}.fiberPhase`),
        moduleName: readBoundedString(record.moduleName, `${path}.moduleName`),
    });
}
function versionKindState(value, path, kind, extensionId, materialPath, root) {
    if (kind === 'skill') {
        ...
        if (skillName !== extensionId)
            fail(`${path}.skillName`, 'does not bind the managed extension');
        return Object.freeze({
    ...
    if (kind === 'mcp') {
        ...
        if (transport === 'stdio') {
            ...
            if (executablePath !== materialPath)
                fail(`${path}.executablePath`, 'does not bind materialPath');
            return Object.freeze({ ...common, executablePath, transport: 'stdio' });
        }
        if (materialPath !== resolve(root))
            fail(`${path}.materialPath`, 'does not bind the Center management root');
    ...
    const loaderPhase = readLiteral(record.loaderPhase, ['absent', 'active', 'pending-restart'], `${path}.loaderPhase`);
    ...
    if (loaderPhase === 'pending-restart') {
        if (consumerObserved || externalRestartObserved || hasEvidence)
            fail(path, 'has contradictory pending-restart evidence');
    }
    ...
    if (evidence !== undefined && evidence.fiberPhase !== loaderPhase) {
...
function managedVersion(value, path, root, identity) {
    const record = readStrictRecord(value, ['artifactIntegrity', 'artifactRevision', 'candidateRef', 'configuration', 'enabled', 'kindState', 'materialPath', 'ownerRevision'], path);
    ...
    if (identity.kind === 'skill') {
        const expected = join(resolve(root), 'material', 'skills', storageKey(identity.targetKey), storageKey(artifactIntegrity), 'SKILL.md');
        if (materialPath !== expected)
            fail(`${path}.materialPath`, 'does not bind the target and artifact integrity');
    }
    ...
    if (!ownerRevision.startsWith(ownerPrefix))
        fail(`${path}.ownerRevision`, 'does not bind the extension owner');
...
function nullableManagedVersion(value, path, root, identity) {
    return value === null ? null : managedVersion(value, path, root, identity);
}
function pendingMutation(value, path, kind, profileId, operationId) {
    if (value === null)
        return null;
    if (kind !== 'plugin')
        fail(path, 'is only valid for Plugin Profile mutations');
    const record = readStrictRecord(value, ['generation', 'operationId', 'operationKind', 'packageName', 'profileId', 'revision', 'treeDigest'], path);
    const heldOperation = readBoundedString(record.operationId, `${path}.operationId`);
    if (heldOperation !== operationId)
        fail(`${path}.operationId`, 'does not bind lastOperationId');
    const heldProfile = readBoundedString(record.profileId, `${path}.profileId`);
    if (heldProfile !== profileId)
        fail(`${path}.profileId`, 'does not bind the managed target');
    return Object.freeze({
        generation: readBoundedString(record.generation, `${path}.generation`, 512),
        operationId: heldOperation,
        operationKind: readLiteral(record.operationKind, [...OPERATIONS, 'rollback'], `${path}.operationKind`),
        packageName: readBoundedString(record.packageName, `${path}.packageName`, 214),
...
export function durableManagedStateDigest(record) {
    if (record === null)
...
export function decodeCenterManifest(value) {
    ...
    if (!UUID.test(centerId))
        fail(`${path}.centerId`, 'must be a UUID');
    return Object.freeze({
        schemaVersion: 1,
        centerId,
...
export function decodeManagedTarget(value, root, expectedTargetKey) {
    ...
    if (targetKey !== targetIdentity(kind, profileId, scopeKey, extensionId))
        fail(`${path}.targetKey`, 'does not bind kind/profile/scope/extension');
    if (expectedTargetKey !== undefined && targetKey !== expectedTargetKey)
        fail(`${path}.targetKey`, 'does not bind its storage identity');
...
function intentCore(value, path) {
    const record = readStrictRecord(value, [
    ...
    if (url.protocol !== 'https:' || url.username !== '' || url.password !== '')
        fail(`${path}.artifactUrl`, 'must be credential-free HTTPS');
    return immutableJsonClone({
        kind,
...
function acquisitionIntent(value, path) {
    const record = readStrictRecord(value, ['continuationId', 'core', 'coreDigest', 'createdAtMs', 'expiresAtMs', 'idempotencyKey', 'intentId', 'origin', 'schemaVersion'], path);
    schema(record, path);
    const origin = readLiteral(record.origin, ['store', 'task'], `${path}.origin`);
    const continuationId = nullableString(record.continuationId, `${path}.continuationId`);
    if ((origin === 'store') !== (continuationId === null))
        fail(`${path}.continuationId`, 'does not bind intent origin');
    ...
    if (createdAtMs >= expiresAtMs)
        fail(path, 'has an invalid validity interval');
    const core = intentCore(record.core, `${path}.core`);
    const coreDigest = readSha256Digest(record.coreDigest, `${path}.coreDigest`);
    if (coreDigest !== canonicalSha256(core))
        fail(`${path}.coreDigest`, 'does not match core');
    return immutableJsonClone({
        schemaVersion: 1,
...
function lifecyclePayload(value, path, intent) {
    const record = readStrictRecord(value, [
    ...
    if (continuationId !== intent.continuationId)
        fail(`${path}.continuationId`, 'does not bind intent continuationId');
    const taskFields = [continuationId, resolutionId, verificationPayloadDigest, taskSessionId, taskOriginalMessageId];
    if (intent.origin === 'store' && taskFields.some(item => item !== null))
        fail(path, 'carries task-only fields for a Store intent');
    if (intent.origin === 'task' && taskFields.some(item => item === null))
        fail(path, 'has an incomplete task binding');
    return immutableJsonClone({
...
export function decodeStoredIntent(value, expectedIntentId) {
    ...
    if (expectedIntentId !== undefined && intent.intentId !== expectedIntentId)
        fail(`${path}.intent.intentId`, 'does not bind its storage identity');
    return immutableJsonClone({
        schemaVersion: 1,
        intent,
...
function resumeAgentOptions(value, path) {
...
function resolutionValue(value, path) {
    const record = readStrictRecord(value, [
    ...
    if (!Array.isArray(record.candidates) || record.candidates.length > 16)
        fail(`${path}.candidates`, 'must be a bounded array');
    const candidates = record.candidates.map((candidate, index) => {
        ...
        return Object.freeze({
            ...
            operationKind: readLiteral(item.operationKind, ['configure', 'enable', 'install', 'restore', 'update'], `${itemPath}.operationKind`),
            targetKey: nullableString(item.targetKey, `${itemPath}.targetKey`, 1024),
        });
    });
    if (new Set(candidates.map(item => item.candidateRef)).size !== candidates.length)
        fail(`${path}.candidates`, 'must not repeat candidateRef');
    const continuationId = nullableString(record.continuationId, `${path}.continuationId`);
    const verificationPayloadDigest = record.verificationPayloadDigest === null
        ? null
        : readSha256Digest(record.verificationPayloadDigest, `${path}.verificationPayloadDigest`);
    ...
    return Object.freeze({
        ...
        taskAttemptId: (() => {
            ...
            if (!/^task-attempt:[0-9a-f-]{36}$/u.test(value))
                fail(`${path}.taskAttemptId`, 'is invalid');
            return value;
        })(),
        verificationPayloadDigest,
    });
...
export function decodeStoredResolution(value, expectedResolutionId) {
    ...
    if (!/^resolution:[0-9a-f-]{36}$/.test(resolutionId))
        fail(`${path}.resolutionId`, 'is invalid');
    if (expectedResolutionId !== undefined && resolutionId !== expectedResolutionId)
        fail(`${path}.resolutionId`, 'does not bind its storage identity');
    const createdAtMs = readNonNegativeInteger(record.createdAtMs, `${path}.createdAtMs`);
    const expiresAtMs = readNonNegativeInteger(record.expiresAtMs, `${path}.expiresAtMs`);
    if (createdAtMs >= expiresAtMs)
        fail(path, 'has an invalid validity interval');
    ...
    const detail = resolutionValue(record.value, `${path}.value`);
    const detailCandidates = detail.candidates;
...
export function decodeOperationIndex(value, expectedOperationId) {
    ...
    if (expectedOperationId !== undefined && operationId !== expectedOperationId)
        fail(`${path}.operationId`, 'does not bind its storage identity');
    const extensionKind = readLiteral(record.extensionKind, KINDS, `${path}.extensionKind`);
    const targetKey = readBoundedString(record.targetKey, `${path}.targetKey`, 1024);
    if (kindFromTarget(targetKey, `${path}.targetKey`) !== extensionKind)
        fail(`${path}.targetKey`, 'does not bind extensionKind');
    return Object.freeze({
        schemaVersion: 1,
        operationId,
        planHash: readSha256Digest(record.planHash, `${path}.planHash`),
...
function nullableRevision(value, path) {
    return value === null ? null : readNonNegativeInteger(value, path);
}
function skillRecoveryPoint(value, path, root, operationId, targetKey) {
    ...
    if ((parsed === null) !== (destination === null && stagingPath === null))
        fail(path, 'has incomplete Skill staging fields');
    if (destination !== null) {
...
function mcpRecoveryPoint(value, path) {
    const record = readStrictRecord(value, [
    ...
    const rawRuntime = record.runtime !== null && typeof record.runtime === 'object' && !Array.isArray(record.runtime)
        ? record.runtime
        : undefined;
    const transport = readLiteral(rawRuntime?.transport, ['stdio', 'streamable-http'], `${path}.runtime.transport`);
...
function pluginConfigurationPatch(value, path) {
    const record = readStrictRecord(value, [
    ...
    if (nextDigest !== observedDigest)
        fail(`${path}.nextDigest`, 'does not bind nextUtf8');
    return Object.freeze({
...
function pluginRecoveryPoint(value, path, root, targetKey) {
    ...
    if (profile !== targetParts[1])
        fail(`${path}.snapshot.profile`, 'does not bind targetKey');
    ...
    return Object.freeze({
        ...
        snapshot: Object.freeze({
            ...
            bootStatus: readLiteral(snapshot.bootStatus, ['live', 'pending-restart', 'verified'], `${path}.snapshot.bootStatus`),
...
function recoveryPoint(value, path, root, operationId, targetKey, kind) {
    if (value === null || typeof value !== 'object' || Array.isArray(value))
        fail(path, 'must be an owner recovery object');
    const heldKind = value.kind;
    if (heldKind !== kind)
        fail(`${path}.kind`, 'does not bind targetKey');
    if (kind === 'skill')
        return skillRecoveryPoint(value, path, root, operationId, targetKey);
    if (kind === 'mcp')
        return mcpRecoveryPoint(value, path);
...
export function decodeProviderSnapshot(value, root, expectedOperationId) {
    ...
    if (beforeDigest !== durableManagedStateDigest(before))
        fail(`${path}.beforeDigest`, 'does not match before');
    return immutableJsonClone({
        schemaVersion: 1,
        operationId,
        targetKey,
...
export function decodeProfileBootAck(value, expectedOperationId) {
    ...
    if (expectedOperationId !== undefined && operationId !== expectedOperationId)
        fail(`${path}.operationId`, 'does not bind its storage identity');
    if (record.consumerObserved !== true)
        fail(`${path}.consumerObserved`, 'must be true');
    return Object.freeze({
        schemaVersion: 1,
        operationId,
        profileId: readBoundedString(record.profileId, `${path}.profileId`, 256),
        generation: readBoundedString(record.generation, `${path}.generation`, 512),
        phase: readLiteral(record.phase, ['candidate', 'rollback'], `${path}.phase`),
        revision: positiveInteger(record.revision, `${path}.revision`),
        treeDigest: readBoundedString(record.treeDigest, `${path}.treeDigest`, 512),
...
export function decodeTaskReceipt(value, expectedContinuationId) {
    ...
    if (expectedContinuationId !== undefined && continuationId !== expectedContinuationId)
        fail(`${path}.continuationId`, 'does not bind its storage identity');
    return Object.freeze({
        schemaVersion: 1,
        continuationId,
...
export function decodeContinuationActivation(value, expectedReservationId) {
    ...
    if (expectedReservationId !== undefined && reservationId !== expectedReservationId)
        fail(`${path}.reservationId`, 'does not bind its storage identity');
    return Object.freeze({
        schemaVersion: 1,
        reservationId,
...
export function decodeContinuationActivationIntent(value, expectedReservationId) {
    ...
    if (mutationId !== reservationId)
        fail(`${path}.mutationId`, 'does not bind reservationId');
    if (record.callerId !== 'extension-center')
        fail(`${path}.callerId`, 'must be extension-center');
    const createdAtMs = readNonNegativeInteger(record.createdAtMs, `${path}.createdAtMs`);
    const expiresAtMs = readNonNegativeInteger(record.expiresAtMs, `${path}.expiresAtMs`);
    if (createdAtMs >= expiresAtMs)
        fail(path, 'has an invalid validity interval');
...
export function assertStateFileIdentity(fileName, identity, path) {
    if (fileName !== `${storageKey(identity)}.json`)
        fail(path, 'filename does not bind the record identity');
}
// Keep the closed union imports checked against the durable literal tables.
const _operationKindCheck = OPERATIONS;
const _desiredStateCheck = DESIRED_STATES;
const _phaseCheck = PHASES;
