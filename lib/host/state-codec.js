...
import { decodeStoredProfileMetadataCacheBinding } from "../recovery/profile-metadata-cache.js";
...
function versionKindState(value, path, kind, extensionId, materialPath, root) {
    ...
    if (loaderPhase === 'pending-restart') {
        if (consumerObserved || restartObserved || hasEvidence)
...
function pendingMutation(value, path, kind, profileId, operationId) {
    if (value === null)
        return null;
    if (kind !== 'plugin')
        fail(path, 'is only valid for managed Plugin mutations');
...
function pluginRecoveryPoint(value, path, root, targetKey) {
    ...
    if (profileId !== targetParts[1])
        fail(`${path}.snapshot.profileId`, 'does not bind targetKey');
    ...
    return Object.freeze({
        artifactPath,
        kind: 'plugin',
        metadataCache: immutableJsonClone(metadataCache),
