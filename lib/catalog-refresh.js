...
import { withCatalogCacheWriter } from "./catalog-cache-reservation.js";
import { ensurePrivateDirectory, openRegularNoFollow, safeChild, } from "./host/files.js";
...
function sameSignedDocument(left, right) {
...
function verifyHistoricalCache(cache) {
    const first = cache.chain[0];
    if (first === undefined)
        throw new Error('catalog last-good cache chain is empty');
    const historicalRoot = Object.freeze({
        ...BOOTSTRAP_CATALOG_ROOT,
        minimumRevision: first.envelope.revision,
    });
    let current = verifyCatalog(historicalRoot, first.envelope, first.signatures, Date.parse(first.envelope.issuedAt) + 1);
    for (const document of cache.chain.slice(1)) {
        ...
        current = verifyCatalogAdvance(historicalRoot, current, document, Date.parse(document.envelope.issuedAt) + 1);
    }
    return current;
}
...
function reconcileCache(cache, now) {
    const historical = verifyHistoricalCache(cache);
    const packaged = bootstrapDocument();
    const anchorIndexes = cache.chain.flatMap((document, index) => (sameSignedDocument(document, packaged) ? [index] : []));
    if (anchorIndexes.length > 1)
        throw new Error('catalog cache repeats the packaged bootstrap');
...
function selectMonotonicCache(durable, candidate) {
    ...
    if (candidate.chain.length > durable.chain.length)
        return candidate;
    if (durable.chain.length > candidate.chain.length)
        return durable;
    return candidate.acceptedAtMs >= durable.acceptedAtMs ? candidate : durable;
}
async function initializeCatalogCache(path, now) {
    return await withCatalogCacheWriter(dirname(path), async () => {
        const durable = await readCache(path);
        const candidate = durable ?? Object.freeze({
            schemaVersion: CACHE_SCHEMA_VERSION,
            catalogId: BOOTSTRAP_CATALOG_ROOT.catalogId,
        ...
        if (durable === undefined || reconciled.changed)
            await replaceCache(path, reconciled.cache);
        return Object.freeze({
...
async function commitCatalogCache(path, candidate, now) {
    return await withCatalogCacheWriter(dirname(path), async () => {
...
export function canonicalCatalogUrl(trustedUrl) {
    let url;
    try {
...
export class CatalogSnapshotManager {
    ...
    async initialize() {
        ...
        if (this.config.trustedUrl === null)
    ...
    async refreshOnce() {
        ...
        try {
            ...
            let failure = error;
            try {
                const committed = await commitCatalogCache(join(this.root, 'catalog', 'last-good.json'), this.cache, attemptedAtMs);
                this.cache = committed.cache;
            }
            catch (synchronizationError) {
                failure = new AggregateError([error, synchronizationError], `catalog refresh failed and durable cache synchronization failed: ${synchronizationError instanceof Error ? synchronizationError.message : String(synchronizationError)}`);
            }
