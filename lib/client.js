	...
	factory: (require) => {
		...
		*/
		...
		function validateEntry(value, index) {
			...
			if (!["0.1.1-rc.2", "0.1.2-alpha.3"].includes(compatibility.dsh)) throw new Error(`extension-center: invalid ${subject}.compatibility.dsh`);
		...
		function receiptPlanEvidence(value, subject) {
			...
			if (officialDsh.schemaVersion !== 2 || officialDsh.packageName !== "@deepseek-ai/dsh" || officialDsh.packageVersion !== "0.1.2-alpha.3") fail(`${subject}.recoveryExecutable.officialDsh identity`);
			for (const field of [
				"entrypointPath",
				"hostHome",
				"packageRoot",
