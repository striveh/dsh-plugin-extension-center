window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function parseCatalogListResponse(value) {
			...
			if (acquisition && !(profileTransaction && dynamicMcpConnection && durableContinuation && skillRegistry && toolRegistry && loaderObservation)) throw new Error("extension-center: Host acquisition claim requires all owners and a ready writable runtime");
		...
		function reviewEvidence(value, subject) {
			...
			if (input.rollbackPoint !== null) {
				...
				literal(point.kind, /* @__PURE__ */ new Set([
					"absent-state",
					"managed-version",
					"profile-generation"
				]), `${subject}.rollbackPoint.kind`);
			...
			if (kind === "plugin") {
				...
				array(input.bundles, `${subject}.bundles`, 64).forEach((item, index) => {
					const row = exactRecord(item, `${subject}.bundles[${String(index)}]`, [
						"id",
						"action",
						"patchDigest",
						"patchBody"
					...
					literal(row.action, /* @__PURE__ */ new Set([
						"add",
						"retain",
						"remove",
						"restore"
					]), `${subject}.bundles[${String(index)}].action`);
		...
		function capabilities(value, subject) {
			...
			if (acquisition && !(profileTransaction && dynamicMcpConnection && durableContinuation && skillRegistry && toolRegistry && loaderObservation) || reason !== (acquisition ? null : "host-capability")) fail(subject);
			return input;
		}
		...
		function inventoryRow(value, snapshot, index) {
			...
			literal(input.ownership, /* @__PURE__ */ new Set([
				"center",
				"external",
				"system",
				"parent-plugin"
			...
			literal(input.rollback, /* @__PURE__ */ new Set([
				"available",
				"running",
				"used",
				"unavailable",
		...
		function receiptPlanEvidence(value, subject) {
			...
			if (reviewEvidence(input.reviewEvidence, `${subject}.reviewEvidence`).kind !== input.extensionKind) fail(`${subject}.reviewEvidence binding`);
			bool(input.restartRequired, `${subject}.restartRequired`);
			const fences = exactRecord(input.fences, `${subject}.fences`, [
				"catalogRevision",
				"inventoryRevision",
			...
			for (const field of [
				...
				const path = string(recovery[field], `${subject}.recoveryExecutable.${field}`, 4096);
				if (!path.startsWith("/") && !/^[A-Za-z]:[\\/]/u.test(path) && !path.startsWith("\\\\")) fail(`${subject}.recoveryExecutable.${field}`);
			}
			for (const field of ["executableSha256", "hostCliSha256"]) digest(recovery[field], `${subject}.recoveryExecutable.${field}`);
		...
		function parseOperationListResponse(value) {
			...
			array(input.operations, "operation list response.operations", MAX_OPERATIONS).forEach((value, index) => {
				...
				if (phase === "recovery-required") if (row.recoveryCommand === null) {
		...
		function PlanReview({ preview, candidate, management, configuration, initialState, t, onClose, onCommitted }) {
			...
			const restart = candidateBound ? `${candidate.restart.required ? t("restart.required") : t("restart.notRequired")} · ${localize$1(candidate.restart.detail, language)}` : t("field.notDeclared");
					...
					rows.length === 0 ? (0, react_jsx_runtime.jsxs)("div", {
						...
						children: rows.map((row) => (0, react_jsx_runtime.jsxs)("article", {
							...
							children: [
								...
								}) : row.candidateRef === "plugin:dsh-capability-resolver@0.1.0" ? (0, react_jsx_runtime.jsx)(ResolverConfigDraft, {
							...
							return (0, react_jsx_runtime.jsx)("li", {
								...
								children: (0, react_jsx_runtime.jsxs)("article", {
									...
									children: [
										...
										operation.phase !== "recovery-required" ? null : (0, react_jsx_runtime.jsxs)("div", {
											className: ExtensionCenter_module_css_default.recoveryCallout,
											role: "alert",
											children: [
												(0, react_jsx_runtime.jsx)("strong", { children: t("recovery.required") }),
												(0, react_jsx_runtime.jsx)("p", { children: t("recovery.required.body") }),
		...
		function CandidateDetail({ entry, language, writable, unavailableReason, selectedScope, t, onScope, onLifecycle, onClose }) {
			return (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					entry.candidateRef === "plugin:dsh-capability-resolver@0.1.0" ? (0, react_jsx_runtime.jsx)(ResolverConfigDisclosure, { t }) : null,
					(0, react_jsx_runtime.jsxs)("section", {
						className: ExtensionCenter_module_css_default.disclosure,
		...
		const en = {
			...
			preview: "Read-only catalog preview",
			host: "DSH 0.1.1-rc.2",
			...
			"lifecycle.body": "Writable P0 requires Profile transactions, dynamic MCP connections, durable continuation, Skill and tool registries, and loader observation.",
			...
			"capability.profileTransaction": "Profile transactions",
			...
			"capability.loaderObservation": "Loader observation",
			...
			"taskAttempt.retryContinuation.consumed": "Continuation message persisted",
			...
			"review.plugin": "Plugin manifest, lock, Bundle, scripts, and settings",
		...
		const zh = {
			...
			"lifecycle.body": "可写 P0 同时需要 Profile transaction、动态 MCP connection、durable continuation、Skill 与 Tool registry，以及 Loader 观测。",
			"lifecycle.code": "unavailable(host-capability)",
			"lifecycle.available": "可用",
			"lifecycle.unavailable": "不可用",
			"capability.heading": "可写 Host 预检",
			"capability.profileTransaction": "Profile transaction",
			"capability.dynamicMcpConnection": "动态 MCP connection",
			"capability.durableContinuation": "Durable continuation",
			"capability.skillRegistry": "Skill registry",
			"capability.toolRegistry": "Tool registry",
			"capability.loaderObservation": "Loader 观测",
			"capability.ready": "就绪",
			...
			"taskAttempt.retryContinuation.consumed": "续跑消息已持久化",
			...
			"review.plugin": "Plugin manifest、lock、Bundle、脚本与设置",
