window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function parseCatalogListResponse(value) {
			...
			const ownersAvailable = profileTransaction && dynamicMcpConnection && durableContinuation && skillRegistry && toolRegistry && loaderObservation;
			if (acquisition !== ownersAvailable) throw new Error("extension-center: Host acquisition claim does not match required owner capabilities");
		...
		function receiptPlanEvidence(value, subject) {
			...
			for (const field of ["executablePath", "hostCliPath"]) {
