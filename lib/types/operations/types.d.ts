...
import type { DesiredState, ExternalRuntimeAction, ManagedExtensionKind, ManagedObject, OperationKind, PlanReviewEvidence, PlannedReviewCheck, PlanRevisionFences, RecoveryExecutableBinding, RuntimeBinding } from '../plans/types.ts';
/** Current operation journal and receipt schema. */
export declare const OPERATION_RECORD_SCHEMA_VERSION: 1;
/** Pure operation phases; only the published transition table can change them. */
export type OperationPhase = 'authorized' | 'staging' | 'applying' | 'verifying' | 'rolling-back' | 'committed' | 'rolled-back' | 'failed' | 'recovery-required';
...
export type OperationJournalEntry = OperationOpenedEntry | OperationPhaseTransitionEntry | OperationMutationEntry | OperationVerificationEntry | OperationReceiptIssuedEntry;
