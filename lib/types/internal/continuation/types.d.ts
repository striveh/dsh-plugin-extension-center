 ...
 */
export type TaskContinuationState = 'pending' | 'ready' | 'consumed' | 'dispatching' | 'dispatched' | 'claimed' | 'delivery-unknown' | 'canceled' | 'superseded' | 'expired' | 'invalid';
/** Secret-free reason why one at-most-once dispatch could not be confirmed. */
export type TaskContinuationDeliveryUnknownReason = 'followup-error' | 'session-flush-error' | 'owner-lease-expired' | 'legacy-consumed';
/** Fixed, secret-free diagnoses for an invalid terminal claim. */
export declare const TASK_CONTINUATION_INVALID_REASONS: readonly ["agent-settled-before-continuation-message", "dispatch-evidence-before-owned-call", "dispatch-evidence-content-mismatch", "dispatch-lease-missing", "dispatched-inbox-evidence-missing", "duplicate-dispatch-evidence-before-claim", "newer-direct-user-message-after-dispatch", "original-user-message-missing", "verifier-echo-mismatch"];
...
export interface ContinuationMessage {
    ...
    readonly source: Readonly<{
        kind: 'plugin';
        plugin: 'dsh-plugin-extension-center';
    }>;
}
