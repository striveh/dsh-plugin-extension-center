...
import { buildCapabilityResolverPatch, hasPluginConfigurationAdapter, } from "./plugin-config-adapter.js";
...
export class LoaderPluginRuntimeProbe {
    ...
    async observe(packageName, operation) {
        ...
        const active = matches.find(entry => entry.options.id === packageName
            ...
            && entry.fiber?.state === 2);
        if (active === undefined)
            throw new Error('Plugin target is not ACTIVE in the settled Loader tree');
...
function snapshot(value) {
    ...
    if (typeof item.profile !== 'string'
        ...
        || !['live', 'pending-restart', 'verified'].includes(item.bootStatus))
...
function pending(record) {
    ...
    if (typeof value !== 'object' || Array.isArray(value))
        throw new Error('pending Profile mutation is invalid');
    const item = value;
...
function mutationId(operationId, phase) {
...
export function pluginConfigurationMutationDigest(patch, ownerRevision) {
...
export class PluginLifecycleProvider {
    ...
    runtime;
    kind = 'plugin';
    constructor(store, owner, runtime) {
        this.store = store;
        this.owner = owner;
    ...
    async prepare(request) {
        ...
        if (profileFence(held) !== request.plan.fences.profileRevision)
            throw new Error('Profile revision/tree fence is stale');
        let configurationPatch = null;
        ...
        return Object.freeze({
            ...
            stagingPath: null,
            prepared: Object.freeze({ snapshot: held, configurationPatch }),
        });
    }
    ...
    async apply(prepared) {
        ...
        if (generation === null)
            throw new Error('Profile mutation did not publish a generation');
        let supplied = null;
        if (request.plan.operationKind === 'install' || request.plan.operationKind === 'update') {
    ...
    async verify(applied) {
        if (!applied.restartRequired)
            return Object.freeze({ digest: canonicalSha256({ structural: applied.afterDigest }) });
        const record = await this.store.getManaged(applied.prepared.request.plan.targetKey);
        if (record === undefined)
            throw new Error('managed Plugin disappeared before boot verification');
        ...
        if (evidenceVersion === null)
            throw new Error('Plugin boot evidence has no exact managed version');
    ...
    async acknowledgeBoot(input) {
        ...
        if (record === undefined)
            throw new Error('Plugin operation target does not exist');
    ...
    async rollback(applied) {
        ...
        if (restored.activeGeneration === null)
            throw new Error('Profile rollback did not publish a generation');
    ...
    async finalizeRollback(applied) {
        if (!applied.rollbackRestart || applied.prepared.before !== null)
            return;
        const record = await this.store.getManaged(applied.prepared.request.plan.targetKey);
     ...
     */
    async reconcileBreakGlassRestore(request, expectedBeforeDigest) {
        const review = request.plan.reviewEvidence;
        if (review.kind !== 'plugin') {
        ...
        if (held.profile !== request.plan.profileId
            ...
            || held.treeDigest !== recoveryPin.digest)
            return null;
        ...
        const temporaryRemoved = immutableJsonClone({
            candidateRef: request.plan.candidateRef,
            artifactRevision: request.plan.artifactRevision,
            artifactIntegrity: request.plan.artifactIntegrity,
            materialPath: held.effectivePath,
    ...
    async recover(request) {
        ...
        if (record?.lastOperationId !== request.authorization.operationId) {
            ...
            const prepared = {
                ...
                stagingPath: null,
                prepared: Object.freeze({ snapshot: point.snapshot, configurationPatch: point.configurationPatch }),
            };
        ...
        if (acknowledged !== undefined && (acknowledged.profileId !== request.plan.profileId
            || evidence === null
            || pluginState(evidence).profileGeneration !== acknowledged.generation))
    ...
    applied(prepared, after, generation, rollbackRestart = false) {
        return Object.freeze({
            prepared,
