...
import { ManagedPluginOwner, OfficialProfileAmbiguityError, } from "../internal/plugin/index.js";
import { decodeProfileMetadataCacheBinding, prepareProfileMetadataCache, } from "../recovery/profile-metadata-cache.js";
import { inspectNpmArchive } from "./npm-archive.js";
import { validateCapabilityResolverConfiguration } from "./plugin-config-adapter.js";
...
export class LoaderPluginRuntimeProbe {
    loader;
    constructor(loader) {
        this.loader = loader;
    }
    async observe(packageName, operation) {
        const matches = [...this.loader.entries()].filter(entry => !entry.options.group && entry.options.name === packageName);
        if (operation === 'uninstall') {
            if (matches.length !== 0)
                throw new Error('uninstalled Plugin remains in the settled Loader tree');
...
function pending(record) {
    ...
    if (typeof value !== 'object' || Array.isArray(value))
        throw new Error('pending managed Plugin mutation is invalid');
    const item = value;
...
function rollbackOperation(record, operationId) {
...
function plannedPackageName(request) {
...
function packageName(record, request) {
...
function recoveryPoint(value) {
    ...
    if (Object.keys(held).sort().join('\0') !== fields.join('\0')
        ...
        || !['live', 'pending-restart', 'verified'].includes(held.bootStatus)) {
        throw new Error('Plugin owner snapshot fields are invalid');
    }
    const digest = held.digest;
    const ownerRevision = `managed-plugin:${String(held.revision)}:${digest}`;
    if (held.ownerRevision !== ownerRevision)
        throw new Error('Plugin owner snapshot revision is invalid');
    return Object.freeze({
...
async function verifyPackedArtifact(path, integrity) {
    ...
    if (observed !== integrity)
        throw new Error('Plugin recovery artifact does not match the immutable plan integrity');
}
/** Exact Center-owned configuration digest; no official Profile patch is read or changed. */
export function pluginConfigurationMutationDigest(configuration, ownerRevision) {
...
export class PluginLifecycleProvider {
    store;
    kind = 'plugin';
    owner;
    officialDsh;
    ...
    async referencesDurableOperation(operationId, targetKey, profileId) {
    ...
    async prepare(request) {
        ...
        if (request.plan.operationKind === 'install' || request.plan.operationKind === 'update') {
            if (request.artifactPath === null)
                throw new Error('Plugin install or update requires an acquired archive');
            ...
            const review = request.plan.reviewEvidence;
            if (review.kind !== 'plugin')
                throw new Error('Plugin plan has no Plugin review evidence');
        ...
        return Object.freeze({
            ...
            prepared: Object.freeze({ snapshot: held, materialPath, activation, metadataCache }),
        });
    }
    ...
    async apply(prepared) {
        ...
        let supplied = null;
        if (request.plan.operationKind === 'install' || request.plan.operationKind === 'update') {
            if (detail.materialPath === null || detail.activation === null)
                throw new Error('Plugin prepared material is absent');
        ...
        const committed = await this.owner.commit(prepared.before, desired, plannedPackageName(request), detail.metadataCache);
        try {
            await this.store.putManaged(committed.sidecar.managed, prepared.before?.revision ?? 0);
        }
        catch (error) {
            throw new OfficialProfileAmbiguityError('managed Plugin owner state advanced before the Center projection was persisted', { cause: error });
        }
    ...
    async acknowledgeBoot(input) {
        ...
        if (pending(record) !== null)
            throw new Error('managed Plugin has not been rehydrated by a new Host process');
    ...
    async rollback(applied) {
        ...
        if (before === null) {
            const receipt = await this.owner.rollbackToAbsent(current, packageName(current, applied.prepared.request), applied.prepared.request.authorization.operationId, applied.prepared.prepared?.metadataCache ?? null);
        ...
        const committed = await this.owner.commit(current, restored, packageName(before ?? current, applied.prepared.request), applied.prepared.prepared?.metadataCache ?? null);
    ...
    async verifyRollbackFinalization(applied) {
    ...
    async finalizeDurableRollback(input) {
        await this.initialize();
    ...
    async finalizeRollback(applied) {
    ...
    async reconcileBreakGlassRestore(request, expectedBeforeDigest, journalHeadDigest) {
        ...
        if (durable.before === null) {
            ...
            if (settledRestored)
                await this.owner.verify(current);
        }
    ...
    async recover(request) {
        ...
        if (point.snapshot.profileId !== request.plan.profileId)
            throw new Error('Plugin recovery owner snapshot does not bind the plan');
        ...
        const prepared = {
            request,
            before: durable.before,
            beforeDigest: durable.beforeDigest,
            stagingPath: null,
        ...
        return this.applied(prepared, record, !rolledBack && request.plan.restartRequired, rolledBack && request.plan.restartRequired);
    }
    cleanup() {
    ...
    applied(prepared, after, restartRequired, rollbackRestartRequired) {
        return Object.freeze({
            prepared,
