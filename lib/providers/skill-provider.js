...
import { openRegularNoFollow, safeChild, storageKey } from "../host/index.js";
...
export async function inspectSkillArtifact(path, expectedIntegrity) {
    ...
    try {
        ...
        if (info.size > 1024 * 1024)
            throw new Error('SKILL.md exceeds the P0 size bound');
...
function object(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        throw new Error('skill configuration must be an object');
    return value;
}
...
export async function preflightSkillConfiguration(value, scopeKey) {
    ...
    if (!['project', 'user'].includes(scopeKey))
        throw new Error('Skill scope is not admitted');
    ...
    return immutableJsonClone(config);
}
function parseSkill(text) {
    ...
    if (Buffer.byteLength(frontmatter, 'utf8') > 32 * 1024)
        throw new Error('SKILL.md frontmatter exceeds the P0 bound');
    const document = parseDocument(frontmatter, { schema: 'core', strict: true, uniqueKeys: true });
    if (document.errors.length > 0 || document.warnings.length > 0)
        throw new Error('SKILL.md frontmatter is not strict YAML');
    let fields;
    try {
    ...
    if (typeof name !== 'string' || typeof description !== 'string')
        throw new Error('SKILL.md name and description must be strings');
...
function recoveryPoint(value) {
    ...
    if (input.parsed !== null) {
        const fields = object(input.parsed);
...
function state(value) {
    ...
    if (typeof raw !== 'object' || raw === null || Array.isArray(raw))
        throw new Error('managed Skill state is invalid');
    const record = raw;
...
async function lookupCwd(options) {
...
async function visibleInLookup(record, options) {
    const current = record.current;
    if (current === null)
        return false;
    const config = configuration(current.configuration);
    if (record.scopeKey === 'user')
        return config.projectRoot === null;
    if (record.scopeKey !== 'project' || config.projectRoot === null)
        return false;
...
function isMaterialDirectory(value, materialPath) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return false;
    const record = value;
...
export class SkillLifecycleProvider {
    root;
    store;
    skills;
    kind = 'skill';
    invalidate = () => { };
    constructor(root, store, skills) {
        this.root = root;
        this.store = store;
        this.skills = skills;
    }
            ...
            return {
                ...
                list: async (options) => {
                    ...
                    for (const record of values) {
                        ...
                        if (current === null || !current.enabled || !await visibleInLookup(record, options))
                            continue;
                        try {
                ...
                get: async (candidate, options) => {
                    ...
                    const current = record?.kind === 'skill' ? record.current : null;
                    if (current === null || current === undefined || !current.enabled || record === undefined || !await visibleInLookup(record, options))
                        return undefined;
                    const metadata = state(current);
    ...
    async prepare(request) {
        ...
        if (review.kind !== 'skill')
            throw new Error('Skill plan has no Skill review evidence');
        ...
        if (request.plan.operationKind === 'install' || request.plan.operationKind === 'update') {
            if (request.artifactPath === null)
                throw new Error('Skill install or update requires an acquired artifact');
            const inspected = await inspectSkillArtifact(request.artifactPath, request.plan.artifactIntegrity);
            if (!inspected.matchesExpected)
                throw new Error('Skill artifact bytes do not match the immutable plan integrity');
            ...
            if (parsed.name !== request.plan.extensionId)
                throw new Error('SKILL.md name does not match the immutable plan');
            const options = config.projectRoot === null ? {} : { cwd: config.projectRoot };
            const visible = await this.skills.snapshot(options);
            if (!visible.complete)
                throw new Error('Skill registry snapshot is incomplete');
            ...
            if (winner !== undefined) {
                ...
                const definition = await this.skills.get(parsed.name, options);
        ...
        return Object.freeze({
            ...
            stagingPath,
            prepared: Object.freeze({ parsed, configuration: config, destination }),
        });
    }
    ...
    async apply(prepared) {
        ...
        if (request.plan.operationKind === 'purge')
            await this.removeRetained(prepared.before);
        this.invalidate();
        return Object.freeze({
            prepared,
    ...
    async verify(applied) {
        ...
        if (operation === 'uninstall' || operation === 'purge') {
            if (current?.current !== null)
                throw new Error('removed Skill remains active in center inventory');
            ...
            if (removed !== null && removed !== undefined) {
                ...
                const winner = snapshot.skills.find((item) => {
                    if (typeof item !== 'object' || item === null)
                        return false;
                    return item.name === metadata.skillName;
                });
                const definition = await this.skills.get(metadata.skillName, options);
        ...
        if (version === null || version === undefined)
            throw new Error('managed Skill has no current material');
        ...
        const winnerInvocation = winner?.invocation;
        if (winner?.provider !== 'extension-center'
    ...
    async rollback(applied) {
        ...
        if (current === undefined)
            throw new Error('managed Skill disappeared before rollback');
    ...
    async recover(request) {
        ...
        if (snapshot === undefined || snapshot.targetKey !== request.plan.targetKey
            ...
            throw new Error('Skill recovery snapshot is absent or corrupt');
        ...
        if (current?.lastOperationId !== request.authorization.operationId) {
            ...
            if (point.destination === null || point.parsed === null)
                return null;
            const requestedConfiguration = configuration(await preflightSkillConfiguration(request.payload.configuration, request.plan.scopeKey));
            ...
            try {
                ...
                if (!info.isDirectory() || info.isSymbolicLink())
                    throw new Error('Skill recovered material directory is unsafe');
                file = join(point.destination, 'SKILL.md');
                const fileInfo = await lstat(file);
                if (!fileInfo.isFile() || fileInfo.isSymbolicLink())
                    throw new Error('Skill recovered material is unsafe');
            }
            catch (error) {
                if (error.code !== 'ENOENT')
                    throw error;
                if (point.stagingPath === null)
                    return null;
                ...
                if (!stagingInfo.isDirectory() || stagingInfo.isSymbolicLink())
                    throw new Error('Skill recovery staging directory is unsafe');
                const stagedFile = join(point.stagingPath, 'SKILL.md');
                const stagedFileInfo = await lstat(stagedFile);
                if (!stagedFileInfo.isFile() || stagedFileInfo.isSymbolicLink())
                    throw new Error('Skill recovery staged material is unsafe');
                const stagedInspection = await inspectSkillArtifact(stagedFile, point.contentIntegrity);
                if (!stagedInspection.matchesExpected)
                    throw new Error('Skill staged material integrity changed after approval');
            ...
            if (!recoveredInspection.matchesExpected)
                throw new Error('Skill recovered material integrity changed after approval');
        ...
        if (request.plan.operationKind === 'purge')
            await this.removeRetained(snapshot.before);
        const prepared = {
            request,
    ...
    async cleanup(prepared) {
        if (prepared.stagingPath !== null)
            await rm(prepared.stagingPath, { recursive: true, force: true });
    }
    async inspectManaged(version) {
        const materialBase = resolve(this.root, 'material', 'skills');
        ...
        if (!inspected.matchesExpected)
            throw new Error('managed Skill content integrity changed');
        return inspected;
    }
    async removeRetained(before) {
        const material = new Set([before?.removed?.materialPath, before?.lastGood?.materialPath].filter((path) => path !== undefined));
        for (const path of material) {
            await this.removeMaterial(path);
        }
    }
    async removeMaterial(path) {
        const directory = dirname(path);
        const base = resolve(this.root, 'material', 'skills');
        const rel = relative(base, directory);
        if (rel === '' || rel === '..' || rel.startsWith(`..${sep}`))
            throw new Error('retained Skill path escapes center material');
    ...
    async removeQuarantine(quarantine, base) {
        let info;
        try {
