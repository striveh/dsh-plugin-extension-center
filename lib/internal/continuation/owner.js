...
import { TaskContinuationInputError, agentOptions, assertSameReservation, continuationAgent, continuationId, continuationMessage, continuationRef, createRequest, isContinuationMessage, listRequest, readyDecision, reserveRequest, sameAgentOptions, supersedeRequest, } from "./codec.js";
import { InternalTaskContinuationStore, } from "./store.js";
...
const CANCELABLE_STATES = ['pending', 'ready', 'consumed'];
...
export class InternalTaskContinuationOwner {
    config;
    protocolVersion = 1;
    store;
    verifiers = new Map();
    ...
    retiringHandles = new Map();
    settling = new Set();
    settledDispatches = new Set();
    retryDelays = new Map();
    retryTimers = new Map();
    leaseTimers = new Map();
    stopSignal = new AbortController();
    ownerId = randomUUID();
    dispatchClaimLeaseMs;
    retryInitialDelayMs;
    retryMaxDelayMs;
    ...
    constructor(config) {
        ...
        if (!Number.isSafeInteger(retryInitialDelayMs) || retryInitialDelayMs < 1
            ...
            throw new TaskContinuationInputError(`task continuation retry delays must satisfy 1 <= initial <= maximum <= ${String(MAX_BUSY_TIMEOUT_MS)}`);
        }
        ...
        if (!Number.isSafeInteger(dispatchClaimLeaseMs) || dispatchClaimLeaseMs < 1
            ...
            throw new TaskContinuationInputError(`task continuation dispatchClaimLeaseMs must be 1-${String(MAX_BUSY_TIMEOUT_MS)}`);
        }
    ...
    async initialize() {
        ...
        if (this.config.observeLifecycle !== undefined) {
            ...
            if (typeof dispose !== 'function')
                throw new Error('task continuation lifecycle observer returned no disposer');
            this.disposeLifecycle = dispose;
        }
    }
    async create(agentValue, requestValue) {
        this.assertActive();
    ...
    async get(id) {
        if (!await this.activateExistingStore())
            return undefined;
        return this.store.get(continuationId(id));
    }
    async list(requestValue = {}) {
        const request = listRequest(requestValue);
        if (!await this.activateExistingStore())
            return [];
        return (await this.store.list()).filter(claim => (request.sessionId === undefined || claim.sessionId === request.sessionId)
    ...
    registerVerifier(verifierValue) {
        this.assertActive();
        ...
        const verifier = {
            id: verifierValue.id,
            verify: async (claim, signal) => await verifierValue.verify(claim, signal),
        };
        const registered = { verifier, cancellation: new AbortController() };
        ...
        return () => {
            if (!active)
                return;
            active = false;
            if (this.verifiers.get(verifier.id) !== registered)
                return;
    ...
    subscribe(listener) {
        ...
        if (typeof listener !== 'function')
            throw new TaskContinuationInputError('task continuation subscriber must be a function');
    ...
    async reconcile(signal) {
        if (this.stopping || !this.storeActivated)
            return;
        this.requested = true;
        if (this.running === undefined) {
            const operation = () => this.runRequested();
    ...
    async reserveRecord(request, options) {
        const now = Date.now();
            ...
            for (const claim of await this.store.list()) {
                ...
                try {
                    ...
                    if (current?.state === 'consumed')
                        this.scheduleRetry(current.continuationId);
                    if (current?.state === 'dispatching')
                        this.scheduleLeaseRecovery(current);
                }
            }
        }
    }
    async processClaim(captured) {
        let claim = await this.store.get(captured.continuationId);
        if (claim === undefined)
            return;
        if (claim.state === 'consumed' && this.retryTimers.has(claim.continuationId))
            return;
        ...
        if (evidence !== 'none') {
            ...
            if (claim.state !== target) {
                const observed = target === 'invalid'
                    ? await this.store.observeDispatch(claim, target, Date.now(), 'dispatch-evidence-content-mismatch')
        ...
        if (claim.state === 'dispatched') {
            ...
            if (this.settledDispatches.has(claim.continuationId)) {
                const settledEvidence = classifyDispatchEvidence(await this.readPersistedEvents(claim.sessionId), claim);
        ...
        if (claim.state === 'pending') {
            ...
            try {
                ...
                if (!this.stopping)
                    this.config.logger?.warn(`extension-center continuation verifier "${verifier.id}" failed: ${renderThrown(error)}`);
                return;
            }
            if (decision === undefined || this.stopping || this.verifiers.get(verifier.id) !== registered)
                return;
            ...
            if (next === undefined || next.state === 'invalid')
                return;
            claim = next;
        }
        ...
        if (claim.state !== 'consumed' && claim.state !== 'dispatching')
            return;
        const agent = await this.resolveAgent(claim);
        if (agent === undefined || this.stopping) {
            if (!this.stopping)
        ...
        try {
            ...
            this.config.logger?.warn(`extension-center continuation "${claim.continuationId}" follow-up was not confirmed: ${renderThrown(error)}`);
            if (unknown !== undefined)
                this.watchSettlement(agent, claim.continuationId);
            return;
        }
        try {
            ...
            this.config.logger?.warn(`extension-center continuation "${claim.continuationId}" Session flush was not confirmed: ${renderThrown(error)}`);
            if (unknown !== undefined)
                this.watchSettlement(agent, claim.continuationId);
            return;
        }
        ...
        if (finished.state === 'claimed')
            await this.releaseOwnedHandle(claim.sessionId);
        else
            this.watchSettlement(agent, claim.continuationId);
    }
    async resolveAgent(claim) {
        ...
        if (owned !== undefined)
            return owned.agent;
        if (this.config.sessions.get(claim.sessionId) !== undefined || this.stopping)
            return undefined;
        try {
            ...
            if (!this.stopping)
                this.config.logger?.warn(`extension-center could not cold-resume session "${claim.sessionId}": ${renderThrown(error)}`);
            return undefined;
        }
    }
    async readEvents(sessionId) {
        ...
        if (live !== undefined)
            return live.session.events;
        const owned = this.ownedHandles.get(sessionId);
        if (owned !== undefined)
            return owned.agent.session.events;
        try {
    ...
    async readPersistedEvents(sessionId) {
        try {
            return (await this.config.sessionPersistence.load(sessionId)).events;
        }
        catch {
    ...
    async failDispatch(claim, reason) {
        ...
        if (failed !== undefined)
            this.claimChanged(failed);
        return failed;
    }
    watchSettlement(agent, continuationId) {
        if (this.settling.has(agent.id))
            return;
        ...
        void agent.whenIdle().then(async () => {
            try {
                ...
                if (!this.stopping) {
                    this.config.logger?.warn(`extension-center continuation Session "${agent.id}" settlement flush failed: ${renderThrown(error)}`);
                }
            }
            finally {
    ...
    logInvalid(claim, reason) {
        this.config.logger?.warn(`extension-center continuation "${claim.continuationId}" invalid: ${reason}`);
    }
    async removeLiveMarker(claim) {
        const agent = this.config.agents.get(claim.sessionId) ?? this.ownedHandles.get(claim.sessionId)?.agent;
        if (agent?.inbox?.remove(claim.dispatchMessageId) !== true)
            return;
        await this.config.sessions.flush(agent.session);
    }
    async releaseOwnedHandle(sessionId) {
        const handle = this.ownedHandles.get(sessionId);
        if (handle === undefined || this.retiringHandles.has(sessionId))
            return;
        const retirement = handle.agent.whenIdle().then(async () => {
            if (this.ownedHandles.get(sessionId) !== handle)
                return;
            this.ownedHandles.delete(sessionId);
            await handle.dispose();
        }).finally(() => {
            if (this.retiringHandles.get(sessionId) === retirement)
                this.retiringHandles.delete(sessionId);
        });
        this.retiringHandles.set(sessionId, retirement);
        void retirement.catch(error => {
            if (!this.stopping)
                this.config.logger?.warn(`extension-center could not retire session "${sessionId}": ${renderThrown(error)}`);
        });
    }
    assertExactLiveAgent(agent, sessionId) {
    ...
    assertOriginalTask(events, request) {
    ...
    assertFuture(expiresAtMs, now = Date.now()) {
        if (expiresAtMs <= now)
            throw new TaskContinuationInputError('task continuation expiry must be in the future');
    }
    async activateStore() {
        await this.store.initialize();
        this.storeActivated = true;
    }
    async activateExistingStore() {
        if (this.storeActivated)
            return true;
        this.storeActivated = await this.store.initializeIfPresent();
    ...
    scheduleRetry(continuationId) {
        if (this.stopping || this.retryTimers.has(continuationId))
            return;
    ...
    scheduleLeaseRecovery(claim) {
        const leaseExpiresAtMs = claim.dispatchLeaseExpiresAtMs;
        if (this.stopping || claim.state !== 'dispatching' || leaseExpiresAtMs === undefined)
            return;
        this.clearLeaseRecovery(claim.continuationId);
    ...
    claimChanged(claim) {
        ...
        if (claim.state !== 'dispatching')
            this.clearLeaseRecovery(claim.continuationId);
        if (claim.state !== 'dispatched')
            this.settledDispatches.delete(claim.continuationId);
        this.notifySubscribers();
    }
    notifySubscribers() {
    ...
    notifySubscriber(listener) {
        if (this.stopping || !this.subscribers.has(listener))
            return;
        try {
            listener();
        }
    ...
    assertActive() {
        if (this.stopping)
            throw new Error('task continuation owner is disposed');
    }
}
/** Construct and initialize the plugin-owned durable owner. */
export async function createInternalTaskContinuations(config) {
    const owner = new InternalTaskContinuationOwner(config);
    await owner.initialize();
    return owner;
}
function persistedPresetId(value) {
    ...
    for (const event of value.events) {
        ...
        presetId = exactPresetId(event.data['agentPreset'], 'agent-preset/selected event');
    }
...
function exactPresetId(value, subject) {
    if (value === undefined)
        return undefined;
...
function hasDirectUserMessage(events, messageId) {
    return events.some(event => isUserMessageEvent(event, messageId) && sourceKind(event.data) === 'user');
}
function laterDirectUserMessage(events, originalMessageId) {
...
function isUserMessageEvent(event, id) {
...
function durableInboxInsertions(events, dispatchMessageId) {
    const output = [];
    for (const event of events) {
        ...
        for (const message of inserted) {
            if (messageId(message) === dispatchMessageId)
                output.push(message);
        }
    }
    return output;
}
function classifyDispatchEvidence(events, claim) {
...
function sourceKind(value) {
    if (typeof value !== 'object' || value === null || !('source' in value)
        || typeof value.source !== 'object' || value.source === null || !('kind' in value.source))
        return undefined;
...
async function verifierDecision(verification, signal) {
    if (signal.aborted)
        return undefined;
    let resolveAbort = () => { };
    const aborted = new Promise(resolve => { resolveAbort = resolve; });
    const onAbort = () => { resolveAbort(undefined); };
