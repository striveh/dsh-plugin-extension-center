...
import { TaskContinuationInputError, TaskContinuationMutationConflictError, agentOptions, continuationId, createRequest, invalidReason, sameAgentOptions, taskRevision, } from "./codec.js";
...
export class TaskContinuationStoreCorruptionError extends Error {
    code = 'TASK_CONTINUATION_STORE_CORRUPT';
}
...
export class InternalTaskContinuationStore {
    ...
    database;
    opening;
    constructor(root, busyTimeoutMs) {
    ...
    async createOrGet(input) {
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, NULL, 0, NULL, NULL, NULL, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`).run(claim.continuationId, claim.callerId, claim.mutationId, claim.recordRevision, claim.state, claim.dispatchMessageId, claim.sessionId, claim.originalMessageId, claim.needDigest, claim.taskRevision, claim.verifierId, claim.verificationPayloadDigest, claim.createdAtMs, claim.updatedAtMs, claim.expiresAtMs, claim.resumeAgentOptions.provider ?? null, claim.resumeAgentOptions.model ?? null, claim.resumeAgentOptions.maxTokens ?? null);
            database.exec('COMMIT');
    ...
    async getByMutation(callerId, mutationId) {
    ...
    async transition(request) {
        ...
        WHERE continuation_id = ? AND record_revision = ?`).run(current.recordRevision + 1, request.to, request.updatedAtMs, replacement ?? null, clearsDispatch ? null : current.dispatchOwnerId ?? null, clearsDispatch ? 0 : current.dispatchFence, clearsDispatch ? null : current.dispatchLeaseExpiresAtMs ?? null, clearsDispatch ? null : current.dispatchStartedAtMs ?? null, clearsDispatch || request.to === 'invalid' ? null : current.deliveryUnknownReason ?? null, exactInvalidReason ?? null, request.id, current.recordRevision);
    ...
    async claimDispatch(request) {
        ...
        if (request.leaseExpiresAtMs <= request.nowMs)
            throw new Error('task continuation dispatch lease must be in the future');
        const database = await this.ready();
        database.exec('BEGIN IMMEDIATE');
        try {
            const row = database.prepare('SELECT * FROM claims WHERE continuation_id = ?').get(request.id);
            ...
            if (!Number.isSafeInteger(fence))
                throw new Error('task continuation dispatch fence overflow');
            const result = database.prepare(`UPDATE claims SET
        ...
        WHERE continuation_id = ? AND record_revision = ?`).run(current.recordRevision + 1, request.nowMs, request.ownerId, fence, request.leaseExpiresAtMs, request.id, current.recordRevision);
            if (Number(result.changes) !== 1) {
                database.exec('ROLLBACK');
                return undefined;
            }
    ...
    async observeDispatch(claim, to, updatedAtMs, reason) {
        ...
        invalid_reason = ?`, [to, exactInvalidReason ?? null]);
            ...
            if (exactInvalidReason === undefined)
                delete observed.invalidReason;
    ...
    async ready() {
        ...
        if (this.database === undefined)
            throw new Error('task continuation store is closed');
...
function initializeSchema(database) {
    ...
    if (applicationId !== APPLICATION_ID || version !== SCHEMA_VERSION) {
        throw new TaskContinuationStoreCorruptionError(`task continuation database has application id ${String(applicationId)} and version ${String(version)}`);
    }
    const names = objects.map(row => readString(record(row), 'name'));
    if (names.length !== 1 || names[0] !== 'claims') {
        throw new TaskContinuationStoreCorruptionError('task continuation database has unexpected schema objects');
...
function createClaimsTable(database, table) {
...
function assertNoLegacyInvalidClaims(database) {
    if (database.prepare("SELECT continuation_id FROM claims WHERE state = 'invalid' LIMIT 1").get() !== undefined) {
        throw new TaskContinuationStoreCorruptionError('task continuation database contains a legacy invalid claim without an exact invalid reason and cannot migrate');
    }
}
function decodeRow(value) {
    try {
        ...
        if ((state === 'superseded') !== (typeof superseded === 'string'))
            throw new Error('invalid superseded revision');
        ...
        const deliveryUnknownReason = readNullableString(row, 'delivery_unknown_reason');
        ...
        assertDispatchProjection(state, dispatchOwnerId, dispatchFence, dispatchLeaseExpiresAtMs, dispatchStartedAtMs, deliveryUnknownReason, exactInvalidReason);
        ...
        if (error instanceof TaskContinuationStoreCorruptionError)
            throw error;
        throw new TaskContinuationStoreCorruptionError('invalid task continuation claim row', { cause: error });
    }
}
function assertSameStoredReservation(existing, input) {
    const request = input;
...
function assertTransition(from, to) {
    ...
    if (!valid)
        throw new Error(`invalid task continuation transition ${from} -> ${to}`);
}
function rowClaim(database, id) {
    const row = database.prepare('SELECT * FROM claims WHERE continuation_id = ?').get(id);
    return row === undefined ? undefined : decodeRow(row);
}
function updateDispatch(database, current, updatedAtMs, assignments, values) {
    const result = database.prepare(`UPDATE claims SET ${assignments},
    record_revision = ?, updated_at_ms = ?
    WHERE continuation_id = ? AND record_revision = ?`).run(...values, current.recordRevision + 1, updatedAtMs, current.continuationId, current.recordRevision);
    if (Number(result.changes) !== 1)
        throw new Error('task continuation dispatch compare-and-set failed');
}
function matchesStartedDispatch(current, request) {
...
function assertDispatchFence(request) {
    assertOwnerId(request.ownerId);
...
function assertOwnerId(value) {
    if (value.length < 1 || value.length > 128)
        throw new Error('task continuation dispatch owner id is invalid');
}
function assertTimestamp(value, subject) {
    if (!Number.isSafeInteger(value) || value < 0)
        throw new Error(`${subject} must be a non-negative integer`);
}
function assertDispatchProjection(state, ownerId, fence, leaseExpiresAtMs, startedAtMs, unknownReason, exactInvalidReason) {
    ...
    if (fence < 1)
        throw new Error('invalid owned dispatch fence');
    if (state === 'dispatching') {
        if (leaseExpiresAtMs === undefined || unknownReason !== undefined)
            throw new Error('invalid dispatching projection');
        return;
    }
    ...
    if (state !== 'invalid' || unknownReason !== undefined)
        throw new Error('invalid owned dispatch state');
}
...
function readNonEmptyString(row, key) {
...
function readNullableInteger(row, key) {
