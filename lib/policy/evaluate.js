...
export const POLICY_REVISION = 'extension-center-p0-policy-v2';
export const SUPPORTED_DSH_VERSION = '0.1.2-alpha.1';
 ...
 */
export function admittedAuthorityDigest(input) {
 ...
 */
export function evaluateCandidatePolicy(input) {
    if (!input.catalogVerified)
        return denied('catalog-unavailable', 'catalog trust verification is unavailable');
    if (!input.catalogComplete)
        return denied('catalog-incomplete', 'catalog or inventory observation is incomplete');
    ...
    const action = input.entry.lifecycle[input.operationKind];
    ...
    if (input.entry.artifact.version === 'latest'
    ...
    if (!input.entry.scopes.includes(input.selectedScope)) {
        return denied('scope-unavailable', 'the selected scope is not admitted for this candidate');
    }
    if (!input.authorityKnown)
        return denied('authority-unknown', 'the authority delta is incomplete');
...
export function snapshotPolicyResult(value) {
