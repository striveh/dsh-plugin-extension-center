...
export async function apply(ctx, config = {}) {
    ...
    const writableBinding = ctx.inject(['mcpConnections', 'taskContinuations', 'skills', 'tools', 'loader'], async (ownerContext) => {
        ...
        const retiredPluginObligations = new Set();
        const retiredPluginTargets = new Set();
        const obligationKey = (operationId, targetKey, profileId) => (`${operationId}\0${targetKey}\0${profileId}`);
        const targetObligationKey = (targetKey, profileId) => `${targetKey}\0${profileId}`;
        const plugin = new PluginLifecycleProvider(state, loader, {
            ...
            isOperationQuarantined: (operationId, targetKey, profileId) => (retiredPluginObligations.has(obligationKey(operationId, targetKey, profileId))),
            isTargetQuarantined: (targetKey, profileId) => (retiredPluginTargets.has(targetObligationKey(targetKey, profileId))),
        });
        ...
        generation.start(async (signal) => {
            ...
            for (const obligation of await runtime.runner.retiredPluginObligations(signal)) {
                retiredPluginObligations.add(obligationKey(obligation.operationId, obligation.targetKey, obligation.profileId));
    ...
    ctx.effect(() => host.connection.rpc.handle(EXTENSION_CENTER_RPC_CHANNEL, handler), 'extension-center: authenticated RPC');
}
