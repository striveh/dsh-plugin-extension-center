...
`;
const WIKI_PAGE_WRITER_V2_BODY = WIKI_PAGE_WRITER_V1_BODY.replace(`description: ${WIKI_PAGE_WRITER_DESCRIPTION}\n---\n`, `description: ${WIKI_PAGE_WRITER_DESCRIPTION}\nlicense: MIT\nmetadata:\n  author: Microsoft\n  version: "1.0.0"\n---\n`);
...
export function filesystemMcpCandidate(candidateRef, version) {
    return FILESYSTEM_MCP_CANDIDATES.find(candidate => candidate.candidateRef === candidateRef && candidate.version === version) ?? null;
}
/** Resolve only an exact Filesystem MCP candidate reference admitted by this build. */
export function filesystemMcpCandidateRef(candidateRef) {
...
export function skillCandidate(candidateRef, version) {
    return SKILL_CANDIDATES.find(candidate => candidate.candidateRef === candidateRef && candidate.version === version) ?? null;
}
const UPDATE_SUCCESSORS = new Map([
...
export function candidateUpdateSuccessor(candidateRef) {
...
export function taskAcquisitionCandidates(entries, scopeKey, outcomeTags) {
    const matching = entries.filter(entry => entry.scopes.includes(scopeKey)
...
//# sourceMappingURL=kind-candidates.js.map
