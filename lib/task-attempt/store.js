...
export class FileTaskAttemptStore {
    ...
    async reconcileContinuation(taskAttemptId, state, nowMs, invalidReason) {
        assertContinuationInvalidReason(state, invalidReason);
        return this.serialize(async () => {
            const prior = await this.required(taskAttemptId);
            if (prior.outcome !== null || state === 'pending' || state === 'ready')
                return prior;
...
function continuationTerminal(state, invalidReasonValue) {
    switch (state) {
...
function assertContinuationInvalidReason(state, value) {
    if (state === 'invalid') {
    ...
    if (value !== undefined)
        throw new Error('task continuation invalid reason requires invalid state');
}
