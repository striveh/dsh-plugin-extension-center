...
import { readBoundedString, readLiteral, readNonNegativeInteger, readSha256Digest, readStrictRecord, } from "../domain/codec.js";
...
const TRIGGERS = ['model', 'choice-selection', 'retry-original'];
const DERIVATIONS = ['choice-selection', 'retry-original'];
const ACTIONS = ['configure', 'enable', 'restore', 'update'];
...
const SCOPES = ['profile:web', 'project', 'user'];
...
function nullableString(value, path, maximum = 512) {
...
function sortedUnique(value, path, maximum, allowed) {
    if (!Array.isArray(value) || value.length > maximum)
        fail(path, 'must be a bounded array');
    const output = value.map((item, index) => {
        const text = readBoundedString(item, `${path}[${String(index)}]`, 256);
        if (allowed === undefined ? !TAG.test(text) : !allowed.includes(text))
            fail(`${path}[${String(index)}]`, 'is invalid');
        return text;
    });
...
function need(value, path) {
    ...
    if (outcomeTags.length === 0)
        fail(`${path}.outcomeTags`, 'must not be empty');
    ...
    if (inputModalities.length === 0 || outputModalities.length === 0)
        fail(path, 'must retain input and output modalities');
...
function route(value, path) {
    ...
    if (keys.some(key => !['maxTokens', 'model', 'provider'].includes(key)))
        fail(path, 'contains an unsupported field');
    const input = value;
...
function candidateRefs(value, path) {
    if (!Array.isArray(value) || value.length === 0 || value.length > 3)
        fail(path, 'must contain one to three candidates');
...
export function decodeTaskAttempt(value, expectedTaskAttemptId) {
    ...
    if ((trigger === 'model') !== (parentAttemptId === null))
        fail(path, 'has contradictory trigger and parent');
    ...
    if (!isAbsolute(projectRoot) || resolve(projectRoot) !== projectRoot)
        fail(`${path}.projectRoot`, 'must be a canonical absolute path');
    const decodedNeed = need(record.need, `${path}.need`);
    const needDigest = readSha256Digest(record.needDigest, `${path}.needDigest`);
    if (needDigest !== canonicalSha256(decodedNeed))
        fail(`${path}.needDigest`, 'does not bind need');
    ...
    if (createdAtMs >= expiresAtMs || updatedAtMs < createdAtMs)
        fail(path, 'has an invalid time interval');
    ...
    if ((outcome === null) !== (reason === null))
        fail(path, 'must assign outcome and bounded reason together');
    const decodedResult = result(record.result, `${path}.result`);
    const phase = readLiteral(record.phase, PHASES, `${path}.phase`);
    if (outcome === 'use-existing' && decodedResult?.kind !== 'use-existing')
        fail(path, 'has no existing-capability result');
    if (outcome === 'choice-required' && decodedResult?.kind !== 'choice-required')
        fail(path, 'has no choice result');
    if (outcome === 'management-required' && decodedResult?.kind !== 'management-required')
        fail(path, 'has no management result');
    if (outcome === null) {
        if (phase === 'checking-existing' || phase === 'resolving') {
            if (decodedResult !== null)
                fail(path, 'assigns a result before resolution');
        }
        ...
        if (decodedResult !== null)
            fail(path, 'has an unexpected terminal result');
    }
...
export function decodeTaskAttemptDerivation(value, expectedSourceAttemptId) {
    ...
    if (expectedSourceAttemptId !== undefined && sourceAttemptId !== expectedSourceAttemptId)
        fail(`${path}.sourceAttemptId`, 'does not bind storage identity');
    ...
    if ((kind === 'choice-selection') !== (candidateRef !== null))
        fail(path, 'has contradictory derivation input');
    const attempt = decodeTaskAttempt(record.attempt);
    if (attempt.parentAttemptId !== sourceAttemptId || attempt.trigger !== kind)
        fail(`${path}.attempt`, 'does not bind source and trigger');
    const createdAtMs = readNonNegativeInteger(record.createdAtMs, `${path}.createdAtMs`);
    if (createdAtMs !== attempt.createdAtMs)
        fail(`${path}.createdAtMs`, 'does not bind the derived attempt');
    return immutableJsonClone({
        schemaVersion: 1,
        sourceAttemptId,
        kind,
...
export function decodeTaskRetryContinuation(value, expectedTaskAttemptId) {
    ...
    if (canceledAtMs !== null && canceledAtMs < createdAtMs)
        fail(`${path}.canceledAtMs`, 'precedes creation');
    return immutableJsonClone({
        schemaVersion: 1,
        taskAttemptId,
