...
async function clearExecutionLease(record) {
    const current = await readOptionalRecord(record.path, MAX_POINTER_BYTES, 'official DSH recovery execution lease');
    if (canonicalJson(current) !== canonicalJson(record.value))
        failure('official DSH recovery execution lease changed');
...
async function runOfficialDsh(binding, profileId, arguments_, label, metadataCache) {
    ...
    try {
        await new Promise((accept, reject) => {
            ...
            const abort = (cause) => {
                ...
                if (pid !== undefined) {
                    ...
                    catch { /* process group did not start */ }
                }
                reject(cause);
            };
            child.once('spawn', () => {
                const pid = child.pid;
                if (pid === undefined)
                    return abort(new Error('official DSH recovery supervisor has no process id'));
            ...
            child.once('close', (code, signal) => {
                ...
                if (launchError !== undefined)
                    return reject(new Error(`official DSH recovery ${label} could not start`, { cause: launchError }));
                if (code === 0)
                    return accept();
                if (code === 124 || timedOut)
                    return reject(new Error(`official DSH recovery ${label} timed out`));
                if (code === 125)
                    return reject(new Error(`official DSH recovery ${label} lost its parent`));
        ...
        if (lease !== null)
            await clearExecutionLease(lease);
...
async function assertProfileExecutionDead(destination, owner, profileId) {
    const executionValue = await readOptionalRecord(join(destination, 'execution.json'), MAX_POINTER_BYTES, 'Plugin recovery held execution');
