...
function decodeRecoveryExecutable(value) {
    ...
    if (!isAbsolute(executablePath) || !isAbsolute(hostCliPath))
        failure('recovery executable paths must be absolute');
    return Object.freeze({
        schemaVersion: 1,
        executablePath,
...
function hostEnvironment() {
    const allowed = ['DSH_HOME', 'LANG', 'LC_ALL', 'LC_CTYPE', 'TZ', 'TMPDIR', 'TMP', 'TEMP', 'SYSTEMROOT', 'WINDIR'];
    const output = {};
...
function runHost(hostCliPath, profileId, operation) {
...
function verifyRestored(before, after, target) {
    ...
    if (after.snapshot.bootStatus !== expectedStatus)
