...
import { isCurrentPnpmExecutionIdentity } from "../plans/pnpm-runtime.js";
...
function executionDispatchRecord(record) {
    const value = Object.freeze({
        schemaVersion: 1,
...
function writeExecutionDispatch(record) {
    if (constants.O_NOFOLLOW === undefined)
        fail('official DSH execution dispatch cannot exclude symlinks');
    const descriptor = openSync(record.path, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, 0o600);
    try {
...
async function clearExecutionFile(record, label) {
    let body;
    try {
...
async function clearOptionalExecutionFile(record, label) {
    try {
        await clearExecutionFile(record, label);
    }
    catch (error) {
        if (!(error instanceof Error && error.message === `${label} disappeared`))
            throw error;
    }
}
async function assertExecutionFileAbsent(path, label) {
    try {
...
export async function runBoundOfficialDsh(binding, profileId, arguments_, label, metadataCache) {
    ...
    const durableRecords = { execution: null, dispatch: null, dispatchDurable: false };
    ...
    try {
        ...
        if (launchError !== undefined)
            throw new Error(`official DSH Plugin ${label} could not start`, { cause: launchError });
        ...
        if (childOutcomeOverflow)
            fail('official DSH supervisor child outcome is not one bounded JSON record');
        if (childOutcomeBytes === 0) {
            if (supervisorClose.code === 125)
                throw new Error(`official DSH Plugin ${label} lost its parent`);
            throw new Error('official DSH supervisor closed without a child outcome');
        }
        const childOutcome = decodeSupervisorChildOutcome(Buffer.concat(childOutcomeChunks, childOutcomeBytes), 'official DSH supervisor child outcome');
        ...
        if (execution !== null && processGroupQuiescent) {
            if (dispatch === null) {
                await assertExecutionFileAbsent(join(dirname(execution.path), 'execution-dispatch.json'), 'official DSH execution dispatch');
            }
            else {
