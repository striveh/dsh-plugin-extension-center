...
async function installExclusive(path, bytes) {
    ...
    try {
        try {
            ...
            if (!installed.equals(bytes))
                fail('installed recovery executable pin already contains different bytes');
        }
...
export async function installRecoveryExecutable(input) {
    ...
    if (!hostHomeState.isDirectory() || hostHomeState.isSymbolicLink())
        fail('Host home must be a canonical directory');
 ...
 */
export async function installPackagedRecoveryExecutable(root, hostHome) {
    const hostArgument = process.argv[1];
    if (hostArgument === undefined)
        fail('the launching DSH CLI path is unavailable');
