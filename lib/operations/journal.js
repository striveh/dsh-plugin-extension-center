...
import { canonicalSha256, immutableJsonClone } from "../domain/json.js";
import { decodeOperationAuthorization, decodeStoredRecoveryExecutableBinding } from "../plans/codec.js";
import { decodePlanReviewEvidence } from "../plans/review-codec.js";
import { OPERATION_RECORD_SCHEMA_VERSION, } from "./types.js";
...
const MANAGED_OBJECTS = ['artifact', 'connection'];
const EXTERNAL_RUNTIME_ACTIONS = ['download', 'none'];
const TERMINAL_PHASES = [
    'committed',
    'rolled-back',
    'failed',
];
const EXTENSION_KINDS = ['plugin', 'mcp', 'skill'];
const DESIRED_STATES = ['enabled', 'disabled', 'removed'];
const EVIDENCE_STATUSES = ['proven', 'not-required', 'not-proven'];
const NOT_PROVEN_CLAIMS = ['mutation', 'verification', 'rollback', 'restart', 'recovery'];
const NEXT_PHASES = {
...
function readNullableReason(value, path) {
    if (value === null)
        return null;
...
function readDigestArray(value, path) {
    if (!Array.isArray(value))
        failDomain('invalid-data', `${path} must be an array`);
...
function decodeFences(value, path) {
    ...
    if (catalogRevision === 0)
        failDomain('invalid-data', `${path}.catalogRevision must be positive`);
...
function decodePlanEvidence(value, path) {
    const record = readStrictRecord(value, [
        'origin',
        'candidateRef',
        'extensionKind',
...
function decodeReceiptEvidence(value, body, path) {
    ...
    if (!Array.isArray(record.notProven))
        failDomain('invalid-data', `${path}.notProven must be an array`);
...
function decodeRuntimeBinding(value, path) {
    if (value === null)
        return null;
...
function assertManagedObjectBinding(operationKind, managedObject, externalRuntimeAction, runtimeBinding, path) {
...
function decodeReceiptBody(value) {
    const record = readStrictRecord(value, [
        'schemaVersion',
        'operationId',
        'planId',
    ...
    assertManagedObjectBinding(body.operationKind, body.managedObject, body.externalRuntimeAction, body.runtimeBinding, 'receipt.body');
    if (body.journalEventCount === 0)
        failDomain('invalid-data', 'receipt must reference a non-empty journal');
    return immutableJsonClone(body);
}
/**
 ...
 */
export function decodeOperationReceipt(value) {
    ...
    if (canonicalSha256(body) !== digest)
        failDomain('journal-corrupt', 'receipt digest does not match its body');
    return immutableJsonClone({ body, digest });
}
function decodeEntry(value) {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
        failDomain('invalid-data', 'journal event entry must be an object');
    }
    const type = readLiteral(value.type, ['operation-opened', 'phase-transition', 'mutation-observed', 'verification-observed', 'receipt-issued'], 'journal.event.entry.type');
    if (type === 'operation-opened') {
        const record = readStrictRecord(value, ['type', 'planId', 'planHash', 'operationKind', 'managedObject', 'externalRuntimeAction', 'runtimeBinding', 'planEvidence', 'beforeDigest'], 'journal.event.entry');
        ...
        assertManagedObjectBinding(opened.operationKind, opened.managedObject, opened.externalRuntimeAction, opened.runtimeBinding, 'journal.event.entry');
        return opened;
    }
    if (type === 'phase-transition') {
        const record = readStrictRecord(value, ['type', 'from', 'to', 'evidenceDigest', 'reason'], 'journal.event.entry');
        return immutableJsonClone({
            type,
    ...
    return immutableJsonClone({ type, receipt: decodeOperationReceipt(record.receipt) });
}
...
function decodeJournal(value) {
    ...
    if (!Array.isArray(record.events))
        failDomain('invalid-data', 'journal.events must be an array');
    return immutableJsonClone({
        schemaVersion: OPERATION_RECORD_SCHEMA_VERSION,
...
function receiptBody(projection, checkpoint, issuedAtMs) {
    if (!TERMINAL_PHASES.includes(projection.phase)) {
...
function assertTransition(projection, entry) {
    ...
    const terminal = TERMINAL_PHASES.includes(entry.to);
    ...
    if (entry.to === 'committed') {
        ...
        if (entry.reason !== null)
            failDomain('journal-transition', 'commit cannot carry a failure reason');
    }
    if (entry.to === 'rolled-back') {
        ...
        if (entry.reason !== null)
            failDomain('journal-transition', 'rolled-back outcome cannot carry a failure reason');
    }
    if (entry.to === 'failed') {
...
function applyEntry(projection, event) {
    const { entry } = event;
    if (projection === null) {
        if (entry.type !== 'operation-opened')
            failDomain('journal-transition', 'journal must begin with operation-opened');
        return immutableJsonClone({
            operationId: event.operationId,
            targetKey: event.targetKey,
            planId: entry.planId,
    ...
    if (projection.receipt !== null)
        failDomain('journal-transition', 'receipt must be the final journal event');
    if (entry.type === 'operation-opened')
        failDomain('journal-transition', 'operation-opened cannot repeat');
    if (entry.type === 'phase-transition') {
        assertTransition(projection, entry);
        return immutableJsonClone({
            ...projection,
            phase: entry.to,
            afterDigest: TERMINAL_PHASES.includes(entry.to) ? entry.evidenceDigest : null,
    ...
    if (entry.type === 'verification-observed') {
        ...
        return immutableJsonClone({
            ...
            completedReviewPhases: [
                ...projection.completedReviewPhases,
                ...(!projection.completedReviewPhases.includes('verify') ? ['verify'] : []),
    ...
    return immutableJsonClone({ ...projection, lastAtMs: event.atMs, receipt: entry.receipt });
}
function projectJournal(journal) {
    let previousDigest = null;
    let previousAtMs = 0;
    let projection = null;
    for (const [index, event] of journal.events.entries()) {
        const expectedSequence = index + 1;
        if (event.operationId !== journal.operationId
    ...
    if (projection === null)
        failDomain('journal-corrupt', 'journal is empty');
    return projection;
}
/**
 ...
 */
export function operationJournalCheckpoint(value) {
 ...
 */
export function verifyOperationJournal(value, expectedCheckpoint) {
    const journal = decodeJournal(value);
    ...
    if (expectedCheckpoint !== undefined) {
        ...
        if (journal.events.length < eventCount)
            failDomain('journal-truncated', 'journal is shorter than its anchored head');
...
function appendEntry(value, entryValue, atValue) {
    const journal = decodeJournal(value);
    const projection = projectJournal(journal);
    const entry = decodeEntry(entryValue);
    const atMs = readNonNegativeInteger(atValue, 'atMs');
    if (atMs < projection.lastAtMs)
        failDomain('journal-corrupt', 'journal append time moved backwards');
    ...
    const event = immutableJsonClone({ ...unsigned, digest: canonicalSha256(unsigned) });
    const appended = immutableJsonClone({ ...journal, events: [...journal.events, event] });
 ...
 */
export function createOperationJournal(authorizationValue, beforeDigestValue, atValue) {
 ...
 */
export function transitionOperation(journal, to, evidenceDigest, reason, atMs) {
 ...
 * @returns New immutable journal.
 */
export function recordOperationMutation(journal, mutationDigest, atMs) {
 ...
 * @returns New immutable journal.
 */
export function recordOperationVerification(journal, verificationDigest, atMs) {
 ...
 */
export function issueOperationReceipt(journal, atMs) {
    const projection = verifyOperationJournal(journal);
    if (projection.receipt !== null)
        failDomain('journal-transition', 'operation receipt was already issued');
    const issuedAtMs = readNonNegativeInteger(atMs, 'atMs');
    if (issuedAtMs < projection.lastAtMs)
        failDomain('journal-corrupt', 'receipt time moved backwards');
