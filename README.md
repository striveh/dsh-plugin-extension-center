...
At Host startup the built dependency-free recovery CLI is copied atomically to `$DSH_HOME/extension-center/recovery/<package-version>/<platform>-<arch>/break-glass.mjs`. Every consumed operation records that exact absolute path and SHA-256 together with the exact public DSH CLI path and SHA-256. If the Center or Web cannot load, run the pinned absolute file with `node <pinned-break-glass.mjs> <center-root> <operation-id>`. It verifies its own bytes, the Host CLI bytes, the canonical journal chain, `CURRENT` head, plan evidence, and any receipt before it asks the Host to list and restore one exact recoverable Profile generation. It never imports the Center runtime and never writes a Profile directly. Success means only `profile restored; Center journal reconciliation pending`; a later healthy Center start must reconcile the retained journal.

The immutable rc.2 Host is the negative lane: its generic pnpm-forwarding `dsh plugin` command does not implement the exact JSON `list` and generation `restore` protocol, so recovery exits fail-closed without claiming a restore. Full recovery remains gated on a published DSH release with that public Profile transaction Consumer and packed crash-path acceptance.
