...
The coordinates below are valid only when the matching GitHub Release exists and its public-release receipt passes. Otherwise the release installation claim is unavailable; a development checkout is not a substitute. For a published release, install the Center from its immutable GitHub Release asset, not from a moving branch. The official DSH Plugin CLI delegates Profile package management to pnpm, so `pnpm` must be available on `PATH`:
...
Use the published `main` branch for source review and development only. Pin an exact reviewed commit instead of a moving branch when exercising the packed Store slice against official DSH rc.2:
