...
> Historical npm-era acceptance lane. It is retained for audit only and is not an active compatibility or completion gate. Current acceptance uses the deterministic packed artifact and `pnpm run test:compat:latest` against the latest reviewed official DSH.
...
The production default targets official DSH `0.1.2-alpha.3`, installs immutable bootstrap release `dsh-plugin-extension-center@0.2.0-alpha.0`, and resolves the public `@next` tag to an exact newer registry target before updating:
