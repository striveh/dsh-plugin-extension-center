...
- `Update release`: every later prerelease or stable release must prove a distinct last-successful-to-current Center artifact update and bind that predecessor's exact successful post-publication receipt. Recovery rc.2 therefore updates from rc.0 and separately binds rc.1 as `not-release-ready`; stable updates from successful rc.2. The predecessor receipt's deployed `rN` catalog must be the current artifact's packaged bootstrap, while the current Pages deployment must be the signed adjacent successor `rN+1`. Historical rc.0 uses release-ready schema 2; rc.2 and later receipts use schema 3 so the rc.1 incident remains transitively bound.
...
2. The runtime-release receipt proves Host boot, Client boot, RPC registration, exact official DSH tree preservation, and, when a previous artifact is supplied, a distinct previous-to-current Center update in one Profile while retaining the same persistent Center root. It records the previous adjacent-only catalog degradation and requires the current artifact to migrate that cache and reach the fresh public successor.
...
6. The post-publication verifier always runs from the workflow-dispatch `github.sha` checked out on protected `main`, records the protected-ref assertion plus that post-publication run id and attempt, and binds the verifier commit to its own exact successful `main`-push CI receipt. The published target commit remains independent. A distinct verifier commit is admitted only to backfill the immutable `0.1.0-rc.0` bootstrap; later candidates require the target and verifier commit to be identical. For an update, the workflow downloads the exact previous successful release-ready receipt plus its original target and verifier CI receipt bytes from a caller-supplied successful run id, binds the Actions run path, head commit and attempt plus all three receipt digests, and verifies the previous version, immutable Release, catalog transition, CI, and acceptance evidence. Recovery rc.2 additionally binds the separate rc.1 `not-release-ready` incident receipt for failed run `33103882901`; it never consumes rc.1 as a successful predecessor. A regenerated CI or incident receipt does not substitute for prior bytes. The successful sequence is rc.0 `r8→r9`, recovery rc.2 with packaged `r9` and deployed `r10`, then stable from rc.2 with packaged `r10` and deployed `r11`; immutable rc.1 remains failed history.
