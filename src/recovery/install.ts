...
import {
  chmod, copyFile, link, lstat, mkdir, open, readFile, readdir, readlink, realpath, rename, rm, unlink,
} from 'node:fs/promises'
import { createRequire } from 'node:module'
import { dirname, isAbsolute, join, relative, resolve, sep } from 'node:path'
import { fileURLToPath } from 'node:url'
import type {
  OfficialDshDependencyBinding,
  OfficialDshRecoveryBinding,
...
interface ParsedPackageManifest extends Readonly<Record<string, unknown>> {
  readonly name: string
  readonly version: string
}
...
function assertRecoverySeparation(
  root: string,
  officialDsh: Pick<OfficialDshRecoveryBinding, 'hostHome' | 'packageRoot'>,
): void {
...
async function readRegularNoFollow(path: string, label: string, maximumBytes = MAX_EXECUTABLE_BYTES): Promise<Buffer> {
...
async function regularSha256(path: string, label: string): Promise<RecoveryExecutableBinding['executableSha256']> {
...
async function readJsonFile(path: string, label: string): Promise<ParsedPackageManifest> {
  let value: unknown
  try {
    value = JSON.parse((await readRegularNoFollow(path, label, 8 * 1024 * 1024)).toString('utf8')) as unknown
  } catch (cause) {
  ...
  return value as ParsedPackageManifest
}
...
async function hashTree(
  ...
  if (info.isFile()) {
    if (info.size > MAX_PACKAGE_FILE_BYTES) fail(`package file is outside its byte bound: ${name}`)
    hash.update(`file:${name}:${String(info.size)}\0`)
    hash.update(await readFile(path))
    return
  }
  if (!info.isDirectory()) fail(`package contains unsupported entry ${JSON.stringify(name)}`)
...
async function packageTreeSha256(
  packageRoot: string,
  ignoreRootNodeModules: boolean,
): Promise<RecoveryExecutableBinding['executableSha256']> {
  const hash = createHash('sha256')
  await hashTree(packageRoot, packageRoot, hash, ignoreRootNodeModules)
  return `sha256:${hash.digest('hex')}`
}

async function packageRootFromResolved(packageName: string, resolvedPath: string): Promise<string> {
  ...
  for (;;) {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT'
        && !(error instanceof Error && error.message.includes('is unreadable'))) throw error
    }
    const parent = dirname(current)
    if (parent === current) fail(`official DSH production dependency ${packageName} has no matching package root`)
    current = parent
  }
}

async function resolveDependencyRoot(packageName: string, ownerRoot: string): Promise<string> {
  ...
  for (;;) {
    ...
    try {
      ...
      if (manifest.name !== packageName) fail(`official DSH production dependency identity changed: ${packageName}`)
...
async function bindProductionClosure(packageRoot: string): Promise<readonly OfficialDshDependencyBinding[]> {
  const queued: string[] = [packageRoot]
  const visited = new Set<string>([packageRoot])
  const dependencies: OfficialDshDependencyBinding[] = []
  while (queued.length > 0) {
    ...
    for (const packageName of [...new Set([...declared, ...optional])].sort()) {
      ...
      try {
        ...
        if (optional.includes(packageName) && !declared.includes(packageName)) continue
        throw error
      }
      if (visited.has(dependencyRoot)) continue
      visited.add(dependencyRoot)
      const dependencyManifest = await readJsonFile(
        join(dependencyRoot, 'package.json'),
        `official DSH production dependency ${packageName} manifest`,
      )
      if (dependencyManifest.name !== packageName) fail(`official DSH production dependency identity changed: ${packageName}`)
      ...
      if (dependencies.length > 1_024) fail('official DSH production dependency closure exceeds its bound')
    }
  }
...
async function installExclusive(path: string, bytes: Buffer, mode = 0o500): Promise<void> {
  ...
  try {
    try {
      ...
      if (!installed.equals(bytes)) fail('installed immutable executable already contains different bytes')
    }
...
async function copyPackageTree(sourceRoot: string, destinationRoot: string, packageRoot = sourceRoot): Promise<void> {
  ...
  for (const entry of (await readdir(sourceRoot, { withFileTypes: true }))
    ...
    if (entry.isSymbolicLink()) fail(`bundled pnpm contains a symbolic link: ${entry.name}`)
    if (entry.isDirectory()) await copyPackageTree(source, destination, packageRoot)
    else if (entry.isFile()) {
      if ((await lstat(source)).size > MAX_PACKAGE_FILE_BYTES) fail(`bundled pnpm file exceeds its byte bound: ${entry.name}`)
      await copyFile(source, destination, constants.COPYFILE_EXCL)
      await chmod(destination, 0o400)
    } else fail(`bundled pnpm contains an unsupported entry: ${entry.name}`)
  }
}

function shellQuote(value: string): string {
  if (value.includes('\n') || value.includes('\r') || value.includes('\0')) fail('private pnpm shim path is unsafe')
  return `'${value.replaceAll("'", "'\"'\"'")}'`
}

async function locateBundledPnpmManifest(configured: string | undefined): Promise<string> {
...
async function installPrivateToolchain(input: Readonly<{
  root: string
  supervisorPath: string
  pnpmManifestPath?: string
  nodePath?: string
}>): Promise<Readonly<{
  node: OfficialDshRecoveryBinding['node']
  supervisorPath: string
  supervisorSha256: RecoveryExecutableBinding['executableSha256']
  pnpm: OfficialDshRecoveryBinding['pnpm']
}>> {
...
async function bindOfficialDsh(
  input: InstallRecoveryExecutableInput['officialDsh'],
  toolchain: Awaited<ReturnType<typeof installPrivateToolchain>>,
...
export async function installRecoveryExecutable(
  ...
  if (platformValue === 'win32') fail('official DSH Plugin mutation and recovery are unsupported on Windows')
  if (platformValue !== 'darwin' && platformValue !== 'linux') fail('recovery platform is unsupported')
 ...
 * @param officialDsh Exact official 0.1.2-alpha.1 startup entrypoint and Harness home.
...
export async function installPackagedRecoveryExecutable(
  root: string,
  officialDsh: InstallRecoveryExecutableInput['officialDsh'],
): Promise<RecoveryExecutableBinding> {
