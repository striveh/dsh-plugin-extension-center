...
const NEXT_PHASES: Readonly<Record<Phase, readonly Phase[]>> = {
...
type ProcessIdentityPlatform = 'darwin' | 'linux' | 'win32'

interface ProcessIdentity {
  readonly schemaVersion: 1
  readonly pid: number
  readonly platform: ProcessIdentityPlatform
  readonly machineDigest: string | null
  readonly bootDigest: string | null
  readonly birthDigest: string | null
}

interface HostIdentityEvidence {
  readonly machineMarker: string
  readonly bootMarker: string
}

let hostIdentityEvidence: Promise<HostIdentityEvidence | null> | null = null
...
interface ProfileMetadataCacheBinding {
  readonly schemaVersion: 1
  readonly profileId: string
  ...
  readonly modulesSha256: string | null
...
interface ManagedRecord extends Record<string, unknown> {
  schemaVersion: 1
  kind: 'plugin'
  extensionId: string
  targetKey: string
...
interface PluginSidecar extends Record<string, unknown> {
  schemaVersion: 1
  profileId: string
  packageName: string
  targetKey: string
  ...
  loaderEntryId: string | null
  loaderName: string | null
  restartPending: boolean
  lastGoodMaterialPath: string | null
  tombstoneMaterialPath: string | null
}

interface ProviderSnapshot {
  readonly operationId: string
  readonly targetKey: string
  readonly before: ManagedRecord | null
  readonly beforeDigest: string
  readonly digest: string
  readonly metadataCache: ProfileMetadataCacheBinding
}

interface MaterialFileEvidence {
...
interface MaterialMarker {
  ...
  readonly files: readonly MaterialFileEvidence[]
}
...
interface BreakGlassRestoreEvidence {
  readonly schemaVersion: 1
  readonly operationId: string
  readonly targetKey: string
  readonly profileId: string
  ...
  readonly beforeDigest: string
  readonly restoredManagedDigest: string
  readonly restoredRevision: number | null
  readonly status: 'settled'
}

interface RecoveryTransaction extends Record<string, unknown> {
  schemaVersion: 1
  operationId: string
  targetKey: string
  profileId: string
  ...
  sourceManaged: ManagedRecord | null
  sourceManagedDigest: string | null
  sourceSidecar: PluginSidecar
  sourceSidecarDigest: string
  restoredManaged: ManagedRecord | null
  restoredSidecar: PluginSidecar | null
  recoveryEvidence: BreakGlassRestoreEvidence
...
function strictRecord(value: unknown, fields: readonly string[], label: string): Record<string, unknown> {
  if (!isRecord(value)) failure(`${label} must be an object`)
...
function digest(value: unknown, label: string): string {
  if (typeof value !== 'string' || !SHA256.test(value)) failure(`${label} must be a canonical SHA-256 digest`)
  return value
}

function literal<T extends string>(value: unknown, values: readonly T[], label: string): T {
  if (typeof value !== 'string' || !values.includes(value as T)) failure(`${label} has an unsupported value`)
  return value as T
}
...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) failure('canonical JSON contains a non-finite number')
    return JSON.stringify(value)
  }
  if (Array.isArray(value)) return `[${value.map(canonicalJson).join(',')}]`
  if (!isRecord(value)) failure(`canonical JSON contains unsupported ${typeof value}`)
...
function processEvidenceDigest(
...
function decodeProcessIdentity(value: unknown, label: string): ProcessIdentity {
  const record = strictRecord(
    value,
    ['schemaVersion', 'pid', 'platform', 'machineDigest', 'bootDigest', 'birthDigest'],
    `${label} process identity`,
  )
  if (record.schemaVersion !== 1) failure(`${label} process identity schemaVersion is unsupported`)
...
async function processProbeCommand(
  executable: string,
  arguments_: readonly string[],
  env: NodeJS.ProcessEnv,
): Promise<Readonly<{ code: number | null; stdout: string }> | null> {
  return await new Promise(resolveResult => {
    ...
    let stdout = ''
    let settled = false
    const finish = (value: Readonly<{ code: number | null; stdout: string }> | null) => {
      if (settled) return
      settled = true
      clearTimeout(timer)
      resolveResult(value)
    ...
    child.stdout.on('data', (chunk: Buffer) => {
      if (Buffer.byteLength(stdout) >= PROCESS_PROBE_OUTPUT_LIMIT) return
...
function linuxProcessStartTicks(stat: string): string | null {
...
async function probeProcessBirth(
  pid: number,
  platform: ProcessIdentityPlatform,
): Promise<Readonly<{ status: 'present'; marker: string } | { status: 'absent' } | { status: 'unknown' }>> {
  if (platform === 'linux') {
    let stat: string
    try {
      stat = await readFile(`/proc/${String(pid)}/stat`, 'utf8')
    } catch (error: unknown) {
      return Object.freeze({ status: (error as NodeJS.ErrnoException).code === 'ENOENT' ? 'absent' : 'unknown' })
    }
  ...
  const result = await processProbeCommand(
...
async function probeHostIdentityEvidence(platform: ProcessIdentityPlatform): Promise<HostIdentityEvidence | null> {
  if (platform === 'linux') {
    try {
...
function currentHostIdentityEvidence(platform: ProcessIdentityPlatform): Promise<HostIdentityEvidence | null> {
...
async function processIdentityStatus(identity: ProcessIdentity): Promise<'alive' | 'dead' | 'unknown'> {
  ...
  if (processEvidenceDigest('machine', identity.platform, host.machineMarker) !== identity.machineDigest) return 'unknown'
  if (processEvidenceDigest('boot', identity.platform, host.bootMarker) !== identity.bootDigest) return 'dead'
...
function decodeProfileLockOwner(value: unknown, profileId: string, label: string): ProfileLockOwner {
  if (isRecord(value) && value.schemaVersion === 1) {
    failure(`${label} has no process-birth evidence; manual recovery is required`)
  }
  const owner = strictRecord(
    value,
    ['schemaVersion', 'profileId', 'ownerId', 'leaseId', 'processIdentity', 'acquiredAtMs'],
    label,
  )
  if (owner.schemaVersion !== 2 || owner.profileId !== profileId) failure(`${label} identity is invalid`)
  const leaseId = boundedString(owner.leaseId, `${label}.leaseId`)
  if (!/^lease:[0-9a-f-]{36}$/u.test(leaseId)) failure(`${label}.leaseId is invalid`)
  return Object.freeze({
    schemaVersion: 2,
    profileId,
...
function decodeProfileTakeover(value: unknown, profileId: string, label: string): ProfileTakeoverRecord {
  ...
  if (claim.schemaVersion !== 1 || claim.profileId !== profileId) failure(`${label} identity is invalid`)
  ...
  if (!/^lease:[0-9a-f-]{36}$/u.test(sourceLeaseId)
    ...
    || !/^takeover:[0-9a-f-]{36}$/u.test(takeoverId)) failure(`${label} token is invalid`)
  return Object.freeze({
    schemaVersion: 1,
    profileId,
    sourceLeaseId,
...
function fileDigest(value: Uint8Array): string {
...
function sameOrBelow(root: string, path: string): boolean {
  return resolve(root) === resolve(path) || below(root, path)
}

function verifyRecoverySeparation(centerRoot: string, binding: OfficialDshRecoveryBinding): void {
...
async function readRegularNoFollow(path: string, maximumBytes: number, label: string): Promise<Buffer> {
  if (constants.O_NOFOLLOW === undefined) failure(`${label} cannot be opened without following links on this platform`)
  const handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW)
  try {
    const opened = await handle.stat()
    if (!opened.isFile() || opened.size <= 0 || opened.size > maximumBytes) failure(`${label} has an invalid file type or byte length`)
    const bytes = await handle.readFile()
    if (bytes.length !== opened.size) failure(`${label} changed while it was read`)
    const current = await lstat(path)
    if (!current.isFile() || current.dev !== opened.dev || current.ino !== opened.ino) failure(`${label} path changed while it was read`)
    return bytes
  } finally {
    await handle.close()
  }
...
function parseCanonicalRecord(bytes: Buffer, label: string): unknown {
  ...
  if (`${canonicalJson(value)}\n` !== text) failure(`${label} is not canonical JSON`)
  return value
}

async function readOptionalRecord(path: string, maximumBytes: number, label: string): Promise<unknown | undefined> {
  try {
...
async function ensureRealDirectory(path: string): Promise<void> {
  ...
  if (canonical !== path || !info.isDirectory() || info.isSymbolicLink()) failure(`Center-owned directory is unsafe: ${path}`)
}
...
async function removeRegular(path: string, label: string): Promise<void> {
  try {
    const info = await lstat(path)
    if (!info.isFile() || info.isSymbolicLink()) failure(`${label} is not a Center-owned regular record`)
    await rm(path)
    await syncDirectory(dirname(path))
  } catch (error: unknown) {
...
function decodeProfileName(value: unknown, label: string): string {
...
function decodeRecoveryExecutable(value: unknown): RecoveryExecutableBinding {
  ...
  if (record.schemaVersion !== 5) failure('journal recoveryExecutable schemaVersion is unsupported')
  ...
  if (!isAbsolute(executablePath) || !isAbsolute(centerRoot)) failure('recovery executable and Center root paths must be absolute')
  ...
  const supervisorPath = boundedString(
    official.supervisorPath,
    'journal recoveryExecutable.officialDsh.supervisorPath',
    4_096,
  )
  ...
  const productionDependencies = official.productionDependencies.map((value, index) => {
    ...
    if (!isAbsolute(dependencyRoot)) failure(`${label}.packageRoot must be absolute`)
    return Object.freeze({
  ...
  if (node.schemaVersion !== 1 || !isAbsolute(nodePath)) failure('journal recoveryExecutable.officialDsh.node is invalid')
  ...
  return Object.freeze({
    ...
    officialDsh: Object.freeze({
      ...
      packageTreeSha256: digest(
        official.packageTreeSha256,
        'journal recoveryExecutable.officialDsh.packageTreeSha256',
      ),
      productionDependencies: Object.freeze(productionDependencies),
      entrypointPath,
      entrypointSha256: digest(
        official.entrypointSha256,
        'journal recoveryExecutable.officialDsh.entrypointSha256',
      ),
      hostHome,
      timeoutMs,
      node: Object.freeze({
        ...
        executableSha256: digest(
          ...
          'journal recoveryExecutable.officialDsh.node.executableSha256',
        ),
        version: boundedString(node.version, 'journal recoveryExecutable.officialDsh.node.version', 64),
      }),
      supervisorPath,
      supervisorSha256: digest(
        official.supervisorSha256,
        'journal recoveryExecutable.officialDsh.supervisorSha256',
      ),
      pnpm: Object.freeze({
        schemaVersion: 1,
        packageName: 'pnpm',
        ...
        packageTreeSha256: digest(
          ...
          'journal recoveryExecutable.officialDsh.pnpm.packageTreeSha256',
        ),
        entrypointPath: pnpmEntrypoint,
        entrypointSha256: digest(
          pnpm.entrypointSha256,
          'journal recoveryExecutable.officialDsh.pnpm.entrypointSha256',
        ),
        shimPath: pnpmShim,
...
function decodePlanEvidence(value: unknown, label: string): Readonly<{
  extensionId: string
  extensionKind: string
  ownerKey: string
  profileId: string
  scopeKey: string
  recoveryExecutable: RecoveryExecutableBinding
}> {
  const record = strictRecord(value, [
  ...
  if (typeof record.restartRequired !== 'boolean') failure(`${label}.restartRequired must be boolean`)
  ...
  if (review.schemaVersion !== 1 || review.kind !== 'plugin') failure(`${label}.reviewEvidence is not Plugin evidence`)
...
function decodeOpening(value: unknown, targetKey: string): Opening {
  const record = strictRecord(value, [
  ...
  if (record.type !== 'operation-opened') failure('journal must begin with operation-opened')
  ...
  if (evidence.extensionId === 'dsh-plugin-extension-center') failure('the Extension Center cannot recover a self-managed operation')
...
function decodeReceipt(value: unknown, operationId: string, targetKey: string, phase: Phase, sequence: number, head: string | null): void {
...
function decodeEntry(value: unknown, context: Readonly<{
  operationId: string
  targetKey: string
  phase: Phase | null
  sequence: number
  ...
  receiptSeen: boolean
}>): Readonly<{ phase: Phase; opening: Opening | null; receiptSeen: boolean }> {
  if (!isRecord(value) || typeof value.type !== 'string') failure('journal event entry is invalid')
  if (context.receiptSeen) failure('journal receipt must be the final event')
  if (context.phase === null) {
    if (context.sequence !== 1) failure('journal opening sequence is invalid')
  ...
  if (value.type === 'phase-transition') {
    ...
    if (record.evidenceDigest !== null) digest(record.evidenceDigest, 'journal phase transition evidenceDigest')
    ...
    if ((to === 'failed' || to === 'recovery-required') && record.reason === null) failure(`${to} requires a stable reason`)
  ...
  if (value.type === 'mutation-observed' || value.type === 'verification-observed') {
    ...
    if (!permitted.includes(context.phase)) failure(`${value.type} is invalid during ${context.phase}`)
...
async function verifyCenterRoot(centerRoot: string): Promise<string> {
  if (!isAbsolute(centerRoot) || resolve(centerRoot) !== centerRoot) failure('Center root argument must be a canonical absolute path')
  const canonical = await realpath(centerRoot)
  const info = await lstat(centerRoot)
  if (canonical !== centerRoot || !info.isDirectory() || info.isSymbolicLink()) failure('Center root is not its canonical real directory')
  const manifest = strictRecord(
    await readOptionalRecord(join(centerRoot, 'manifest.json'), MAX_POINTER_BYTES, 'Center manifest'),
    ['schemaVersion', 'centerId', 'createdAtMs'],
    'Center manifest',
  )
  if (manifest.schemaVersion !== 1 || typeof manifest.centerId !== 'string'
    ...
    || !Number.isSafeInteger(manifest.createdAtMs) || (manifest.createdAtMs as number) < 0) {
    failure('Center manifest values are invalid')
  }
  return canonical
}

async function verifyJournal(centerRoot: string, operationId: string): Promise<VerifiedJournal> {
  ...
  const pointer = strictRecord(
    parseCanonicalRecord(await readRegularNoFollow(join(directory, CURRENT_FILENAME), MAX_POINTER_BYTES, 'operation CURRENT'), 'operation CURRENT'),
    ['schemaVersion', 'operationId', 'targetKey', 'eventCount', 'headDigest'],
    'operation CURRENT',
  )
  if (pointer.schemaVersion !== 1 || pointer.operationId !== operationId) failure('operation CURRENT identity is invalid')
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (Number(match[1]) !== sequence) failure('journal event filenames are not contiguous')
    const event = strictRecord(
      parseCanonicalRecord(await readRegularNoFollow(join(directory, entry.name), MAX_EVENT_BYTES, `journal event ${String(sequence)}`), `journal event ${String(sequence)}`),
      ['schemaVersion', 'operationId', 'targetKey', 'sequence', 'previousDigest', 'atMs', 'entry', 'digest'],
      `journal event ${String(sequence)}`,
    )
    ...
    if (event.previousDigest !== previousDigest) failure(`journal event ${String(sequence)} chain is invalid`)
    const atMs = safeInteger(event.atMs, `journal event ${String(sequence)} atMs`)
    if (index > 0 && atMs < previousAtMs) failure(`journal time moved backwards at event ${String(sequence)}`)
    ...
    if (canonicalDigest(unsigned) !== eventDigest) failure(`journal event ${String(sequence)} digest does not match its content`)
  ...
  if (previousDigest !== headDigest) failure('operation CURRENT headDigest does not match the journal head')
...
async function verifyExecutable(path: string, expectedDigest: string, label: string): Promise<string> {
  const canonical = await realpath(path)
  if (canonical !== path) failure(`${label} path is not its canonical realpath`)
  const bytes = await readRegularNoFollow(path, MAX_EXECUTABLE_BYTES, label)
  if (fileDigest(bytes) !== expectedDigest) failure(`${label} hash does not match its pin`)
  return canonical
}

async function hashImmutableTree(
  root: string,
  path: string,
  ...
  if (!info.isDirectory()) failure(`official DSH package contains unsupported entry ${JSON.stringify(name)}`)
  hash.update(`dir:${name}\0`)
  const entries = (await readdir(path, { withFileTypes: true }))
    .filter(entry => !(ignoreRootNodeModules && path === root && entry.name === 'node_modules'))
    .sort((left, right) => left.name.localeCompare(right.name))
  for (const entry of entries) await hashImmutableTree(root, join(path, entry.name), hash, ignoreRootNodeModules)
}

async function immutableTreeDigest(root: string, ignoreRootNodeModules: boolean): Promise<string> {
  const hash = createHash('sha256')
  await hashImmutableTree(root, root, hash, ignoreRootNodeModules)
  return `sha256:${hash.digest('hex')}`
}

async function verifyBoundPackage(
  root: string,
  packageName: string,
  ...
  if (canonical !== root || !info.isDirectory() || info.isSymbolicLink()) failure(`${label} root changed`)
...
async function verifyBoundNode(binding: OfficialDshRecoveryBinding): Promise<void> {
  await verifyExecutable(binding.node.executablePath, binding.node.executableSha256, 'bound Node executable')
  const result = await new Promise<Readonly<{ code: number | null; output: string }>>((accept, reject) => {
...
async function verifyOfficialDsh(binding: OfficialDshRecoveryBinding): Promise<void> {
  if (process.platform === 'win32') failure('official DSH Plugin mutation and recovery are unsupported on Windows')
  ...
  const entrypoint = await verifyExecutable(
    binding.entrypointPath,
    binding.entrypointSha256,
    'official DSH recovery entrypoint',
  )
  if (!below(packageRoot, entrypoint)) failure('official DSH recovery entrypoint escapes its package root')
  ...
  if (declared !== entrypoint) failure('official DSH recovery package no longer declares its bound entrypoint')
...
function appendOutput(held: string, chunk: Buffer): string {
  if (held.length >= OUTPUT_LIMIT) return held
  return (held + chunk.toString('utf8')).slice(0, OUTPUT_LIMIT)
}

async function auditProfileExecution(binding: OfficialDshRecoveryBinding, profileId: string): Promise<string> {
  ...
  if ((await readRegularNoFollow(
    join(profilePath, 'pnpm-workspace.yaml'),
    MAX_POINTER_BYTES,
    'official DSH recovery Profile workspace',
  )).toString('utf8') !== PROFILE_WORKSPACE) {
    failure('official DSH recovery Profile workspace contains unsupported execution controls')
  }
  let manifest: unknown
  try {
    manifest = JSON.parse((await readRegularNoFollow(
      join(profilePath, 'package.json'),
      MAX_STATE_BYTES,
      'official DSH recovery Profile manifest',
    )).toString('utf8')) as unknown
  } catch {
...
async function readInstalledProfileStore(profilePath: string): Promise<string | null> {
  ...
  try {
    metadataBytes = await readRegularNoFollow(
      metadataPath,
      MAX_MODULES_METADATA_BYTES,
      'official DSH recovery Profile pnpm modules metadata',
    )
  } catch (error: unknown) {
...
function decodeMetadataCache(value: unknown, binding: OfficialDshRecoveryBinding, profileId: string): ProfileMetadataCacheBinding {
  const record = strictRecord(value, [
  ...
  const profileManifestSha256 = digest(
    record.profileManifestSha256,
    'Plugin recovery metadata cache profileManifestSha256',
  )
...
async function metadataCacheFiles(root: string): Promise<readonly Readonly<{
  path: string
  sizeBytes: number
  sha256: string
}>[]> {
  const output: Array<Readonly<{ path: string; sizeBytes: number; sha256: string }>> = []
  const visit = async (path: string): Promise<void> => {
    const info = await lstat(path)
    if (info.isSymbolicLink()) failure('Plugin recovery metadata cache contains a symbolic link')
    if (info.isFile()) {
    ...
    if (!info.isDirectory()) failure('Plugin recovery metadata cache contains an unsupported entry')
...
async function verifyMetadataCache(
  ...
  if (fileDigest(bytes) !== binding.manifestSha256) failure('Plugin recovery metadata cache manifest digest changed')
...
async function executionLease(binding: OfficialDshRecoveryBinding, profileId: string, processGroupPid: number): Promise<RecoveryExecutionRecord> {
  ...
  const owner = decodeProfileLockOwner(
    ...
    profileId,
    'official DSH recovery lease owner',
  )
...
function validateJson(value: unknown, label: string, depth = 0, budget = { nodes: 0 }): void {
  budget.nodes += 1
  if (budget.nodes > 4_096 || depth > 16) failure(`${label} exceeds the JSON complexity bound`)
  if (value === null || typeof value === 'string' || typeof value === 'boolean') return
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) failure(`${label} contains a non-finite number`)
    return
  }
  if (Array.isArray(value)) {
    if (value.length > 256) failure(`${label} exceeds the JSON array bound`)
    value.forEach((item, index) => validateJson(item, `${label}[${String(index)}]`, depth + 1, budget))
    return
  }
  if (!isRecord(value) || Object.keys(value).length > 128) failure(`${label} is not bounded JSON`)
  for (const key of Object.keys(value)) {
    if (key.length === 0 || key.length > 128) failure(`${label} contains an invalid key`)
    validateJson(value[key], `${label}.${key}`, depth + 1, budget)
  }
}

function decodePluginKindState(value: unknown, extensionId: string, label: string): void {
  if (!isRecord(value)) failure(`${label} must be an object`)
  ...
  if (hasRollback) boundedString(record.rollbackOperationId, `${label}.rollbackOperationId`, 512)
  digest(record.treeDigest, `${label}.treeDigest`)
  if (phase === 'pending-restart') {
    if (record.consumerObserved || record.restartObserved || hasEvidence) failure(`${label} has contradictory pending evidence`)
  } else {
    if (!record.consumerObserved || !record.restartObserved || !hasEvidence) failure(`${label} has incomplete settled evidence`)
    ...
    if (evidence.fiberPhase !== phase) failure(`${label}.runtimeEvidence does not bind loaderPhase`)
  }
}

function decodeVersion(value: unknown, root: string, targetKey: string, label: string): Record<string, unknown> | null {
  ...
  if (!candidateRef.startsWith('plugin:')) failure(`${label}.candidateRef is not a Plugin reference`)
  boundedString(record.artifactRevision, `${label}.artifactRevision`, 256)
  if (typeof record.artifactIntegrity !== 'string' || !ARTIFACT_INTEGRITY.test(record.artifactIntegrity)) {
    failure(`${label}.artifactIntegrity is invalid`)
  }
  ...
  if (!isAbsolute(materialPath) || materialPath !== expectedPath) failure(`${label}.materialPath does not bind its target and artifact`)
  if (typeof record.enabled !== 'boolean') failure(`${label}.enabled must be boolean`)
  ...
  if (!ownerRevision.startsWith('managed-plugin:')) failure(`${label}.ownerRevision does not bind the Plugin owner`)
  return record
}

function decodeManaged(value: unknown, root: string, expected: Opening, targetKey: string, label: string): ManagedRecord {
  const record = strictRecord(value, [
  ...
  if (record.lastOperationId !== null) boundedString(record.lastOperationId, `${label}.lastOperationId`, 512)
...
function managedStateDigest(record: ManagedRecord | null): string {
  if (record === null) return canonicalDigest(null)
...
function materialPath(record: ManagedRecord, field: 'lastGood' | 'removed'): string | null {
  const version = record[field]
  return version === null ? null : version.materialPath as string
}

function decodeSidecar(value: unknown, root: string, expected: Opening, targetKey: string, label: string): PluginSidecar {
  const record = strictRecord(value, [
...
function decodeProviderSnapshot(value: unknown, root: string, journal: VerifiedJournal): ProviderSnapshot {
  const record = strictRecord(value, [
  ...
  const point = strictRecord(
    ...
    ['artifactPath', 'kind', 'metadataCache', 'snapshot'],
    'Plugin recovery point',
  )
  if (point.kind !== 'plugin') failure('Plugin recovery point is invalid')
  ...
  if (point.metadataCache === null) failure('Plugin recovery point has no metadata cache binding')
...
function decodeMaterialMarker(
  ...
  const files = record.files.map((item, index): MaterialFileEvidence => {
...
async function verifyArtifact(marker: MaterialMarker): Promise<void> {
  ...
  if (match === null) failure('managed Plugin retained artifact integrity is invalid')
  const hexadecimal = /^[0-9a-f]+$/u.test(match[2]!)
  const observed = createHash(match[1]!).update(bytes).digest(hexadecimal ? 'hex' : 'base64')
  if (observed !== match[2]) failure('managed Plugin retained artifact changed after admission')
}
...
async function collectPackageFiles(
  ...
  for (const entry of entries) {
    if (ignoreNodeModules && current === root && entry.name === 'node_modules') continue
    const path = join(current, entry.name)
    const relativePath = relative(root, path).split(sep).join('/')
    if (entry.isSymbolicLink()) failure(`managed Plugin package contains an unexpected symbolic link: ${relativePath}`)
    if (entry.isDirectory()) output.push(...await collectPackageFiles(root, path, ignoreNodeModules))
    else if (entry.isFile()) output.push(relativePath)
    else failure(`managed Plugin package contains an unsupported entry: ${relativePath}`)
  }
  return output
}

async function packageTreeMatches(packageRoot: string, marker: MaterialMarker, ignoreNodeModules: boolean): Promise<boolean> {
  ...
  if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink()) failure('managed Plugin package root is unsafe')
  const paths = await collectPackageFiles(canonicalRoot, canonicalRoot, ignoreNodeModules)
  if (paths.length !== marker.files.length || paths.some((path, index) => path !== marker.files[index]!.path)) return false
  for (const file of marker.files) {
    ...
    if (bytes.length !== file.sizeBytes || fileDigest(bytes) !== file.sha256) return false
  }
  let manifest: unknown
  try {
    manifest = JSON.parse((await readFile(join(canonicalRoot, 'package.json'))).toString('utf8')) as unknown
  } catch {
    return false
  }
  if (!isRecord(manifest) || manifest.name !== marker.packageName || manifest.version !== marker.version
    ...
    || !isRecord(manifest.dsh.bundle) || typeof manifest.dsh.bundle.patch !== 'string') return false
  const declaredPatch = manifest.dsh.bundle.patch
  if (declaredPatch.includes('\\') || declaredPatch.startsWith('/')) return false
...
async function materialMarker(
  root: string,
  targetKey: string,
  packageName: string,
  version: Record<string, unknown>,
): Promise<MaterialMarker> {
  const materialPath = boundedString(version.materialPath, 'managed Plugin version materialPath', 4_096)
  const expectedPath = join(root, 'material', 'plugins', storageKey(targetKey), storageKey(
    boundedString(version.artifactIntegrity, 'managed Plugin version artifactIntegrity', 256),
  ))
  if (materialPath !== expectedPath) failure('managed Plugin version material path is invalid')
  const markerValue = await readOptionalRecord(`${materialPath}.owner.json`, MAX_STATE_BYTES, 'managed Plugin material marker')
  if (markerValue === undefined) failure('managed Plugin material marker is absent')
...
async function readProfileObservation(
  binding: OfficialDshRecoveryBinding,
  profileId: string,
  packageName: string,
): Promise<ProfileObservation> {
  const profilePath = join(binding.hostHome, 'profiles', profileId)
  let profileInfo: Awaited<ReturnType<typeof lstat>>
  try {
  ...
  if (!isRecord(manifest)) failure('official DSH recovery Profile manifest is invalid')
  ...
  return Object.freeze({
    profileExists: true,
    dependency: dependencyValue as string | undefined ?? null,
    bundleCount: Array.isArray(bundles) ? bundles.filter(item => item === packageName).length : 0,
  })
}

function currentVersion(record: ManagedRecord | null): Record<string, unknown> | null {
  return record?.current ?? null
}

async function physicalStatus(
  root: string,
  binding: OfficialDshRecoveryBinding,
  profileId: string,
  packageName: string,
...
async function restoreOfficialProfile(
  ...
  if (restoredStatus === 'exact') return
  const sourceStatus = await physicalStatus(root, binding, profileId, packageName, journal.targetKey, source)
  if (sourceStatus === 'mismatch') failure('official DSH Profile changed outside the recovery operation')
  ...
  if (desired !== null) {
    ...
    if (observed.dependency === null && observed.bundleCount === 1) {
      ...
      if (repair === null) failure('official DSH Profile stale bundle has no retained repair artifact')
...
function statePaths(root: string, journal: VerifiedJournal): Readonly<{
  managed: string
  sidecar: string
  transaction: string
  evidence: string
  absentRollback: string
}> {
  return Object.freeze({
...
function restoredState(source: PluginSidecar, snapshot: ProviderSnapshot, journal: VerifiedJournal, nowMs: number): Readonly<{
  managed: ManagedRecord | null
  sidecar: PluginSidecar | null
}> {
  if (snapshot.before === null) return Object.freeze({ managed: null, sidecar: null })
  if (source.managed.revision < snapshot.before.revision) failure('managed Plugin revision moved behind its provider snapshot')
  const managed = {
    ...snapshot.before,
...
function decodeRestoreEvidence(
  ...
  if (record.restoredRevision !== null) safeInteger(record.restoredRevision, `${label}.restoredRevision`, 1)
  return record as unknown as BreakGlassRestoreEvidence
}

function decodeTransaction(value: unknown, root: string, journal: VerifiedJournal, snapshot: ProviderSnapshot): RecoveryTransaction {
  const record = strictRecord(value, [
  ...
  ], 'Plugin recovery transaction') as RecoveryTransaction
  if (record.schemaVersion !== 1 || record.operationId !== journal.operationId || record.targetKey !== journal.targetKey
  ...
  const sourceSidecar = decodeSidecar(
    record.sourceSidecar,
    root,
    journal.opening,
    journal.targetKey,
    'Plugin recovery transaction.sourceSidecar',
  )
  ...
  if (record.committedAtMs !== null) safeInteger(record.committedAtMs, 'Plugin recovery transaction.committedAtMs')
  ...
  const recoveryEvidence = decodeRestoreEvidence(
    record.recoveryEvidence,
    journal,
    snapshot,
    managed,
    'Plugin recovery transaction evidence',
  )
  if (canonicalDigest(recoveryEvidence) !== digest(
    record.recoveryEvidenceDigest,
    'Plugin recovery transaction evidence digest',
...
function sameProfileLockOwner(left: ProfileLockOwner, right: ProfileLockOwner): boolean {
...
async function assertProfileExecutionDead(
  ...
  if (execution.schemaVersion !== 1 || execution.profileId !== profileId || execution.ownerId !== owner.ownerId
    ...
    || (execution.processGroupPid as number) < 1 || !Number.isSafeInteger(execution.startedAtMs)) {
...
interface ProfileCoordinationPaths {
  readonly locks: string
  readonly takeovers: string
  readonly quarantines: string
  readonly destination: string
  readonly takeover: string
  readonly profileQuarantine: string
}
...
function profileCoordinationPaths(binding: OfficialDshRecoveryBinding, profileId: string): ProfileCoordinationPaths {
...
async function pathStatus(path: string): Promise<'absent' | 'directory' | 'other'> {
  try {
    const info = await lstat(path)
...
async function readProfileTakeover(path: string, profileId: string): Promise<ProfileTakeoverRecord | undefined> {
  const status = await pathStatus(path)
  if (status === 'absent') return undefined
  if (status !== 'directory') failure('managed Plugin profile takeover path is unsafe')
  const value = await readOptionalRecord(join(path, 'record.json'), MAX_POINTER_BYTES, 'Plugin recovery takeover')
  if (value === undefined) failure('managed Plugin profile takeover is incomplete')
  return decodeProfileTakeover(value, profileId, 'Plugin recovery takeover')
}

async function readAnyProfileTakeover(path: string): Promise<ProfileTakeoverRecord | undefined> {
  const status = await pathStatus(path)
  if (status === 'absent') return undefined
  if (status !== 'directory') failure('managed Plugin profile takeover path is unsafe')
  const value = await readOptionalRecord(join(path, 'record.json'), MAX_POINTER_BYTES, 'Plugin recovery takeover')
  if (value === undefined || !isRecord(value)) failure('managed Plugin profile takeover is incomplete')
  const profileId = boundedString(value.profileId, 'Plugin recovery takeover.profileId', 256)
  return decodeProfileTakeover(value, profileId, 'Plugin recovery takeover')
}

async function assertNoProfileTakeover(paths: ProfileCoordinationPaths, profileId: string): Promise<void> {
...
async function removeProfileTakeover(
  paths: ProfileCoordinationPaths,
  record: ProfileTakeoverRecord,
): Promise<void> {
...
async function removeExactProfileTakeoverPath(
  paths: ProfileCoordinationPaths,
  path: string,
  expected: ProfileTakeoverRecord,
): Promise<void> {
...
async function listProfileTakeovers(
  paths: ProfileCoordinationPaths,
  profileId: string,
): Promise<readonly ProfileTakeoverEntry[]> {
  ...
  for (const entry of (await readdir(paths.takeovers, { withFileTypes: true }))
    ...
    if (record !== undefined) output.push(Object.freeze({ path, canonical, record }))
  }
  return Object.freeze(output.sort((left, right) => Number(right.canonical) - Number(left.canonical)))
}

async function removeRetiredProfileTakeovers(paths: ProfileCoordinationPaths, profileId: string): Promise<void> {
  for (const entry of await listProfileTakeovers(paths, profileId)) {
    if (!entry.canonical) await removeExactProfileTakeoverPath(paths, entry.path, entry.record)
  }
}

async function installProfileTakeoverRecord(
  paths: ProfileCoordinationPaths,
...
async function acquireProfileTakeover(
  paths: ProfileCoordinationPaths,
  held: ProfileLockOwner,
  claimantOwnerId: string,
  claimantProcessIdentity: ProcessIdentity,
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    if (existing !== undefined) {
      ...
      await syncDirectory(paths.takeovers)
      try {
        const record = await installProfileTakeoverRecord(
          paths,
          held,
          claimantOwnerId,
          claimantProcessIdentity,
...
async function assertProfileOwnerDead(destination: string, owner: ProfileLockOwner, profileId: string): Promise<void> {
  const status = await processIdentityStatus(owner.processIdentity)
  if (status === 'alive') failure(`managed Plugin profile is busy: ${profileId}`)
  if (status === 'unknown') failure(`managed Plugin profile owner identity cannot be verified: ${profileId}`)
  await assertProfileExecutionDead(destination, owner, profileId)
}

async function installProfileOwner(
  paths: ProfileCoordinationPaths,
  temporary: string,
  owner: ProfileLockOwner,
): Promise<void> {
  ...
  const installed = decodeProfileLockOwner(
    await readOptionalRecord(join(paths.destination, 'owner.json'), MAX_POINTER_BYTES, 'Plugin recovery installed lock'),
    owner.profileId,
    'Plugin recovery installed lock',
  )
  if (!sameProfileLockOwner(installed, owner)) failure(`managed Plugin profile takeover is incomplete: ${owner.profileId}`)
}

async function finishProfileTakeover(
  paths: ProfileCoordinationPaths,
  record: ProfileTakeoverRecord,
): Promise<void> {
  const current = await readProfileTakeover(paths.takeover, record.profileId)
...
async function takeoverInstalledProfileLock(
  paths: ProfileCoordinationPaths,
  profileId: string,
  claimantOwner: ProfileLockOwner,
  temporary: string,
): Promise<void> {
  const held = decodeProfileLockOwner(
    await readOptionalRecord(join(paths.destination, 'owner.json'), MAX_POINTER_BYTES, 'Plugin recovery held lock'),
    profileId,
    'Plugin recovery held lock',
  )
  await assertProfileOwnerDead(paths.destination, held, profileId)
  const takeover = await acquireProfileTakeover(
    paths,
    held,
    claimantOwner.ownerId,
    claimantOwner.processIdentity,
  )
  const current = decodeProfileLockOwner(
    await readOptionalRecord(join(paths.destination, 'owner.json'), MAX_POINTER_BYTES, 'Plugin recovery claimed lock'),
    profileId,
    'Plugin recovery claimed lock',
  )
  if (!sameProfileLockOwner(current, held)) failure(`managed Plugin profile lock changed during stale recovery: ${profileId}`)
  ...
  const moved = decodeProfileLockOwner(
    await readOptionalRecord(join(quarantine, 'owner.json'), MAX_POINTER_BYTES, 'Plugin recovery quarantined lock'),
    profileId,
    'Plugin recovery quarantined lock',
  )
...
async function resumeProfileTakeover(
  paths: ProfileCoordinationPaths,
  profileId: string,
  claimantOwner: ProfileLockOwner,
  temporary: string,
  ...
  if (entries.length === 0) {
    if (quarantineStatus !== 'absent') failure(`managed Plugin profile quarantine requires manual recovery: ${profileId}`)
    return false
  }
  for (const entry of entries) {
  ...
  if (destinationStatus === 'other') failure(`managed Plugin profile lock path is unsafe: ${profileId}`)
  if (destinationStatus === 'directory') {
    await takeoverInstalledProfileLock(
      paths,
      profileId,
      claimantOwner,
      temporary,
  ...
  if (quarantineStatus === 'other') failure(`managed Plugin profile quarantine path is unsafe: ${profileId}`)
  if (quarantineStatus === 'absent') {
    const canonical = entries.find(candidate => candidate.canonical)
    if (canonical !== undefined) await removeProfileTakeover(paths, canonical.record)
    else {
  ...
  const reread = decodeProfileLockOwner(
    await readOptionalRecord(join(quarantine, 'owner.json'), MAX_POINTER_BYTES, 'Plugin recovery quarantined lock'),
    profileId,
    'Plugin recovery quarantined lock',
  )
  if (!sameProfileLockOwner(reread, held)) failure(`managed Plugin profile quarantine changed during recovery: ${profileId}`)
  await assertProfileOwnerDead(quarantine, reread, profileId)
  const finalOwner = decodeProfileLockOwner(
    await readOptionalRecord(join(quarantine, 'owner.json'), MAX_POINTER_BYTES, 'Plugin recovery quarantined lock'),
    profileId,
    'Plugin recovery quarantined lock',
  )
  if (!sameProfileLockOwner(finalOwner, reread)) failure(`managed Plugin profile quarantine changed during recovery: ${profileId}`)
...
async function acquireProfileLock(binding: OfficialDshRecoveryBinding, profileId: string): Promise<() => Promise<void>> {
  ...
  const owner: ProfileLockOwner = Object.freeze({
    schemaVersion: 2,
    profileId,
  ...
  try {
    ...
    try {
      ...
      const installedValue = await readOptionalRecord(
        join(paths.destination, 'owner.json'),
        MAX_POINTER_BYTES,
        'Plugin recovery raced lock',
      )
...
async function stateDigest(path: string, label: string): Promise<string | null> {
  const value = await readOptionalRecord(path, MAX_STATE_BYTES, label)
  return value === undefined ? null : canonicalDigest(value)
}

async function verifyFinalState(paths: ReturnType<typeof statePaths>, transaction: RecoveryTransaction): Promise<void> {
...
async function writeBoundRecord(path: string, value: unknown, label: string): Promise<void> {
  const existing = await readOptionalRecord(path, MAX_STATE_BYTES, label)
  if (existing !== undefined) {
    if (canonicalJson(existing) !== canonicalJson(value)) failure(`${label} conflicts with existing recovery evidence`)
    return
  }
  try {
    await writeExclusive(path, value)
    ...
    if (canonicalJson(raced) !== canonicalJson(value)) failure(`${label} conflicts with raced recovery evidence`)
  }
}

function absentRollbackEvidence(transaction: RecoveryTransaction): Record<string, unknown> | null {
  if (transaction.restoredManaged !== null) return null
...
async function publishRecoveryEvidence(
  ...
  if (absent !== null) await writeBoundRecord(paths.absentRollback, absent, 'Plugin absent rollback evidence')
}
...
async function clearBoundQuarantine(
  root: string,
  binding: OfficialDshRecoveryBinding,
  journal: VerifiedJournal,
): Promise<void> {
...
async function applyPreparedTransaction(paths: ReturnType<typeof statePaths>, transaction: RecoveryTransaction): Promise<void> {
  ...
  if (!managedKnown || !sidecarKnown) failure('managed Plugin current state changed after recovery was prepared')
  if (transaction.restoredSidecar === null) await removeRegular(paths.sidecar, 'managed Plugin sidecar')
  else await writeAtomic(paths.sidecar, transaction.restoredSidecar)
  if (transaction.restoredManaged === null) await removeRegular(paths.managed, 'managed Plugin state')
  else await writeAtomic(paths.managed, transaction.restoredManaged)
}

async function loadProviderSnapshot(root: string, journal: VerifiedJournal): Promise<ProviderSnapshot> {
  ...
  if (value === undefined) failure('Plugin provider snapshot is absent')
  return decodeProviderSnapshot(value, root, journal)
}

async function loadCurrentSidecar(root: string, journal: VerifiedJournal, paths: ReturnType<typeof statePaths>): Promise<Readonly<{
  managedValue: unknown | undefined
  ...
  if (sidecarValue === undefined) failure('managed Plugin current sidecar is absent')
  ...
  if (managedValue !== undefined) {
    ...
    if (!same && !sidecarAhead) failure('managed Plugin state and owner sidecar diverged')
    if (same && sidecar.revision !== managed.revision) failure('managed Plugin sidecar revision is invalid')
  }
 ...
 */
export async function recoverProfile(centerRoot: string, operationId: string, invokedPath: string): Promise<void> {
  const root = await verifyCenterRoot(centerRoot)
  const journal = await verifyJournal(root, operationId)
  if (journal.phase !== 'recovery-required') failure('operation is not a recovery-required Plugin operation')
  const pins = journal.opening.recoveryExecutable
  if (pins.centerRoot !== root) failure('journal recovery executable is bound to a different Center root')
  ...
  if (pins.platform !== process.platform || pins.arch !== process.arch) failure('recovery executable platform does not match this process')
  ...
  if (pinnedSelf !== modulePath || invokedRealpath !== modulePath) failure('running recovery executable does not match the journal pin')
  ...
  if (existingTransactionValue !== undefined) {
    ...
    transaction = {
      ...
      journalHeadDigest: journal.headDigest,
      providerSnapshotDigest: snapshot.digest,
      sourceManaged: source.managedValue === undefined ? null : source.managedValue as ManagedRecord,
  ...
  if (transaction.status === 'committed') {
    ...
    if (await physicalStatus(
      root,
      pins.officialDsh,
      journal.opening.profileId,
      journal.opening.extensionId,
      journal.targetKey,
      transaction.restoredManaged,
    ) !== 'exact') {
  ...
  try {
    await restoreOfficialProfile(
      root,
      pins.officialDsh,
      journal,
      snapshot.metadataCache,
      transaction.sourceSidecar.managed,
      transaction.restoredManaged,
    )
