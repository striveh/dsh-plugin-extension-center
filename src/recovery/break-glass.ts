...
interface Opening {
  ...
  readonly extensionKind: 'plugin'
  readonly recoveryExecutable: RecoveryExecutableBinding
  readonly recoveryTarget: GenerationSummary
}
...
async function readRegularNoFollow(path: string, maximumBytes: number, label: string): Promise<Buffer> {
  ...
  try {
    ...
    if (!opened.isFile()) failure(`${label} must be a regular file`)
    if (opened.size <= 0 || opened.size > maximumBytes) failure(`${label} has an invalid byte length`)
...
function decodeRuntimeBinding(value: unknown, label: string): void {
...
function decodePlanEvidence(value: unknown, label: string): Readonly<{
  ...
  recoveryTarget: GenerationSummary
}> {
...
function stringArray(value: unknown, label: string): void {
  if (!Array.isArray(value) || value.some(item => typeof item !== 'string')) failure(`${label} must be a string array`)
}

function decodePluginReviewRecovery(value: unknown, label: string): GenerationSummary {
  const record = strictRecord(value, [
  ...
  if (record.schemaVersion !== 1 || record.kind !== 'plugin') failure(`${label} is not Plugin review evidence`)
  ...
  for (const field of ['checks', 'removed', 'retained', 'dependencies', 'bundles'] as const) {
    if (!Array.isArray(record[field])) failure(`${label}.${field} must be an array`)
  }
  ...
  if (manifest.beforeVersion !== null) boundedString(manifest.beforeVersion, `${label}.manifest.beforeVersion`)
  if (manifest.afterVersion !== null) boundedString(manifest.afterVersion, `${label}.manifest.afterVersion`)
  ...
  if (lockfile.path !== 'pnpm-lock.yaml') failure(`${label}.lockfile.path is unsupported`)
  ...
  if (point.kind !== 'profile-generation') failure(`${label}.rollbackPoint is not a recoverable Profile generation`)
  const generation = decodeGeneration(point.id, `${label}.rollbackPoint.id`)
  if (generation === null) failure(`${label}.rollbackPoint.id cannot be null`)
...
function decodeOpening(value: unknown): Opening {
  const record = strictRecord(value, [
    'type',
    'planId',
    'planHash',
  ...
  if (record.managedObject !== 'artifact') failure('break-glass recovery supports only Plugin artifact operations')
  ...
  if (record.runtimeBinding !== null) failure('Plugin artifact recovery cannot carry a runtime binding')
...
function decodeReceipt(value: unknown, context: Readonly<{
  operationId: string
  targetKey: string
  phase: Phase
  sequence: number
  ...
  if (body.schemaVersion !== RECORD_SCHEMA_VERSION) failure('journal receipt schemaVersion is unsupported')
  ...
  for (const field of ['mutationDigests', 'verificationDigests'] as const) {
    if (!Array.isArray(body[field])) failure(`journal receipt body.${field} must be an array`)
  ...
  if (canonicalDigest(body) !== receiptDigest) failure('journal receipt digest does not match its body')
}
...
async function verifyJournal(centerRoot: string, operationId: string): Promise<VerifiedJournal> {
  ...
  if (!(await lstat(root)).isDirectory()) failure('Center root must be a directory')
...
async function verifyExecutable(path: string, expectedDigest: string, label: string): Promise<string> {
  ...
  if (pinnedRealpath !== path) failure(`${label} path is not its canonical realpath`)
...
function decodeGeneration(value: unknown, label: string): string | null {
  if (value === null) return null
  if (typeof value !== 'string' || !GENERATION.test(value)) failure(`${label} is not an exact Profile generation id`)
  return value
}

function decodeSummary(value: unknown, label: string): GenerationSummary {
  ...
  if (generation === null) failure(`${label}.generation cannot be null`)
  const treeDigest = digest(record.treeDigest, `${label}.treeDigest`)
  if (record.mutationId !== null) boundedString(record.mutationId, `${label}.mutationId`, 128)
  if (record.mutation !== null && !isRecord(record.mutation)) failure(`${label}.mutation must be an object or null`)
  return Object.freeze({ generation, treeDigest })
}

function parseHostJson(text: string, label: string): unknown {
  if (Buffer.byteLength(text) > MAX_HOST_OUTPUT_BYTES || text.includes('\0')) failure(`${label} output is invalid`)
  try {
...
function decodeInventory(text: string, profileId: string): ProfileInventory {
  ...
  const allGenerations = [active, ...staged, ...recoverable].filter((value): value is GenerationSummary => value !== null)
...
function recoveryTarget(inventory: ProfileInventory, pinned: GenerationSummary): GenerationSummary {
  ...
  if (target === null || target !== pinned.generation) failure('Host Profile current recovery selector drifted from the journal pin')
