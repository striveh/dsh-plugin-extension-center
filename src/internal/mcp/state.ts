import { canonicalJson, canonicalSha256, immutableJsonClone, isSha256Digest } from '../../domain/index.ts'
import type {
  CenterMcpConnectionDesired,
  CenterMcpConnectionMutationReceipt,
  CenterMcpConnectionOperation,
} from './types.ts'
import { parseCenterMcpConnectionDesired } from './types.ts'
...
export function emptyCenterMcpState(): StoredCenterMcpState {
  return { schemaVersion: 1, revision: 0, connections: [], removed: [], mutations: [] }
}

/** Clone one validated state for mutation. */
export function cloneCenterMcpState(value: StoredCenterMcpState): StoredCenterMcpState {
  return JSON.parse(canonicalJson(value)) as StoredCenterMcpState
}

/** Canonically sort all identity-indexed records before persistence. */
export function normalizeCenterMcpState(value: StoredCenterMcpState): void {
...
export function envelopeForCenterMcpState(value: StoredCenterMcpState): StoredCenterMcpEnvelope {
...
export function parseCenterMcpEnvelope(value: unknown): StoredCenterMcpState {
  const envelope = exactObject(value, ['schemaVersion', 'state', 'digest'], '$')
  if (envelope.schemaVersion !== 1) throw new Error('Center MCP state envelope version is unsupported')
  if (!isSha256Digest(envelope.digest)) throw new Error('Center MCP state envelope digest is invalid')
  const state = parseState(envelope.state)
  if (canonicalSha256(state) !== envelope.digest) throw new Error('Center MCP state envelope digest does not match its state')
  return state
}

/** Return a recursively frozen detached public value. */
export function immutableCenterMcpValue<T>(value: T): Readonly<T> {
  return immutableJsonClone(value)
}

function parseState(value: unknown): StoredCenterMcpState {
  const object = exactObject(value, ['schemaVersion', 'revision', 'connections', 'removed', 'mutations'], '$.state')
  if (object.schemaVersion !== 1) throw new Error('Center MCP desired-state version is unsupported')
  ...
  for (const record of [...connections, ...removed]) {
    if (ids.has(record.desired.id)) throw new Error(`Center MCP state contains duplicate connection "${record.desired.id}"`)
    ids.add(record.desired.id)
    if (record.revision > revision) throw new Error(`Center MCP connection "${record.desired.id}" exceeds the global revision`)
  }
  for (const record of removed) {
    if (record.removedAtRevision > revision) throw new Error(`Center MCP removal "${record.desired.id}" exceeds the global revision`)
  }
  const mutationIds = new Set<string>()
  for (const mutation of mutations) {
    if (mutationIds.has(mutation.mutationId)) throw new Error(`Center MCP state contains duplicate mutation "${mutation.mutationId}"`)
    mutationIds.add(mutation.mutationId)
    if (mutation.receipt.snapshotRevision > revision) throw new Error(`Center MCP mutation "${mutation.mutationId}" exceeds the global revision`)
  }
  return { schemaVersion: 1, revision, connections, removed, mutations }
}

function parseMutation(value: unknown, index: number): StoredCenterMcpMutation {
  const path = `$.state.mutations[${String(index)}]`
  const object = exactObject(value, ['mutationId', 'requestDigest', 'receipt'], path)
  if (typeof object.mutationId !== 'string') throw new TypeError(`${path}.mutationId must be a string`)
  if (!isSha256Digest(object.requestDigest)) throw new TypeError(`${path}.requestDigest must be a SHA-256 digest`)
  const receipt = parseReceipt(object.receipt, `${path}.receipt`)
  if (receipt.mutationId !== object.mutationId) throw new Error(`${path}.receipt mutation id does not match its entry`)
  return { mutationId: object.mutationId, requestDigest: object.requestDigest, receipt }
}

function parseReceipt(value: unknown, path: string): CenterMcpConnectionMutationReceipt {
  ...
  const object = exactObject(
    ...
    hasPrevious
      ...
      : ['mutationId', 'operation', 'id', 'revision', 'snapshotRevision', 'changed', 'desiredDigest'],
    path,
  )
  if (typeof object.mutationId !== 'string' || typeof object.id !== 'string') {
    throw new TypeError(`${path} has invalid identity fields`)
  }
  if (!isOperation(object.operation)) throw new TypeError(`${path}.operation is unsupported`)
  if (typeof object.changed !== 'boolean') throw new TypeError(`${path}.changed must be a boolean`)
...
function isOperation(value: unknown): value is CenterMcpConnectionOperation {
