...
export const OFFICIAL_PROFILE_AMBIGUITY_CODE = 'profile-state-ambiguous' as const
...
export class OfficialProfileAmbiguityError extends Error {
  readonly code = OFFICIAL_PROFILE_AMBIGUITY_CODE
...
export function isOfficialProfileAmbiguityError(error: unknown): error is OfficialProfileAmbiguityError {
  return error instanceof OfficialProfileAmbiguityError
    || typeof error === 'object' && error !== null
      && (error as Readonly<{ code?: unknown }>).code === OFFICIAL_PROFILE_AMBIGUITY_CODE
}
...
export interface ManagedPluginLoader {
  update(
    id: string,
    options: Readonly<{ config?: unknown; group?: boolean | null; disabled?: boolean | null; inject?: unknown }>,
    parent?: string | null,
    position?: number,
  ): Promise<void>
  await(): Promise<void>
  entries(): Iterable<Readonly<{
    id: string
    options: Readonly<{ id?: string; name: string; config?: unknown; group?: boolean }>
    disabled: boolean
...
export type ManagedPluginActivationResolver = (
  targetKey: string,
  packageName: string,
  profileId: string,
