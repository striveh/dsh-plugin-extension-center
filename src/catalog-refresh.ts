...
function record(value: unknown, subject: string, fields: readonly string[]): Record<string, unknown> {
...
function decodeSignedDocument(value: unknown): SignedCatalogDocument {
  ...
  return Object.freeze({
    envelope: envelope as unknown as CatalogEnvelope,
    signatures: Object.freeze(decodedSignatures),
  })
}

function decodeCache(value: unknown): CatalogCache {
  ...
  if (input.schemaVersion !== CACHE_SCHEMA_VERSION
    ...
    || !Number.isSafeInteger(input.acceptedAtMs) || (input.acceptedAtMs as number) < 0
  ...
  return Object.freeze({
    ...
    acceptedAtMs: input.acceptedAtMs as number,
  })
}

function sameEnvelope(left: CatalogEnvelope, right: CatalogEnvelope): boolean {
  return canonicalSha256(left) === canonicalSha256(right)
}

/** Verify one same-or-next revision without allowing rollback, gaps, or a broken predecessor link. */
export function verifyCatalogAdvance(
  root: CatalogRoot,
  current: VerifiedCatalog,
  document: SignedCatalogDocument,
  now = Date.now(),
): VerifiedCatalog {
  const next = verifyCatalog(root, document.envelope, document.signatures, now)
  if (next.envelope.revision < current.envelope.revision) throw new Error('catalog revision rollback is forbidden')
...
function bootstrapDocument(): SignedCatalogDocument {
  return Object.freeze({
...
async function readCache(path: string): Promise<CatalogCache | undefined> {
  ...
  try {
    ...
    if (info.size > MAX_CACHE_BYTES) throw new Error('catalog last-good cache exceeds its bound')
    const text = await handle.readFile('utf8')
    if (!text.endsWith('\n') || text.slice(0, -1).includes('\n')) {
      throw new Error('catalog last-good cache is incomplete')
    }
    const value: unknown = JSON.parse(text)
    if (`${canonicalJson(value)}\n` !== text) throw new Error('catalog last-good cache is not canonical')
...
async function replaceCache(path: string, value: CatalogCache): Promise<void> {
  const directory = dirname(path)
  await ensurePrivateDirectory(directory)
  try {
    const info = await lstat(path)
    if (!info.isFile() || info.isSymbolicLink()) throw new Error('catalog last-good cache is not a regular file')
  } catch (error: unknown) {
...
async function responseBytes(response: Response): Promise<Uint8Array> {
  if (response.status !== 200) throw new Error(`catalog endpoint returned HTTP ${String(response.status)}`)
  const contentType = response.headers.get('content-type')?.split(';', 1)[0]?.trim().toLowerCase()
  if (contentType !== 'application/json') throw new Error('catalog endpoint did not return application/json')
...
function parseDocument(bytes: Uint8Array): SignedCatalogDocument {
  let value: unknown
  try {
...
export class CatalogSnapshotManager {
  private snapshot: AdmittedCatalogSnapshot | null = null
  private cache: CatalogCache | null = null
  private refreshing: Promise<AdmittedCatalogSnapshot> | null = null

  constructor(
    private readonly root: string,
    private readonly config: CatalogRefreshConfig,
    private readonly dependencies: CatalogRefreshDependencies = {
  ...
  async initialize(): Promise<AdmittedCatalogSnapshot> {
  ...
  current(): AdmittedCatalogSnapshot {
    if (this.snapshot === null) throw new Error('catalog snapshot manager is not initialized')
    verifyCatalog(
      BOOTSTRAP_CATALOG_ROOT,
      this.snapshot.catalog.envelope,
      this.cache?.chain.at(-1)?.signatures ?? BOOTSTRAP_CATALOG_SIGNATURES,
  ...
  private async refreshOnce(): Promise<AdmittedCatalogSnapshot> {
    ...
    try {
      ...
      if (chain.length > MAX_CHAIN_LENGTH) throw new Error('catalog revision chain exceeds the packaged bound')
      const cache: CatalogCache = Object.freeze({
        schemaVersion: CACHE_SCHEMA_VERSION,
        catalogId: BOOTSTRAP_CATALOG_ROOT.catalogId,
