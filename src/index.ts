...
import { createHostRpcHandler } from './service/rpc-service.ts'
...
export async function apply(ctx: Context, config: Config = {}): Promise<void> {
  ...
  const owners: HostOwners = probeHostOwners(host, await loadHostOwnerDefinitions())
  ...
  let acquisition: CapabilityAcquisitionService | null = null
  if (capabilities.acquisition) {
    await runner.recover(new AbortController().signal)
    const readyAcquisition = new CapabilityAcquisitionService(
      state,
      inventory,
      intentPlans,
      plans,
