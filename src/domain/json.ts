...
function canonicalize(value: unknown, path: string, ancestors: Set<object>): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) failDomain('invalid-data', `${path} contains a non-finite number`)
    return JSON.stringify(value)
  }
  if (Array.isArray(value)) {
    if (ancestors.has(value)) failDomain('invalid-data', `${path} contains a cycle`)
    const ownKeys = Reflect.ownKeys(value)
    const permittedKeys = new Set<PropertyKey>(['length', ...value.map((_, index) => String(index))])
  ...
  if (typeof value === 'object') {
    ...
    if (ancestors.has(object)) failDomain('invalid-data', `${path} contains a cycle`)
    const ownKeys = Reflect.ownKeys(object)
    if (ownKeys.some(key => typeof key === 'symbol')) {
      failDomain('invalid-data', `${path} contains a symbol key`)
    }
    const propertyNames = ownKeys as string[]
    ...
    for (const key of propertyNames) {
      ...
      if (descriptor.value === undefined) failDomain('invalid-data', `${path}.${key} is undefined`)
    }
 ...
 */
export function immutableJsonClone<T>(value: T): Readonly<T> {
  const clone = JSON.parse(canonicalJson(value)) as unknown
  return freezeRecursively(clone) as Readonly<T>
}
...
export function isArtifactIntegrity(value: unknown): value is ArtifactIntegrity {
  ...
  if (/^sha256:[0-9a-f]{64}$/.test(value) || /^sha512:[0-9a-f]{128}$/.test(value)) return true
