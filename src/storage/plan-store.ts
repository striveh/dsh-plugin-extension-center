...
function planDirectoryName(hash: Sha256Digest): string {
...
async function writeSingleAssignment(path: string, content: string): Promise<void> {
...
function parseCanonical(text: string, subject: string): unknown {
...
function replayFailure(subject: string): ExtensionDomainError {
...
export class FilePlanStore {
  private readonly queues = new Map<Sha256Digest, Promise<void>>()
  ...
  constructor(
    ...
    private readonly recoveryExecutable: RecoveryExecutableBinding | null,
  ) {}
  ...
  decide(
    ...
    context: PlanUseContext,
    nowMs: number,
  ): Promise<PlanAuthorizationState> {
    return this.serialize(hash, async () => {
      const state = await this.requireUnlocked(hash)
      if (state.status !== 'pending') throw replayFailure(`plan ${hash} decision`)
      const next = decidePlan(state, decision, context, nowMs).state
      try {
        await writeSingleAssignment(
          join(this.planDirectory(hash), DECISION_FILENAME),
          `${canonicalJson(next)}\n`,
        )
      } catch (error: unknown) {
        if ((error as NodeJS.ErrnoException).code === 'EEXIST') throw replayFailure(`plan ${hash} decision`)
        throw error
      }
  ...
  consume(
    ...
    context: PlanUseContext,
    nowMs: number,
  ): Promise<Readonly<{ state: PlanAuthorizationState; authorization: OperationAuthorization }>> {
    return this.serialize(hash, async () => {
      const state = await this.requireUnlocked(hash)
      if (state.status !== 'approved') throw replayFailure(`plan ${hash} consumption`)
      ...
      try {
        await writeSingleAssignment(
          join(this.planDirectory(hash), CONSUMPTION_FILENAME),
          `${canonicalJson(next.state)}\n`,
        )
      } catch (error: unknown) {
        if ((error as NodeJS.ErrnoException).code === 'EEXIST') throw replayFailure(`plan ${hash} consumption`)
        throw error
      }
  ...
  async list(): Promise<readonly PlanAuthorizationState[]> {
    ...
    const states: PlanAuthorizationState[] = []
    for (const entry of entries) {
      if (!entry.isDirectory() || !/^[0-9a-f]{64}$/.test(entry.name)) continue
      const state = await this.load(`sha256:${entry.name}` as Sha256Digest)
  ...
  private planDirectory(hash: Sha256Digest): string {
    return join(this.root, 'plans', planDirectoryName(hash))
  }

  private async requireUnlocked(hash: Sha256Digest): Promise<PlanAuthorizationState> {
    const state = await this.loadUnlocked(hash)
    if (state === undefined) throw new ExtensionDomainError('invalid-data', `plan ${hash} does not exist`)
    return state
  }

  private async loadUnlocked(hash: Sha256Digest): Promise<PlanAuthorizationState | undefined> {
    ...
    if (plan.hash !== hash) throw new ExtensionDomainError('plan-integrity', `plan ${hash} hash does not match its path`)
  ...
  private serialize<T>(hash: Sha256Digest, action: () => Promise<T>): Promise<T> {
    const prior = this.queues.get(hash) ?? Promise.resolve()
    const run = prior.then(action, action)
    const settled = run.then(() => undefined, () => undefined)
    this.queues.set(hash, settled)
    void settled.then(() => {
      if (this.queues.get(hash) === settled) this.queues.delete(hash)
    })
    return run
  }
}
