...
export type TaskAttemptResult =
  | null
  | Readonly<{ kind: 'use-existing'; capabilityId: string }>
  | Readonly<{ kind: 'choice-required'; candidateRefs: readonly string[] }>
  | Readonly<{
...
export type TaskRetryContinuationState =
  | 'pending'
  | 'ready'
  | 'consumed'
  | 'dispatching'
...
export type TaskRetryContinuationProjectionState =
...
export interface TaskAttemptProjection {
  ...
  readonly outcome: TaskAttemptOutcome | null
  readonly reason: string | null
  readonly choice: Readonly<{ candidateRefs: readonly string[] }> | null
