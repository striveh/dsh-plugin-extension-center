...
import type { HostOwners, ManagedTargetRecord, StoredIntent } from '../host/index.ts'
...
import {
  ...
  preflightSkillConfiguration,
} from '../providers/index.ts'
import { FilePlanStore } from '../storage/index.ts'
import { admitCenterManagement, admitInstallTarget } from './management-admission.ts'
...
export class IntentPolicyDeniedError extends Error {
  constructor(readonly policy: Extract<CandidatePolicyResult, { status: 'denied' }>) {
    super(policy.reason)
...
export function resolveDesiredState(
  ...
  record: ManagedTargetRecord | undefined,
): DesiredState {
  if (operation === 'uninstall' || operation === 'purge') return 'removed'
  if (operation === 'disable') return 'disabled'
  if (operation === 'enable') return 'enabled'
  if (operation === 'install') return origin === 'store' && kind === 'mcp' ? 'disabled' : 'enabled'
...
function authorityDelta(entry: CatalogEntry, configuration: RpcJson, runtimeDescriptorDigest: string | null): Sha256Digest {
...
export class IntentPlanService {
  ...
  async preview(
    ...
    if (entry === undefined) throw new Error('candidateRef is absent from the current verified catalog')
    ...
    if (request.origin === 'task') await this.assertTaskBinding(request, entry, taskBinding)
    else if (taskBinding !== null) throw new Error('Store intent cannot carry a task binding')
    ...
    const skillProjectRoot = entry.kind === 'skill' && request.scopeKey === 'project'
      ? String((payloadConfiguration as Readonly<Record<string, RpcJson>>).projectRoot)
      : null
    ...
    if (management) {
      ...
      if (admitted.status === 'denied') throw new IntentPolicyDeniedError(admitted.policy)
      ...
      if (request.operationKind !== 'install') {
        ...
        if (denied !== null) throw new IntentPolicyDeniedError(denied)
      }
    ...
    const runtime = entry.kind === 'mcp'
      ? await this.preflight.mcpRuntime(entry.candidateRef, payloadConfiguration)
      : null
    if (entry.kind === 'mcp' && runtime === null) throw new Error('MCP runtime preflight changed while creating the plan')
  ...
  async context(plan: ImmutablePlan): Promise<PlanUseContext> {
    ...
    if (stored === undefined || stored.planHash !== plan.hash) throw new Error('plan has no exact durable intent binding')
    ...
    const skillProjectRoot = entry.kind === 'skill' && plan.content.scopeKey === 'project'
      ? String((stored.payload.configuration as Readonly<Record<string, RpcJson>>).projectRoot)
      : null
  ...
  async listTaskApprovals(maximum = 128): Promise<readonly TaskApprovalRow[]> {
    const output: TaskApprovalRow[] = []
    for (const state of await this.plans.list()) {
      if (state.plan.content.origin !== 'task' || !['pending', 'approved'].includes(state.status)) continue
      ...
      output.push(Object.freeze({
        state: state as Extract<typeof state, { status: 'pending' | 'approved' }>,
        configuration: intent.payload.configuration,
      }))
      if (output.length > maximum) throw new Error('task approval queue exceeds its P0 bound')
    }
  ...
  private async assertTaskBinding(
    request: IntentPreviewRequest,
    entry: CatalogEntry,
    binding: TaskIntentBinding | null,
  ): Promise<void> {
    if (binding === null || request.continuationId === null) throw new Error('task intent has no Host continuation binding')
    ...
    const value = typeof resolution.value === 'object' && resolution.value !== null && !Array.isArray(resolution.value)
      ? resolution.value as Readonly<Record<string, RpcJson>>
      : undefined
