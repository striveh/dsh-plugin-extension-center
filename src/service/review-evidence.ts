...
function bytesDigest(value: string): Sha256Digest {
...
async function managedBody(version: ManagedVersion | null | undefined): Promise<string | null> {
  ...
  try {
    ...
    if (bytes.length > 1024 * 1024) throw new Error('managed Skill review body exceeds the P0 bound')
...
function currentForOperation(record: ManagedTargetRecord | undefined, operation: OperationKind): ManagedVersion | null {
  if (operation === 'restore') return record?.removed ?? record?.lastGood ?? null
  return record?.current ?? null
}

function invocation(version: ManagedVersion | null): Readonly<{ model: boolean | null; user: boolean | null }> {
...
function check(code: PlannedReviewCheck['code'], phase: PlannedReviewCheck['phase']): PlannedReviewCheck {
...
): Readonly<{ removed: readonly ReviewMaterial[]; retained: readonly ReviewMaterial[] }> {
...
function rollbackPoint(record: ManagedTargetRecord | undefined): ReviewRollbackPoint {
...
function configInvocation(value: RpcJson): Readonly<{ model: boolean; user: boolean }> {
...
async function skillEvidence(input: BuildReviewEvidenceInput): Promise<SkillReviewEvidence> {
  ...
  }) as SkillReviewEvidence
}

function mcpEvidence(input: BuildReviewEvidenceInput): McpReviewEvidence {
  if (catalogReviewEvidenceSupport(input.entry) !== 'runtime-bound') throw new Error('MCP has no runtime-bound review recipe')
  if (input.runtime === null) throw new Error('MCP review requires an exact runtime preflight')
...
export async function buildPlanReviewEvidence(input: BuildReviewEvidenceInput): Promise<PlanReviewEvidence> {
