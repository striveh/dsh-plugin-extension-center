...
import type {
  McpHttpReviewDescriptor,
  McpStdioReviewDescriptor,
  OperationKind,
  PlanReviewEvidence,
  PlannedReviewCheck,
...
const CHECK_PHASES = ['planning', 'prepare', 'apply', 'verify', 'external-restart'] as const
...
function exactText(value: unknown, path: string): string | null {
...
function exactArgument(value: unknown, path: string, maximum = 4_096): string {
...
function array(value: unknown, path: string, maximum = 512): readonly unknown[] {
  if (!Array.isArray(value) || value.length > maximum) failDomain('invalid-data', `${path} must be a bounded array`)
  return value
}

function literalArray<T extends string>(value: unknown, accepted: readonly T[], path: string): readonly T[] {
  const output = array(value, path).map((item, index) => readLiteral(item, accepted, `${path}[${String(index)}]`))
  if (new Set(output).size !== output.length) failDomain('invalid-data', `${path} contains duplicates`)
  return Object.freeze(output)
}

function stringArray(value: unknown, path: string, maximum = 512): readonly string[] {
  const output = array(value, path, maximum).map((item, index) => readBoundedString(item, `${path}[${String(index)}]`, 512))
  if (new Set(output).size !== output.length) failDomain('invalid-data', `${path} contains duplicates`)
  return Object.freeze(output)
}

function checks(value: unknown, path: string): readonly PlannedReviewCheck[] {
...
function materials(value: unknown, path: string): readonly ReviewMaterial[] {
...
function rollbackPoint(value: unknown, path: string): ReviewRollbackPoint | null {
  if (value === null) return null
  const record = readStrictRecord(value, ['digest', 'id', 'kind'], path)
  return Object.freeze({
    kind: readLiteral(record.kind, ['absent-state', 'managed-version', 'profile-generation'], `${path}.kind`),
    id: readBoundedString(record.id, `${path}.id`, 512),
    digest: readSha256Digest(record.digest, `${path}.digest`),
  })
}

function common(value: unknown, path: string): Readonly<{
  schemaVersion: 1
  operationKind: OperationKind
  checks: readonly PlannedReviewCheck[]
  removed: readonly ReviewMaterial[]
  retained: readonly ReviewMaterial[]
  credentialChoice: PlanReviewEvidence['credentialChoice']
  rollbackPoint: ReviewRollbackPoint | null
  rollbackLimits: PlanReviewEvidence['rollbackLimits']
  notProven: PlanReviewEvidence['notProven']
}> {
  const record = value as Readonly<Record<string, unknown>>
  if (record.schemaVersion !== 1) failDomain('invalid-data', `${path}.schemaVersion is unsupported`)
  return Object.freeze({
    schemaVersion: 1,
    ...
    credentialChoice: readLiteral(
      record.credentialChoice,
      ['not-applicable', 'retain-local-record', 'delete-local-record'],
      `${path}.credentialChoice`,
    ),
...
function dependencies(value: unknown, path: string): readonly PluginDependencyChange[] {
  const output = array(value, path, 256).map((item, index) => {
    ...
    return Object.freeze({
      kind: readLiteral(record.kind, ['profile', 'host', 'runtime', 'extension', 'peer'], `${at}.kind`),
...
function fileChanges(value: unknown, path: string): readonly SkillFileChange[] {
...
function reconnect(value: unknown, path: string): McpStdioReviewDescriptor['reconnect'] {
...
function descriptor(value: unknown, path: string): McpStdioReviewDescriptor | McpHttpReviewDescriptor {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) failDomain('invalid-data', `${path} must be an object`)
  const transport = readLiteral((value as Record<string, unknown>).transport, ['stdio', 'http'], `${path}.transport`)
  if (transport === 'stdio') {
...
export function decodePlanReviewEvidence(value: unknown, path = 'plan.content.reviewEvidence'): PlanReviewEvidence {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) failDomain('invalid-data', `${path} must be an object`)
  const kind = readLiteral((value as Record<string, unknown>).kind, ['plugin', 'mcp', 'skill'], `${path}.kind`)
  const baseFields = [
  ...
  if (kind === 'plugin') {
    const record = readStrictRecord(value, [
      ...baseFields, 'manifest', 'dependencies', 'lockfile', 'bundles', 'scripts', 'settings',
    ], path)
    ...
    const lockfile = readStrictRecord(record.lockfile, [
      'afterVersion', 'beforeDigest', 'beforeVersion', 'packageName', 'path', 'targetIntegrity',
    ], `${path}.lockfile`)
    if (lockfile.path !== 'pnpm-lock.yaml') failDomain('invalid-data', `${path}.lockfile.path is unsupported`)
    const bundles = array(record.bundles, `${path}.bundles`, 64).map((item, index) => {
      const at = `${path}.bundles[${String(index)}]`
      const row = readStrictRecord(item, ['action', 'id', 'patchBody', 'patchDigest'], at)
      return Object.freeze({
        id: readBoundedString(row.id, `${at}.id`, 256),
        action: readLiteral(row.action, ['add', 'retain', 'remove', 'restore'], `${at}.action`),
        patchDigest: readSha256Digest(row.patchDigest, `${at}.patchDigest`),
        patchBody: exactText(row.patchBody, `${at}.patchBody`) ?? failDomain('invalid-data', `${at}.patchBody cannot be null`),
      })
    })
    if (new Set(bundles.map(item => item.id)).size !== bundles.length) failDomain('invalid-data', `${path}.bundles contains duplicates`)
    ...
    const schema = array(settings.schema, `${path}.settings.schema`, 128).map((item, index) => {
      ...
      if (field.type !== 'integer') failDomain('invalid-data', `${at}.type is unsupported`)
      ...
      if (minimum > maximum) failDomain('invalid-data', `${at} range is invalid`)
      return Object.freeze({
    ...
    if (new Set(schema.map(field => field.field)).size !== schema.length) failDomain('invalid-data', `${path}.settings.schema contains duplicates`)
    const decoded = {
      ...base,
      kind,
      manifest: {
      ...
      lockfile: {
        ...
        beforeDigest: readNullableSha256Digest(lockfile.beforeDigest, `${path}.lockfile.beforeDigest`),
        packageName: readBoundedString(lockfile.packageName, `${path}.lockfile.packageName`, 256),
        beforeVersion: nullableString(lockfile.beforeVersion, `${path}.lockfile.beforeVersion`, 256),
        afterVersion: nullableString(lockfile.afterVersion, `${path}.lockfile.afterVersion`, 256),
        targetIntegrity: lockfile.targetIntegrity === null
          ? null
          : readArtifactIntegrity(lockfile.targetIntegrity, `${path}.lockfile.targetIntegrity`),
      },
    ...
    return immutableJsonClone(decoded) as PlanReviewEvidence
  }
  if (kind === 'skill') {
