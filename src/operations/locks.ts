import { readBoundedString, readStrictRecord } from '../domain/codec.ts'
import { failDomain } from '../domain/errors.ts'
import { immutableJsonClone } from '../domain/json.ts'
import type { TargetLease, TargetLockState } from './types.ts'

function decodeLockState(value: unknown): TargetLockState {
  const record = readStrictRecord(value, ['leases'], 'targetLocks')
  if (!Array.isArray(record.leases)) failDomain('invalid-data', 'targetLocks.leases must be an array')
  const leases = record.leases.map((lease, index): TargetLease => {
...
export function acquireTarget(
  ...
  if (targetLease !== undefined) {
    if (targetLease.operationId === operationId) return state
