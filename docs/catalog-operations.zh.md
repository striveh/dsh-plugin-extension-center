...
Alpha 路径是受保护的固定候选专用流程，不是开放式 curator 表单。它只能把准确已提交且已签名的公开 revision 11 相邻推进到 revision 12，把 rc.2 entry set 替换为 package 已审查的 `microsoft/skills` `wiki-page-writer` 两个 commit：`6142f8e60ac58372845c0fcdd2dbf043cd1bb698` 与 `67ae723a23ba880e3e5c8a3e5e2320092024476e`，并把它们的 compatibility 和所需 Host dependency 设为准确官方 DSH `0.1.2-alpha.3`、commit `dd6322d604e00eec1ba5e0c8541159906a21094a`。成功的 stable post-publication run `33130950000` 已证明公开 r11 部署与刷新，alpha package 现已内置该准确 r11 前序；准确前序 preflight 已通过。这只关闭 r11 前置条件。r12 lifecycle、admission、review、Pages 部署、runtime refresh 与公开 artifact 生命周期证据仍是 `Pending`/RED。搜索结果、用户 URL、模型输出或测试 fixture 都不能改变这些坐标，也不能替代这些 receipt。
...
最终 receipt 字段是 `activeCandidateAbsent`，并不声称 inventory row 或恢复材料已经消失。最终 Uninstall 必须让候选变为 inactive 且不可见，同时保留 tombstone 和 Restore 使用的准确 rollback material。该 producer 不执行也不声称 Purge；历史普通用户生命周期设计把这项未来破坏性证明分配给独立 lane。
...
Admission workflow 只上传 canonical 签名 `plugins.json`、状态为 `prepared-for-review` 的 evidence record 与 checksum；它不会 commit、deploy 或改写 Profile。维护者必须通过 pull request 审查并提交准确 byte，再由独立 Pages 部署与 runtime refresh receipt 证明发布。准确公开 artifact 安装与普通用户 Store 生命周期仍是后续独立门禁；把 Center 发布到 npm 不在这些门禁中。在真实 lifecycle producer run、catalog signing run、pull-request commit、Pages receipt 与公开 artifact 生命周期 receipt 全部存在前，alpha catalog admission、部署与普通用户使用都保持 `Pending`；包括 `tests/support/alpha-catalog.ts` 在内的单元测试 fixture 不能满足其中任何门禁。
