...
人工 Pages workflow 只从 `main` 运行，根据 package 内置 bootstrap 与准确已提交后继推导预期 revision，验证该相邻签名迁移后再复制准确 canonical byte。它没有签名 secret，也不能构造其他 revision。已提交 revision 是发布输入，不是部署证据。恢复候选 rc.2 只有在 `https://striveh.github.io/dsh-plugin-extension-center/plugins.json` 以 HTTP 200 返回这些 revision-10 byte，且 runtime 记录同一 Center root 中从 rc.0 cache 更新到 rc.2 并成功完成 revision 9 到 10 刷新后，才算发布完成；rc.1 未通过该门禁。
