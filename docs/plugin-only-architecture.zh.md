...
本文定义扩展中心适配 DeepSeek Harness `0.1.2-alpha.3` 的实现边界。兼容性目标是未经修改的官方源码 tag `dsh-v0.1.2-alpha.3`，对应 `deepseek-ai/deepseek-harness@dd6322d604e00eec1ba5e0c8541159906a21094a`。Center 以 GitHub 托管的确定性 tarball 分发，不要求发布到 npm。Stable Center `0.1.0` 证据仍是历史证据，只适用于官方 DSH `0.1.1-rc.2`。
...
实现只能依赖准确官方 DSH `0.1.2-alpha.3` 暴露的公开行为：
...
每项动作都有且只有一个扩展中心自有 operation record、单调 revision、不可变已批准 plan、逐 target 锁、append-only journal、验证结果和内容寻址 receipt。扩展中心的 archive 存储采用私有目录、no-follow 读取、canonical path、排他创建和原子 pointer 替换。调用官方 Plugin CLI 之前，扩展中心会对已观测的 Profile dependency 加 revision fence，并拒绝外来或已漂移 target。官方 Profile package manager 始终是 dependency、lock、`node_modules`、Bundle membership 与 Loader row 的物理 owner。官方 `0.1.2-alpha.3` 外部 CLI 不向扩展中心提供 lock 或 compare-and-swap token，因此逐 target 锁只能串行化 Center operation。验收会运行一个受控的外部 CLI ABA 顺序，并要求进入 `recovery-required` 而不是虚假成功；该 receipt 不声称覆盖所有可能的进程交错。
...
## 可选扩展 Release provenance

本节记录历史与未来的公开 Release 证明模型，不属于当前最新版官方 DSH 兼容门禁，也不要求把 Center 发布到 npm。
...
证明按 receipt 拆分，不依赖会随时间变化的状态段落。当前最新版 DSH 兼容性 receipt 绑定确定性 Center tarball、准确官方 DSH package 与 integrity、标准 Plugin CLI 安装和卸载、真实 Host/Client 商店与 RPC 观测，以及未被改动的官方 DSH package tree。随包签名 revision 11 目录保持不可变的历史 rc.2 数据，因此准确条目取得经过审查的 alpha.3 签名准入前，当前 alpha.3 child-candidate mutation 保持 unavailable。历史 stable `0.1.0` receipt 仍只适用于官方 DSH `0.1.1-rc.2`。Live provider 运行只是 advisory，不能替代确定性证据。

## 兼容与扩展 P0 验收

兼容性门禁通过标准 Plugin CLI，把一个确定性 packed Center 安装到准确官方 `dsh@0.1.2-alpha.3` 的隔离 Profile；Center 不需要发布到 npm。扩展候选的完整生命周期 receipt 还必须证明：
...
修改过的 DSH checkout 或未打包源码不能满足当前兼容门禁。仅 mock 的 owner 或只读商店不能满足独立的扩展候选生命周期门禁。Live provider smoke 可以增加兼容性证据，但不能替代未来的确定性 Replay receipt。
