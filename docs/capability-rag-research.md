...
2. **Managed runtime evidence:** center-owned inventory plus official Skill, MCP Tool, Loader, and Profile observations.
...
The Store uses the same deterministic eligibility filter but lets the user browse all admitted matches, filter, and compare up to three candidates. User choice may resolve a trade-off but cannot override trust, compatibility, lifecycle, integrity, or authority policy. Both entrances use the same Host-derived intent schema and transaction engine. Only when candidate, scope, operation, and desired state match must their canonical mutation and authority core match; origin, task-only continuation, idempotency, and plan identity remain per request. Manual MCP Install and task-composed Install-and-Enable therefore share admission and owners without pretending to be the same intent or plan.
...
| Install after confirmation | Host-owned transaction, never model execution |
...
For a Skill or MCP connection whose owner can mutate and expose the new contribution live, the acquisition call may wait for confirmation, verify the new winner or Tool generation, re-read the exact Agent-visible capability set, and dispatch a new step for the same objective. Published rc.2 lacks the dynamic MCP connection owner; a separate local DSH HEAD implements it for integration testing, but the path remains a release gate until an exact DSH release publishes the owner. Store-originated acquisition has no continuation claim.

A Profile Plugin requires a real Host restart. Published `dsh-v0.1.1-rc.2` persists Session history but has no generic restart-safe continuation owner. The local DSH HEAD now implements a durable single-use claim with cancellation and supersession fences and at-most-once dispatch of the bound continuation message. It does not promise exactly-once task completion. Until an exact DSH release publishes and passes that gate, Plugin acquisition ends at `restart-required`; after restart, a new turn must re-check evidence before continuing.
